...
… (${e("media.previewTruncated")})`:g;v.innerHTML=`<pre class="media-lb-text">${t(C)}</pre>`}).catch(()=>{const g=document.querySelector("#modal-back .media-lb-stage");g&&(g.innerHTML=`<div class="error-box">${t(e("media.previewFail"))}</div>`)})}function Xa(){return`
  ...
  </footer>`}function Cs(){return{dashboard:e("nav.dashboard"),chat:e("nav.chat"),chats:e("nav.chats"),keys:e("nav.keys"),documents:e("nav.documents"),audit:e("nav.audit"),settings:e("nav.settings"),apiFeatures:e("nav.apiFeatures"),media:e("nav.media"),catalog:e("nav.catalog"),usage:e("nav.usage"),ddos:e("nav.ddos"),queue:e("nav.queue"),pm2:e("nav.pm2"),system:e("nav.system"),support:e("nav.support")}[d.page]||e("brand")}function It(){document.body.classList.remove("nav-open")}function Ls(){document.body.classList.add("nav-open")}function le(a){return`
  <div class="app-shell">
    <header class="mobile-bar">
      <button type="button" class="icon-btn" id="nav-open" aria-label="${t(e("shell.menu"))}">☰</button>
      <div class="mobile-title">${t(Cs())}</div>
      ${aa()}
  ...
  ${d.modal||""}
  `}function ce(a,s){return`<button type="button" class="nav-btn ${d.page===a?"active":""}" data-nav="${a}">${t(s)}</button>`}function de(){It(),document.querySelectorAll("[data-nav]").forEach(s=>{s.onclick=()=>{It(),fa(s.dataset.nav)}});const a=()=>Ht(!0);document.getElementById("btn-logout")?.addEventListener("click",a),document.getElementById("btn-logout-mobile")?.addEventListener("click",a),document.getElementById("nav-open")?.addEventListener("click",Ls),document.getElementById("nav-backdrop")?.addEventListener("click",It),document.addEventListener("keydown",s=>{s.key==="Escape"&&It()},{once:!0}),document.querySelectorAll("[data-lang]").forEach(s=>{s.onclick=()=>{Ga(s.dataset.lang),Qt().catch(b)}}),$a(document)}function Hs(a){if(!a)return"";const s=(a.getAttribute("data-label")||"").trim();if(s)return s;const o=a.querySelector(".th-sort-btn");return(o?[...o.childNodes].filter(n=>n.nodeType===Node.TEXT_NODE).map(n=>n.textContent||"").join(""):a.textContent||"").replace(/[▲▼]/g,"").replace(/\s+/g," ").trim()}function $a(a){(a||document).querySelectorAll("table.data-table").forEach(o=>{const i=[...o.querySelectorAll("thead th")].map(Hs);o.querySelectorAll("tbody tr").forEach(n=>{n.classList.contains("empty-row")||[...n.children].forEach((r,l)=>{const u=[...r.children],m=r.classList.contains("row-actions")||!!r.querySelector(":scope > .row-actions")||u.length>0&&u.every(p=>p.matches("button, .btn, .row-actions"));if(r.classList.toggle("is-actions",m),r.classList.toggle("is-primary",l===0&&!m),m){r.removeAttribute("data-label");return}i[l]&&r.setAttribute("data-label",i[l])})})})}function b(a){console.error(a),Q(a.message||String(a))}async function Ya(){if(!d.key)return!1;const a=await I("/me");return d.me=a.data,!0}async function $t(a=!1){try{const s=await I(`/models${a?"?refresh=1":""}`);return d.models=s.data?.models||[],s.data}catch{return d.models=[],{models:[],source:"fallback",defaultModel:""}}}async function wt(){try{const a=await I("/keys?all=1");d.keys=a.data||[]}catch{d.keys=[]}}function ke(a){const s=(Array.isArray(a)?a:[a]).filter(Boolean);return s.length?`<div class="page-meta" role="status">${s.map(i=>`<span>${typeof i=="string"?t(i):i}</span>`).join('<span class="page-meta-sep" aria-hidden="true">·</span>')}</div>`:""}function Oe({title:a,hint:s,meta:o,searchHtml:i,gridHtml:n}){return`
    ...
    </div>`;document.getElementById("app").insertAdjacentHTML("beforeend",r);const l=()=>he();document.getElementById("modal-close").onclick=l,document.getElementById("modal-back").onclick=m=>{m.target.id==="modal-back"&&l()};const u=m=>{m.key==="Escape"&&(l(),document.removeEventListener("keydown",u))};document.addEventListener("keydown",u)}async function Za(){const a="ysk-omni admin otp";document.getElementById("app").innerHTML=`
      ...
      <aside class="dash-side">
        ...
        <div class="panel dash-panel">
          ...
          <div class="panel-pad dash-prot">
            ...
            <div class="dash-prot-meta muted">
              ${t(e("dash.proxy"))}: ${t(Ds(i.proxyIpSource))}
...
`):s,hasRoles:!0,blocks:r}}async function pt(){await Promise.all([$t(),wt()]);const a=d.chatFilter,s=new URLSearchParams;if(s.set("limit",String(a.limit)),s.set("offset",String(a.offset)),a.status&&s.set("status",a.status),a.model&&s.set("model",a.model),a.apiKeyId&&s.set("apiKeyId",a.apiKeyId),a.q&&s.set("q",a.q),a.from&&s.set("from",new Date(a.from).toISOString()),a.to){const c=new Date(a.to);c.setHours(23,59,59,999),s.set("to",c.toISOString())}a.policyMode&&s.set("policyMode",a.policyMode),a.hasDocuments!==""&&s.set("hasDocuments",a.hasDocuments),Ee(s,a);const o=await I(`/chats?${s}`),i=o.total||0,n=[`<option value="">${t(e("chats.allModels"))}</option>`,...d.models.map(c=>`<option value="${t(c)}" ${a.model===c?"selected":""}>${t(c)}</option>`)].join(""),r=[`<option value="">${t(e("chats.allKeys"))}</option>`,...d.keys.map(c=>`<option value="${c.id}" ${a.apiKeyId===c.id?"selected":""}>${t(c.name)} (${t(c.keyPrefix)})</option>`)].join(""),l=(o.items||[]).map(c=>{const w=es(c.promptPreview||""),h=!!w.system,g=h?w.body.slice(0,160):c.promptPreview||"";return`
    <tr>
      ...
      <td>${Os(c.documents)} ${c.documentCount?`<span class="muted">×${c.documentCount}</span>`:""}</td>
  ...
  `),de(),dt("chats",d.chatFilter,()=>pt().catch(b)),Ge(d.chatFilter,()=>pt().catch(b));const p=()=>{d.chatFilter.q=document.getElementById("f-q").value.trim(),d.chatFilter.status=document.getElementById("f-status").value,d.chatFilter.model=document.getElementById("f-model").value,d.chatFilter.apiKeyId=document.getElementById("f-key").value,d.chatFilter.policyMode=document.getElementById("f-mode").value,d.chatFilter.from=document.getElementById("f-from").value,d.chatFilter.to=document.getElementById("f-to").value,d.chatFilter.hasDocuments=document.getElementById("f-docs").checked?"true":"",d.chatFilter.offset=0,pt().catch(b)};document.querySelector("[data-filter-apply]").onclick=p,document.getElementById("f-q").onkeydown=c=>{c.key==="Enter"&&p()},document.querySelector("[data-filter-reset]").onclick=()=>{d.chatFilter={q:"",status:"",model:"",apiKeyId:"",from:"",to:"",policyMode:"",hasDocuments:"",sortBy:"createdAt",sortDir:"desc",limit:50,offset:0},pt().catch(b)},document.querySelectorAll("[data-chat]").forEach(c=>{c.onclick=()=>ts(c.dataset.chat)})}async function ts(a){const{data:s}=await I(`/chats/${a}`),o=s.response||{},i=s.documents||[];let n=`<p class="muted">${t(e("chats.noAttach"))}</p>`;if(i.length){const m=[];for(const p of i){let c="";if(St(p.mimeType))try{const w=await I(`/documents/${p.id}`),h=await os(w.data||{id:p.id,isImage:!0,mimeType:p.mimeType});h?.src&&(c=`<img class="preview" src="${h.src}" alt="${t(p.originalName)}" />`)}catch{c=`<span class="muted">${t(e("chats.previewFailed"))}</span>`}m.push(`
        ...
        </div>`)}n=`<div class="attach-list">${m.join("")}</div>`}const r=es(s.prompt||""),l=r.system?`<div class="block block-system">
...
${r.data?.key||JSON.stringify(r.data)}`);const u=document.getElementById("k-save");u&&(u.textContent=e("chats.close"),u.onclick=()=>{he(),Fe().catch(b)})}}catch(r){b(r)}}}function as(a){return e(a==="filesystem"?"docs.storageFs":"docs.storageDb")}async function ss(a,s){try{const o=await fetch(`${ht}/documents/${a}/download`,{headers:d.key?{Authorization:`Bearer ${d.key}`}:{}});if(!o.ok){const l=await o.text();let u=l;try{u=JSON.parse(l).error?.message||l}catch{}throw new Error(u||e("docs.downloadFail"))}const i=await o.blob(),n=URL.createObjectURL(i),r=document.createElement("a");r.href=n,r.download=s||"download",document.body.appendChild(r),r.click(),r.remove(),URL.revokeObjectURL(n)}catch(o){Q(o.message||e("docs.downloadFail"))}}async function st(){await wt();const a=d.docFilter,s=new URLSearchParams({limit:String(a.limit),offset:String(a.offset)});if(a.q&&s.set("q",a.q),a.apiKeyId&&s.set("apiKeyId",a.apiKeyId),a.storageType&&s.set("storageType",a.storageType),a.from&&s.set("from",new Date(a.from).toISOString()),a.to){const c=new Date(a.to);c.setHours(23,59,59,999),s.set("to",c.toISOString())}Ee(s,a);const o=await I(`/documents?${s}`),i=o.total??0,n=o.meta||{},r=A("docs.storageHint",{dir:n.storageDir||"—",dbMax:Ae(n.documentDbMaxBytes),upMax:Ae(n.uploadMaxBytes)}),l=[`<option value="">${t(e("common.all"))}</option>`,...d.keys.map(c=>`<option value="${c.id}" ${a.apiKeyId===c.id?"selected":""}>${t(c.name)}</option>`)].join(""),u=(o.data||[]).map(c=>`
    ...
    ${m}
    ${p}
  `),de(),dt("docs",d.docFilter,()=>st().catch(b)),Ge(d.docFilter,()=>st().catch(b)),document.querySelector("[data-filter-apply]").onclick=()=>{d.docFilter.q=document.getElementById("df-q").value.trim(),d.docFilter.apiKeyId=document.getElementById("df-key").value,d.docFilter.storageType=document.getElementById("df-storage").value,d.docFilter.from=document.getElementById("df-from").value,d.docFilter.to=document.getElementById("df-to").value,d.docFilter.offset=0,st().catch(b)},document.querySelector("[data-filter-reset]").onclick=()=>{d.docFilter={q:"",apiKeyId:"",storageType:"",from:"",to:"",sortBy:"createdAt",sortDir:"desc",limit:20,offset:0},st().catch(b)},document.querySelectorAll("[data-doc]").forEach(c=>{c.onclick=()=>ns(c.dataset.doc)}),document.querySelectorAll("[data-dl]").forEach(c=>{c.onclick=()=>ss(c.getAttribute("data-dl"),c.getAttribute("data-name")||"file")}),document.querySelectorAll("[data-del]").forEach(c=>{c.onclick=async()=>{await Z({message:e("docs.confirmDel"),variant:"danger",confirmText:e("docs.delete")})&&(await I(`/documents/${c.dataset.del}`,{method:"DELETE"}),st().catch(b))}})}async function _s(a){const s=await fetch(`${ht}/documents/${a}/download`,{headers:d.key?{Authorization:`Bearer ${d.key}`}:{}});if(!s.ok){const i=await s.text();let n=i;try{n=JSON.parse(i)?.error?.message||i}catch{}throw new Error(n||e("docs.downloadFail"))}const o=await s.blob();return URL.createObjectURL(o)}async function os(a){if(a?.imageDataUrl)return{src:a.imageDataUrl,revoke:null};if(a?.isImage||St(a?.mimeType)){const s=await _s(a.id);return{src:s,revoke:s}}return null}async function ns(a){const{data:s}=await I(`/documents/${a}`);let o,i=null;try{const l=await os(s);l?(i=l.revoke,o=`<img class="preview doc-preview-img" src="${l.src}" alt="${t(s.originalName||"")}" />`):s.isBinary||s.content==null?o=`<div class="data-empty"><div class="data-empty-icon">⧉</div><strong>${t(e("docs.binaryPreview"))}</strong></div>`:o=`<div class="pre" id="doc-content">${t(s.content||e("chats.none"))}</div>`}catch{o=`<div class="data-empty"><div class="data-empty-icon">⧉</div><strong>${t(e("chats.previewFailed")||e("docs.binaryPreview"))}</strong></div>`}const n=`${as(s.storageType)}${s.storagePath?` · ${s.storagePath}`:""}`;rt({title:e("docs.detail"),subtitle:`${t(s.originalName)} · ${t(s.mimeType)} · ${Ae(s.sizeBytes)}<br/><span class="muted">${t(e("docs.storage"))}: ${t(n)}</span>`,size:"lg",bodyHtml:`
      <div class="block">
  ...
  `),de(),Ge(d.usageFilter,()=>pe().catch(b)),document.getElementById("btn-usage-refresh").onclick=()=>pe().catch(b),document.querySelectorAll("[data-usage-tab]").forEach(M=>{M.onclick=()=>{const P=M.dataset.usageTab==="key"?"key":"model";d.usageFilter.tab!==P&&(d.usageFilter.tab=P,pe().catch(b))}}),document.getElementById("umodel-prev")?.addEventListener("click",()=>{d.usageFilter.modelPage=Math.max(0,a.modelPage-1),pe().catch(b)}),document.getElementById("umodel-next")?.addEventListener("click",()=>{(a.modelPage+1)*r<u&&(d.usageFilter.modelPage+=1,pe().catch(b))}),document.getElementById("umodel-limit")?.addEventListener("change",M=>{d.usageFilter.pageSize=Number(M.target.value)||10,d.usageFilter.modelPage=0,pe().catch(b)}),document.getElementById("ukey-prev")?.addEventListener("click",()=>{d.usageFilter.keyPage=Math.max(0,a.keyPage-1),pe().catch(b)}),document.getElementById("ukey-next")?.addEventListener("click",()=>{(a.keyPage+1)*r<w&&(d.usageFilter.keyPage+=1,pe().catch(b))}),document.getElementById("ukey-limit")?.addEventListener("change",M=>{d.usageFilter.pageSize=Number(M.target.value)||10,d.usageFilter.keyPage=0,pe().catch(b)}),document.querySelectorAll("#usage-tab-model [data-filter-apply]").forEach(M=>{M.onclick=()=>{d.usageFilter.modelQ=document.getElementById("uf-model")?.value?.trim()||"",d.usageFilter.modelPage=0,pe().catch(b)}}),document.querySelectorAll("#usage-tab-model [data-filter-reset]").forEach(M=>{M.onclick=()=>{d.usageFilter.modelQ="",d.usageFilter.modelPage=0,pe().catch(b)}}),document.querySelectorAll("#usage-tab-key [data-filter-apply]").forEach(M=>{M.onclick=()=>{d.usageFilter.keyQ=document.getElementById("uf-key")?.value?.trim()||"",d.usageFilter.keyActive=document.getElementById("uf-active")?.value||"",d.usageFilter.keyPage=0,pe().catch(b)}}),document.querySelectorAll("#usage-tab-key [data-filter-reset]").forEach(M=>{M.onclick=()=>{d.usageFilter.keyQ="",d.usageFilter.keyActive="",d.usageFilter.keyPage=0,pe().catch(b)}})}function Ta(a){const s=a.versionStatus||(a.updateAvailable?"update_available":a.latest?"up_to_date":"unknown");return s==="update_available"?{badge:`<span class="badge warn" title="${t(e("system.statusHintUpdate"))}">${t(e("system.badgeUpdate"))}</span>`,hint:e("system.statusHintUpdate")}:s==="ahead"?{badge:`<span class="badge pending" title="${t(e("system.statusHintAhead"))}">${t(e("system.badgeAhead"))}</span>`,hint:e("system.statusHintAhead")}:s==="up_to_date"?{badge:`<span class="badge success" title="${t(e("system.statusHintOk"))}">${t(e("system.badgeOk"))}</span>`,hint:e("system.statusHintOk")}:{badge:`<span class="badge pending" title="${t(e("system.statusHintUnknown"))}">${t(e("system.badgeUnknown"))}</span>`,hint:e("system.statusHintUnknown")}}function Qs(a){return e(a==="git"?"system.channelGit":a==="npm-global"?"system.channelNpmGlobal":a==="npm-local"?"system.channelNpmLocal":"system.channelUnknown")}function Ws(a){return a==="required"?e("system.levelRequired"):a==="recommended"?e("system.levelRecommended"):a==="optional"?e("system.levelOptional"):a==="bundled"?e("system.levelBundled"):a||"—"}function zs(a){return a.installed?a.ok?`<span class="badge success">${t(e("system.softOk"))}</span>`:`<span class="badge warn">${t(e("system.softWarn"))}</span>`:a.level==="required"||a.level==="bundled"?`<span class="badge error">${t(e("system.softMissing"))}</span>`:`<span class="badge pending">${t(e("system.softMissing"))}</span>`}function Aa(a){return a==="up"?`<span class="badge success">${t(e("system.up"))}</span>`:`<span class="badge error">${t(e("system.down"))}</span>`}async function ot(){const{data:a}=await I("/system");if(d.page!=="system")return;const s=a.version||{},o=Ta(s),i=a.software||{checks:[],allRequiredOk:!0},n=i.checks||[],r=d.systemTab==="package"||d.systemTab==="env"||d.systemTab==="sessions"?d.systemTab:"software";d.systemTab=r;let l={data:[],total:0};try{const P=new URLSearchParams({limit:r==="sessions"?"50":"1",offset:"0"});r==="sessions"&&d.grokSessionQ&&P.set("q",d.grokSessionQ);const K=await I(`/grok/sessions?${P}`);l={data:K.data||[],total:K.total||0}}catch(P){l={data:[],total:0,error:P.message||String(P)}}const u=n.map(P=>`
      ...
      </tr>`).join(""),m=i.allRequiredOk?`<span class="badge success">${t(e("system.allRequiredOk"))}</span>`:`<span class="badge error">${t(e("system.requiredMissing"))}</span>`,p=a.encryption&&a.encryption.ready,c=Qs(s.channel),w=s.installSource?`${c} · ${s.installSource}`:c,h=$e({headHtml:`
...
`)),Sa(a.autoBanEnabled),_e()}function Sa(a){const s=document.getElementById("ddos-auto-badge");s&&(s.className=`badge ${a?"success":"pending"}`,s.textContent=e(a?"ddos.autoOn":"ddos.autoOff"))}function Js(a){const s=(r,l)=>`<label class="data-filter-check policy-enable"><input type="checkbox" id="${r}" ${l?"checked":""} /> <span>${t(e("ddos.enableRule"))}</span></label>`,o=(r,l,u,m="1")=>`<label>${t(r)}<input type="number" id="${l}" value="${t(String(u))}" min="1" step="${m}" /></label>`,i=la(a),n=ds(i);return`
...
`))}</textarea>
    ...
    </div>`}function Xs(a){return a?.length?a.map(s=>`
    <tr>
      <td>${se(s.at)}</td>
...
`+A("pm2.portAfterRestart",{port:k}))}catch(x){b(x)}}),document.getElementById("pm2-cfg-reset").onclick=async()=>{if(await Z({message:e("pm2.confirmReset"),variant:"danger",confirmText:e("pm2.resetConfig")}))try{await I("/pm2/config/reset",{method:"POST",body:"{}"}),Le().catch(b)}catch(k){b(k)}}}let ae=[],ve=null,ie=[],ft=!1,Ne=0;const Re=new Map,H={keyId:"",model:"",reasoning:!0,effort:"",resumeId:"",forkSession:!1,memory:!1,noPlan:!1,permissionMode:"",systemPrompt:"",systemOpen:!1,settingsOpen:!1},_={mode:"full",recentN:6,summary:"",summaryAt:null,summarySourceCount:0},ao=3,rs=40,so=20,oo=2200,O={conversationId:null,historyPage:0,historyLimit:20,historyQ:"",historyTotal:0,historyItems:[],historyLoading:!1,historyOpenMobile:!1,saving:!1,saveQueued:!1,renamingId:null};let Xt=null;const Ot=10,wa=/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;function no(a){const s=String(a||"").split(/[/\\]/).pop()||"",o=s.lastIndexOf(".");return o<0?"":s.slice(o).toLowerCase()}function _t(a){return Qa.has(no(a))}function Rt(){return e("chat.formatsHint")}function cs(){_.mode="full",_.recentN=6,_.summary="",_.summaryAt=null,_.summarySourceCount=0,Ne=0,Re.clear()}function io(a){const s=(a?.title||"").trim();if(s)return s;const o=(a?.preview||"").trim();return o||e("chat.untitled")}function lo(){return ae.filter(a=>!a.streaming).map(a=>{const s={role:a.role,content:a.content||""};return a.reasoning&&(s.reasoning=a.reasoning),a.docs&&a.docs.length&&(s.docs=a.docs),a.error&&(s.error=!0),s})}function ro(){return{contextMode:_.mode,contextRecentN:_.recentN,summaryText:_.summary||"",summaryAt:_.summaryAt,summarySourceCount:_.summarySourceCount||0}}function co(a){a&&(_.mode=a.contextMode==="summary"||a.contextMode==="recent"?a.contextMode:"full",_.recentN=Math.min(40,Math.max(2,Number(a.contextRecentN)||6)),_.summary=(a.summaryText||"").trim(),_.summaryAt=a.summaryAt||null,_.summarySourceCount=Number(a.summarySourceCount)||0,_.mode==="summary"&&!_.summary&&(_.mode="full"))}function us(a){return a.reduce((s,o)=>s+(o.content||"").length+(o.reasoning||"").length,0)}function ms(){const a=ae.filter(s=>!s.streaming);return a.length<2?!1:a.length>=ao?!0:us(a)>=800}function uo(){const a=(H.systemPrompt||"").trim(),s=ae.filter(l=>!l.streaming),o=Math.min(40,Math.max(2,Number(_.recentN)||6));let i=s.map(l=>({role:l.role,content:l.content||""})),n=a;if(_.mode==="summary"&&_.summary){const l=(bt()==="zh-Hant"?`【先前對話摘要 — 僅供延續語境，完整記錄仍在用戶介面】
`:`[Prior conversation summary — full history remains in the UI]
`)+_.summary;n=n?`${n}

${l}`:l;const u=s.slice(_.summarySourceCount||0);i=(u.length?u:s.slice(-o)).slice(-o).map(p=>({role:p.role,content:p.content||""}))}else _.mode==="recent"&&(i=s.slice(-o).map(l=>({role:l.role,content:l.content||""})));const r=i.map(l=>({role:l.role,content:l.content}));return n&&r.unshift({role:"system",content:n}),r}function mo(){return _.mode==="summary"&&_.summary?A("chat.ctxModeSummaryLabel",{n:_.recentN}):_.mode==="recent"?A("chat.ctxModeRecentLabel",{n:_.recentN}):e("chat.ctxModeFullLabel")}function lt(){const a=document.getElementById("chat-compress");if(!a)return;const s=ae.some(n=>n.streaming),o=!!ve||ft||s,i=ms();a.disabled=o||!i,a.textContent=e(ft?"chat.compressing":"chat.compress"),a.title=e(i?"chat.compress":"chat.compressNeedMore"),je(),tt()}function tt(){const a=document.getElementById("chat-ctx-mode"),s=document.getElementById("chat-ctx-n");if(a){const o=_.mode==="summary"&&!_.summary?"full":_.mode;a.value=o;const i=a.querySelector('option[value="summary"]');i&&(i.disabled=!_.summary)}s&&(s.value=String(_.recentN),s.disabled=_.mode==="full"),Ft()}function je(){const a=document.getElementById("chat-compress-banner");if(!a)return;const s=!!_.summary,o=ae.filter(r=>!r.streaming).length>40||us(ae)>6e4;if(!s&&_.mode==="full"&&!o){a.hidden=!0,a.innerHTML="";return}a.hidden=!1;const i=s?_.summary.length>160?`${_.summary.slice(0,159)}…`:_.summary:"",n=o?`<p class="chat-compress-warn">${t(e("chat.ctxLongHint"))}</p>`:"";a.innerHTML=`
    <div class="chat-compress-banner-inner">
      <div class="chat-compress-banner-text">
        <strong>${t(e("chat.ctxPolicyTitle"))}</strong>
        <span class="muted">${t(mo())}</span>
    ...
    </div>`,document.getElementById("chat-summary-view")?.addEventListener("click",()=>{ps()})}async function ps(){if(!_.summary){Q(e("chat.compressNeedSummary"));return}const a=_.summaryAt?se(_.summaryAt):"—",s=za(_.summary);Te&&Ve(!1);const o=document.createElement("div");return o.className="ui-dialog-back",o.id="ui-dialog-back",o.dataset.cancelable="1",o.innerHTML=`
...
`)}async function fo(){if(ft||ve||ae.some(n=>n.streaming)){Q(e("chat.compressBusy"));return}const a=ae.filter(n=>!n.streaming);if(!ms()){Q(e("chat.compressNeedMore"));return}if(!await Z({title:e("chat.compress"),message:e("chat.compressConfirm"),variant:"confirm",confirmText:e("chat.compress")}))return;const s=Et();if(!s){Q(e("chat.needKey"));return}me(),ft=!0,lt();const o=document.getElementById("chat-send");o&&(o.disabled=!0);const i=document.getElementById("chat-stream-status");i&&(i.hidden=!1,i.textContent=e("chat.compressing"));try{let n=po(a);_.summary&&(n=(bt()==="zh-Hant"?`先前摘要：
${_.summary}
...
`)+n}]},m=Pt();m&&(u.apiKeyId=m);const p=await fetch("/admin/api/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${s}`,"Content-Type":"application/json"},body:JSON.stringify(u)});if(!p.ok){const h=await p.text();let g=h;try{g=JSON.parse(h).error?.message||h}catch{}throw new Error(g||e("chat.compressFail"))}const c=await p.json();let w=c?.choices?.[0]?.message?.content||c?.choices?.[0]?.delta?.content||"";if(typeof w!="string"&&(w=String(w||"")),w=w.trim().replace(/^【對話摘要】\s*/u,"").replace(/^\[Conversation summary\]\s*/i,""),!w)throw new Error(e("chat.compressFail"));_.summary=w,_.summaryAt=new Date().toISOString(),_.summarySourceCount=a.length,_.mode="summary",je(),tt(),Q(""),i&&(i.hidden=!1,i.textContent=e("chat.compressOk"),setTimeout(()=>{const h=document.getElementById("chat-stream-status");h&&h.textContent===e("chat.compressOk")&&(h.hidden=!0,h.textContent="")},2800)),await kt().catch(()=>{}),await ps()}catch(n){Q(n.message||e("chat.compressFail"))}finally{ft=!1,lt(),o&&(o.disabled=!1),i&&i.textContent===e("chat.compressing")&&(i.hidden=!0,i.textContent="")}}function ra(a){O.historyOpenMobile=!!a,document.body.classList.toggle("chat-history-open",O.historyOpenMobile)}function ca(){ra(!1)}async function Ue(){if(d.key){O.historyLoading=!0,De();try{const a=O.historyPage*O.historyLimit,s=new URLSearchParams({limit:String(O.historyLimit),offset:String(a)});O.historyQ.trim()&&s.set("q",O.historyQ.trim());const o=await I(`/conversations?${s}`);O.historyItems=o.data||[],O.historyTotal=o.total??0}catch(a){O.historyItems=[],O.historyTotal=0,console.warn(a)}finally{O.historyLoading=!1,De()}}}function De(){const a=document.getElementById("chat-history-list"),s=document.getElementById("chat-history-pager");if(a){if(O.historyLoading&&!O.historyItems.length?a.innerHTML=`<li class="chat-history-empty">${t(e("common.loading"))}</li>`:O.historyItems.length?a.innerHTML=O.historyItems.map(o=>{const i=O.conversationId===o.id?" is-active":"",n=io(o),r=o.title&&o.preview&&o.preview!==o.title?o.preview:o.model||A("chat.msgs",{n:o.messageCount||0}),l=O.renamingId===o.id,u=n,m=l?`<input type="text" class="chat-history-title-input" data-title-input="${t(o.id)}" value="${t(u)}" maxlength="120" placeholder="${t(e("chat.renamePh"))}" aria-label="${t(e("chat.renamePh"))}" />
    ...
    `;const m=document.getElementById("chat-hist-prev"),p=document.getElementById("chat-hist-next");m&&(m.onclick=()=>{O.historyPage>0&&(O.historyPage-=1,Ue())}),p&&(p.onclick=()=>{(O.historyPage+1)*o<O.historyTotal&&(O.historyPage+=1,Ue())})}if(a.querySelectorAll("[data-open-conv]").forEach(o=>{const i=o.getAttribute("data-open-conv");if(!i)return;let n=null;const r=()=>{n&&(clearTimeout(n),n=null)};o.addEventListener("click",l=>{O.renamingId||l.target instanceof Element&&l.target.closest(".chat-history-item-actions")||(r(),n=setTimeout(()=>{n=null,!O.renamingId&&Ha(i)},280))}),o.addEventListener("dblclick",l=>{l.preventDefault(),l.stopPropagation(),r(),Yt(i)}),o.addEventListener("keydown",l=>{(l.key==="Enter"||l.key===" ")&&(l.preventDefault(),O.renamingId||Ha(i))})}),a.querySelectorAll("[data-title-label]").forEach(o=>{o.addEventListener("dblclick",i=>{i.preventDefault(),i.stopPropagation();const n=o.getAttribute("data-title-label");n&&Yt(n)})}),a.querySelectorAll("[data-rename-conv]").forEach(o=>{o.addEventListener("click",i=>{i.preventDefault(),i.stopPropagation();const n=o.getAttribute("data-rename-conv");n&&Yt(n)})}),a.querySelectorAll("[data-del-conv]").forEach(o=>{o.addEventListener("click",i=>{i.preventDefault(),i.stopPropagation();const n=o.getAttribute("data-del-conv");n&&bo(n)})}),O.renamingId){const o=String(O.renamingId).replace(/\\/g,"\\\\").replace(/"/g,'\\"'),i=a.querySelector(`[data-title-input="${o}"]`);i instanceof HTMLInputElement&&(yo(i,O.renamingId),requestAnimationFrame(()=>{i.isConnected&&(i.focus(),i.select())}))}}}function Yt(a){a&&(O.renamingId&&O.renamingId!==a&&(O.renamingId=null),O.renamingId=a,De())}function yo(a,s){let o=!1;const i=async n=>{if(o)return;o=!0;const r=a.value;if(O.renamingId=null,!n){De();return}const l=String(r??"").trim().slice(0,120),u=O.historyItems.find(p=>p.id===s),m=u?(u.title||"").trim():"";if(l===m){De();return}u&&(u.title=l),De();try{await I(`/conversations/${s}`,{method:"PATCH",body:JSON.stringify({title:l})}),await Ue()}catch(p){Q(p.message||e("chat.saveFail")),await Ue()}};a.addEventListener("keydown",n=>{n.stopPropagation(),n.key==="Enter"?(n.preventDefault(),i(!0)):n.key==="Escape"&&(n.preventDefault(),i(!1))}),a.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation()}),a.addEventListener("mousedown",n=>n.stopPropagation()),a.addEventListener("dblclick",n=>{n.preventDefault(),n.stopPropagation()}),a.addEventListener("blur",()=>{setTimeout(()=>i(!0),0)})}async function Ha(a){(!a||ve)&&ve&&ve.abort();try{Q("");const s=await I(`/conversations/${a}`),o=s.data||s;O.conversationId=o.id,Ne=0,Re.clear(),ae=(o.messages||[]).filter(u=>!u.compressed).map(u=>({role:u.role,content:u.content||"",reasoning:u.reasoning||void 0,docs:u.docs,error:u.error})),ie=[],H.systemPrompt=o.systemPrompt||"",co(o),o.model&&(H.model=o.model),o.apiKeyId&&(H.keyId=o.apiKeyId);const i=document.getElementById("chat-system");i&&(i.value=H.systemPrompt);const n=document.getElementById("chat-system-wrap");n&&(n.hidden=!H.systemPrompt.trim()&&!H.systemOpen);const r=document.getElementById("chat-model");r&&o.model&&(r.value=o.model);const l=document.getElementById("chat-key-select");l&&o.apiKeyId&&[...l.options].some(m=>m.value===o.apiKeyId)&&(l.value=o.apiKeyId,H.keyId=o.apiKeyId),He(),Qe(),De(),je(),tt(),ca()}catch(s){Q(s.message||e("chat.loadFail"))}}function Pt(){const a=Pa();return!a||String(a).startsWith("admin-session:")||!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(a)?null:a}async function kt(){const a=lo();if(!a.length&&!_.summary)return;if(O.saving){O.saveQueued=!0;return}O.saving=!0,O.saveQueued=!1,me();const s={messages:a,model:H.model||null,systemPrompt:H.systemPrompt||"",apiKeyId:Pt(),...ro()};try{if(O.conversationId)await I(`/conversations/${O.conversationId}`,{method:"PATCH",body:JSON.stringify(s)});else{if(!a.length)return;const o=await I("/conversations",{method:"POST",body:JSON.stringify({...s,title:""})}),i=o.data||o;O.conversationId=i.id}await Ue()}catch(o){console.warn(o)}finally{O.saving=!1,O.saveQueued&&(O.saveQueued=!1,kt().catch(()=>{}))}}async function bo(a){if(await Z({title:e("chat.deleteConversation"),message:e("chat.deleteConfirm"),variant:"danger",confirmText:e("chat.deleteConversation")}))try{await I(`/conversations/${a}`,{method:"DELETE"}),O.conversationId===a&&(O.conversationId=null,ae=[],ie=[],cs(),He(),Qe(),je(),tt()),O.historyItems.length<=1&&O.historyPage>0&&(O.historyPage-=1),await Ue()}catch(s){Q(s.message||e("common.requestFailed"))}}function ho(a=!0){ve&&ve.abort(),ae=[],ie=[],O.conversationId=null,cs(),a||(H.systemPrompt="",H.systemOpen=!1),He(),Qe(),De(),je(),tt()}function Et(){return d.key}function Pa(){const s=document.getElementById("chat-key-select")?.value||H.keyId||"";return s&&s!=="session"?s:d.me?.id||""}function me(){const a=document.getElementById("chat-key-select"),s=document.getElementById("chat-model"),o=document.getElementById("chat-reasoning"),i=document.getElementById("chat-system");a&&(H.keyId=a.value==="session"?"":a.value),s&&(H.model=s.value),o&&(H.reasoning=o.checked);const n=document.getElementById("chat-effort");n&&(H.effort=n.value||"");const r=document.getElementById("chat-resume");r&&(H.resumeId=r.value.trim());const l=document.getElementById("chat-fork");l&&(H.forkSession=l.checked);const u=document.getElementById("chat-memory");u&&(H.memory=u.checked);const m=document.getElementById("chat-no-plan");m&&(H.noPlan=m.checked);const p=document.getElementById("chat-perm");p&&(H.permissionMode=p.value||""),i&&(H.systemPrompt=i.value),Ft()}function Ft(){const a=document.querySelector(".chat-shell"),s=document.getElementById("chat-settings-toggle"),o=document.getElementById("chat-settings-summary"),i=document.getElementById("chat-settings-toggle-label");if(a&&a.classList.toggle("is-settings-open",!!H.settingsOpen),s&&s.setAttribute("aria-expanded",H.settingsOpen?"true":"false"),i&&(i.textContent=H.settingsOpen?e("chat.settingsHide"):e("chat.settings")),o){const n=H.model||"—",r=_.mode==="summary"?"chat.ctxModeSummary":_.mode==="recent"?"chat.ctxModeRecent":"chat.ctxModeFull";o.textContent=`${n} · ${e(r)}`}}function Qe(){const a=document.getElementById("chat-pending");if(a){if(!ie.length){a.innerHTML="",a.hidden=!0;return}a.hidden=!1,a.innerHTML=ie.map((s,o)=>`
      ...
      </div>`).join(""),a.querySelectorAll("[data-rm-doc]").forEach(s=>{s.onclick=()=>{const o=Number(s.getAttribute("data-rm-doc"));ie.splice(o,1),Qe()}})}}function vo(a,s){return`${a}:${(s||"").length}:${(s||"").slice(0,40)}`}function He(){const a=document.getElementById("chat-messages");if(!a)return;const s=a.scrollHeight-a.scrollTop-a.clientHeight<120,o=ae.some(c=>c.streaming),i=document.getElementById("chat-stream-status");if(i&&(i.hidden=!o,i.textContent=o?e("chat.streaming"):""),!ae.length){a.innerHTML=`
      <div class="chat-empty">
        <strong>${t(e("chat.emptyTitle"))}</strong>
        <p>${t(e("chat.emptyHint"))}</p>
      </div>`,lt();return}const n=ae.length,r=Math.max(0,n-rs);Ne>r&&(Ne=r);const l=Ne,u=ae.slice(l),m=l,p=m>0?`<div class="chat-load-older">
          ...
          </details>`:"";let R=c.content||"";!R&&c.streaming&&(R=c.reasoning?"":"…");const q=c.error?" error":"",U=c.streaming?" is-streaming":"",M=g==="assistant"&&!c.streaming&&!!R;let P;if(M){const x=vo(h,R);if(Re.has(x))P=Re.get(x);else if(P=za(R),Re.set(x,P),Re.size>200){const N=Re.keys().next().value;Re.delete(N)}}else P=t(R);const K=!c.streaming&&R.length>oo,T=`${M?"chat-content md":"chat-content"}${K?" is-collapsible":""}`,f=K?`<button type="button" class="btn ghost sm chat-expand-btn" data-expand="${h}">${t(e("chat.showMore"))}</button>`:"",$=$o(c),G=$?`<div class="muted chat-spend">${t($)}</div>`:"",y=Array.isArray(c.tools)&&c.tools.length?`<div class="chat-tools">${c.tools.map(x=>`<span class="chat-tool-chip" title="${t(x.arguments||"")}">${t(x.name||"tool")}</span>`).join("")}</div>`:"",k=R?`<button type="button" class="chat-copy-btn" data-copy-msg="${h}" title="${t(e("chat.copy"))}">${t(e("chat.copy"))}</button>`:"";return`<div class="chat-bubble ${g}${q}${U}" data-msg-idx="${h}">
        ...
        <div class="chat-shell${H.settingsOpen?" is-settings-open":""}">
          ...
          <div class="chat-toolbar" id="chat-toolbar">
            <label class="chat-field-full">${t(e("chat.keySelect"))}
              <select id="chat-key-select">${Po()}</select>
            </label>
            <label>${t(e("chats.model"))}
          ...
          <div class="chat-composer" id="chat-composer">
            ...
            <div class="chat-composer-actions">
              <div class="chat-composer-left">
                <input type="file" id="chat-file" class="chat-file-input" multiple accept="${t(ws)}" />
...
✗ ${L.message||L}`,Q(L.message||String(L)))}finally{c.streaming=!1,ve=null,He(),lt(),g&&(g.disabled=!1),S&&(S.disabled=!1),C&&(C.disabled=!1),v&&(v.disabled=!0),kt().catch(()=>{})}}const ea="email@ysk.hk",qo="https://github.com/sponsors/yanshekki",To="https://linktr.ee/yanshekki",Ao="https://ysk.hk/",Bo="https://github.com/yanshekki/ysk-omni#readme";async function Co(){const s=[[e("support.netEvm"),"yanshekki.eth"],[e("support.netNear"),"yanshekki.near"],[e("support.netAda"),"$yanshekki"]].map(([o,i])=>`
      <tr>
        <td>${t(o)}</td>
    ...
    <div class="support-stack">
      ...
      <section class="panel support-panel" id="support-sponsor">
        ...
        <div class="panel-pad">
          ...
          <div class="support-actions">
            <a class="btn" href="${qo}" target="_blank" rel="noopener noreferrer">${t(e("support.githubSponsors"))}</a>
      ...
      <section class="panel support-panel" id="support-ysk">
        ...
        <div class="panel-pad">
          ...
          <a class="btn secondary sm" href="${Ao}" target="_blank" rel="noopener noreferrer">${t(e("support.site"))}</a>
        </div>
      </section>
  ...
  `),de(),document.querySelectorAll("[data-jump]").forEach(o=>{o.addEventListener("click",()=>{const i=o.getAttribute("data-jump");i&&document.getElementById(i)?.scrollIntoView({behavior:"smooth",block:"start"})})}),document.querySelectorAll("[data-copy]").forEach(o=>{o.addEventListener("click",async()=>{const i=o.getAttribute("data-copy")||"",n=await Ut(i);o.textContent=e(n?"chat.copied":"support.copy"),setTimeout(()=>{o.textContent=e("support.copy")},1400)})})}const Lo=[{id:"Qwen/Qwen2.5-0.5B-Instruct-GGUF",modality:"text",runtime:"llamacpp",quants:["Q4_K_M","Q5_K_M","Q8_0"],vramMb:512},{id:"Qwen/Qwen2.5-7B-Instruct",modality:"text",runtime:"vllm",quants:[],vramMb:16e3},{id:"Tongyi-MAI/Z-Image-Turbo",label:"Z-Image-Turbo",modality:"image",runtime:"diffusion",quants:[],vramMb:8e3},{id:"black-forest-labs/FLUX.2-klein-4B",label:"FLUX.2 Klein 4B",modality:"image",runtime:"diffusion",quants:[],vramMb:8e3},{id:"Qwen/Qwen3-TTS",modality:"tts",runtime:"diffusion",quants:[],vramMb:4e3},{id:"Systran/faster-whisper-small",modality:"stt",runtime:"whisper",quants:[],vramMb:1e3},{id:"Lightricks/LTX-2.5",label:"LTX-2.5",modality:"video",runtime:"diffusion",quants:[],vramMb:12e3},{id:"Wan-AI/Wan2.2",label:"Wan 2.2",modality:"video",runtime:"diffusion",quants:[],vramMb:2e4}];function ta(a){return a.label||String(a.id||"").split("/").pop()||a.id||""}function Oa(a,s){const o=String(a.id||"");return(s||[]).filter(i=>i.id===o||i.repoId===o||String(i.id||"").startsWith(`${o}:`))}function Tt(a){const s=`catalog.mod.${a}`;return Me(s)?e(s):a||"—"}function Ho(a){const s=a||[];return s.includes("Q4_K_M")?"Q4_K_M":s[0]||""}function Do(a){const s=Number(a)||0;return s>=1e6?`${(s/1e6).toFixed(1)}M`:s>=1e3?`${(s/1e3).toFixed(1)}k`:String(s)}function Nt(a){const s=Number(a)||0;return s<=0?"—":s>=1024?`${(s/1024).toFixed(1)} GB`:`${Math.round(s).toLocaleString()} MB`}function _a(a,s){return`
    ...
    </div>`}function Ra(a){const s=a?.status,o=a?.entry?.path;if(s==="done"&&o)return;const i=String(a?.reason||"");throw i.includes("no GGUF")?new Error(e("catalog.pullNoGguf")):new Error(i||e("catalog.pullFail"))}function Fa(a,s,o){if(!a)return;a.hidden=!1;const i=a.querySelector(".catalog-pull-bar"),n=a.querySelector(".catalog-pull-bar > span"),r=a.querySelector(".catalog-pull-meta"),l=Number(s)||0,u=Number(o)||0;if(u>0){const m=Math.min(100,Math.max(0,Math.round(l/u*100)));a.classList.remove("is-indeterminate"),n&&(n.style.width=`${m}%`),i&&i.setAttribute("aria-valuenow",String(m)),r&&(r.textContent=`${m}% · ${Nt(l/(1024*1024))} / ${Nt(u/(1024*1024))}`)}else a.classList.add("is-indeterminate"),n&&(n.style.width="40%"),r&&(r.textContent=e("catalog.pulling"))}async function mt({append:a=!1}={}){if(!d.catalogHubBusy){d.catalogHubBusy=!0;try{const s=new URLSearchParams;d.catalogHubQ&&s.set("q",d.catalogHubQ),d.catalogModality&&s.set("modality",d.catalogModality),a&&d.catalogHubNext&&s.set("cursor",d.catalogHubNext);const o=await I(`/catalog/hub?${s}`),i=o.hits||[];d.catalogHubHits=a?[...d.catalogHubHits||[],...i]:i,d.catalogHubNext=o.nextCursor||""}catch(s){a||(d.catalogHubHits=[]),b(s)}finally{d.catalogHubBusy=!1,await be()}}}async function be(){let a={};try{a=await I("/catalog")}catch(y){b(y)}const s=a.packs&&a.packs.length?a.packs:Lo,o=a.local||[],i=a.loaded||[];a.popularSyncedAt&&(d.catalogPopularSyncedAt=a.popularSyncedAt),d.catalogHubHits==null&&Array.isArray(a.popular)&&a.popular.length&&(d.catalogHubHits=a.popular);const n=new Set(i.map(y=>y.id)),r=a.usedMb??0,l=a.budgetMb??0,u=l?Math.min(100,Math.round(r/l*100)):0,m=d.catalogTab==="hub"||d.catalogTab==="local"?d.catalogTab:"local";d.catalogTab=m;const p=d.catalogModality||"",c=["text","image","video","tts","stt"],w=p?s.filter(y=>y.modality===p):s,h=d.catalogPulling||"",g=w.map(y=>{const k=Oa(y,o),x=y.quants||[],N=Ho(x),ee=x.length?`<select class="catalog-quant-select" data-quant-for="${t(y.id)}">${x.map(V=>`<option value="${t(V)}" ${V===N?"selected":""}>${t(V)}</option>`).join("")}</select>`:`<span class="muted">${t(e("catalog.noQuant"))}</span>`,z=h===y.id,j=z?e("catalog.pulling"):k.length?e("catalog.pullAgain"):e("catalog.pull");return`
      <tr>
        <td>
...
`).map(j=>j.trim()).filter(Boolean),z=ee.length?JSON.parse(ee[ee.length-1]):{};if(!x.ok)throw new Error(z.error?.message||z.reason||x.statusText);Ra(z),d.catalogPulling="",d.catalogTab="local",await be()}catch(x){d.catalogPulling="",k&&(k.removeAttribute("disabled"),k.textContent=e("catalog.pullSpecBtn")),b(x)}})}async function Qt(){const a=document.getElementById("app");try{if(!d.key){await Za();return}d.me||await Ya(),d.page==="dashboard"?await sa():d.page==="chat"?await Io():d.page==="chats"?await pt():d.page==="keys"?await Fe():d.page==="documents"?await st():d.page==="media"?await xe():d.page==="catalog"?await be():d.page==="audit"?await gt():d.page==="settings"?await is():d.page==="apiFeatures"?await Bt():d.page==="usage"?await pe():d.page==="ddos"?await ne():d.page==="queue"?await ue():d.page==="pm2"?await Le():d.page==="system"?await ot():d.page==="support"?await Co():await sa()}catch(s){a.innerHTML=le(`<div class="error-box">${t(s.message)}</div>`),de()}}let Je=null;const W={tab:"overview",status:"",sortBy:"queuedAt",sortDir:"desc",limit:20,offset:0};function Kt(a){return!a||a<0?"—":a<1e3?`${a}ms`:a<6e4?`${Math.round(a/1e3)}s`:a<36e5?`${Math.round(a/6e4)}m`:`${(a/36e5).toFixed(1)}h`}const Oo=["enabled","globalConcurrency","perKeyConcurrency","maxQueueDepth","maxQueueDepthPerKey","fairness","defaultPriority","playgroundPriority","leaseMs","maxWaitMs"];function fs(){return{relaxed:{enabled:!0,globalConcurrency:6,perKeyConcurrency:2,maxQueueDepth:200,maxQueueDepthPerKey:40,fairness:"weighted_round_robin",defaultPriority:100,playgroundPriority:40,leaseMs:6e4,maxWaitMs:9e5},balanced:{enabled:!0,globalConcurrency:4,perKeyConcurrency:1,maxQueueDepth:100,maxQueueDepthPerKey:20,fairness:"weighted_round_robin",defaultPriority:100,playgroundPriority:50,leaseMs:45e3,maxWaitMs:6e5},strict:{enabled:!0,globalConcurrency:2,perKeyConcurrency:1,maxQueueDepth:40,maxQueueDepthPerKey:8,fairness:"fifo_global",defaultPriority:100,playgroundPriority:80,leaseMs:3e4,maxWaitMs:3e5}}}function Na(a){if(!a)return{};const s={};for(const o of Oo){const i=a[o];typeof i=="boolean"?s[o]=i:typeof i=="number"&&Number.isFinite(i)?s[o]=Math.round(i):typeof i=="string"?s[o]=i:i==null?s[o]=null:s[o]=i}return s}function ys(a,s){return JSON.stringify(Na(a))===JSON.stringify(Na(s))}function ua(a){if(!a)return"custom";const s=fs();for(const o of["relaxed","balanced","strict"])if(ys(a,s[o]))return o;return"custom"}function _o(a){return e(a==="relaxed"?"queue.presetRelaxed":a==="balanced"?"queue.presetBalanced":a==="strict"?"queue.presetStrict":"queue.presetCustom")}function bs(a,{unsaved:s=!1}={}){const o=_o(a),i=a==="relaxed"?"relaxed":a==="balanced"?"balanced":a==="strict"?"strict":"custom",n=s?A("queue.presetFormLabel",{name:o}):A("queue.presetActiveLabel",{name:o});return`<span class="ddos-preset-badge is-${i}" id="queue-preset-badge" title="${t(n)}">${t(n)}</span>`}function hs(){return{enabled:document.getElementById("q-master-enabled")?Xe("q-master-enabled"):!0,globalConcurrency:Math.max(1,Math.min(64,Math.floor(J("qp-gconc",4)))),perKeyConcurrency:Math.max(1,Math.min(16,Math.floor(J("qp-kconc",1)))),maxQueueDepth:Math.max(1,Math.floor(J("qp-depth",100))),maxQueueDepthPerKey:Math.max(1,Math.floor(J("qp-depthk",20))),fairness:document.getElementById("qp-fair")?.value==="fifo_global"?"fifo_global":"weighted_round_robin",defaultPriority:Math.max(0,Math.min(1e3,Math.floor(J("qp-pri",100)))),playgroundPriority:Math.max(0,Math.min(1e3,Math.floor(J("qp-ppri",50)))),leaseMs:Math.max(5e3,Math.floor(J("qp-lease",45e3))),maxWaitMs:Math.max(5e3,Math.floor(J("qp-wait",6e5)))}}function jt(a){Ye("q-master-enabled",a,e("queue.masterOn"),e("queue.masterOff")),et("queue-root",!a),Ze("queue-disabled-banner",!a);const s=document.getElementById("qk-pill-enabled");s&&(s.innerHTML=Ke(a,e("dash.on"),e("dash.off")))}function Ro(a){if(!a)return;const s=(i,n)=>{const r=document.getElementById(i);r&&(r.value=String(n))};jt(a.enabled!==!1),s("qp-gconc",a.globalConcurrency),s("qp-kconc",a.perKeyConcurrency),s("qp-depth",a.maxQueueDepth),s("qp-depthk",a.maxQueueDepthPerKey);const o=document.getElementById("qp-fair");o&&(o.value=a.fairness||"weighted_round_robin"),s("qp-pri",a.defaultPriority),s("qp-ppri",a.playgroundPriority),s("qp-lease",a.leaseMs),s("qp-wait",a.maxWaitMs),yt()}function yt(){if(!document.getElementById("queue-policy-panel"))return;let a;try{a=hs()}catch{return}const s=ua(a),o=ua(d._queuePolicyCache||a),i=!ys(a,d._queuePolicyCache||a);document.querySelectorAll("[data-queue-preset]").forEach(l=>{const u=l.dataset.queuePreset;if(u==="custom"){const w=s==="custom";l.classList.toggle("is-active",w),l.setAttribute("aria-pressed",w?"true":"false"),l.disabled=!w;return}const m=u===s,p=u===o;l.classList.toggle("is-active",m),l.classList.toggle("is-saved",p&&!m),l.setAttribute("aria-pressed",m?"true":"false");const c=e(u==="relaxed"?"queue.presetRelaxed":u==="balanced"?"queue.presetBalanced":"queue.presetStrict");m&&p?l.innerHTML=`${t(c)} <span class="preset-tag">${t(e("queue.presetTagActive"))}</span>`:m&&i?l.innerHTML=`${t(c)} <span class="preset-tag preset-tag--draft">${t(e("queue.presetTagDraft"))}</span>`:p?l.innerHTML=`${t(c)} <span class="preset-tag preset-tag--saved">${t(e("queue.presetTagSaved"))}</span>`:l.textContent=c});const n=document.getElementById("queue-preset-badge");n&&(n.outerHTML=bs(s,{unsaved:i&&s!==o}));const r=document.getElementById("queue-preset-hint");if(r){const l={relaxed:e("queue.presetRelaxedHint"),balanced:e("queue.presetBalancedHint"),strict:e("queue.presetStrictHint"),custom:e("queue.presetCustomHint")};r.textContent=l[s]||l.custom}}function Fo(){document.querySelectorAll("[data-queue-preset]").forEach(a=>{a.dataset.queuePreset!=="custom"&&(a.onclick=()=>{const s=a.dataset.queuePreset,o=fs()[s];o&&Ro(o)})}),["qp-gconc","qp-kconc","qp-depth","qp-depthk","qp-fair","qp-pri","qp-ppri","qp-lease","qp-wait"].forEach(a=>{const s=document.getElementById(a);s&&(s.addEventListener("change",()=>yt()),s.addEventListener("input",()=>yt()))}),yt()}function Ka(){return document.querySelector(".main")}function vs(a){return a.map(s=>{const o=s.status==="queued"||s.status==="leased"||s.status==="running",i=s.status==="failed"||s.status==="dead"||s.status==="cancelled",n=s.startedAt||s.finishedAt?null:s.queuedAt?Date.now()-new Date(s.queuedAt).getTime():null;return`
    <tr data-q-row="${t(s.id)}">
      ...
      <td>
        ...
        </div>`,document.getElementById("q-filter-dead")?.addEventListener("click",()=>{W.status="dead",W.offset=0,W.tab="jobs",ue().catch(b)})):C.innerHTML="");const L=(T,f)=>{const $=document.getElementById(T);$&&($.textContent=String(f??0))};L("q-tab-count-jobs",i),L("q-tab-count-dead",r);const R=document.getElementById("qk-jobs-meta");R&&(R.textContent=A("queue.jobsMeta",{n:i}));const q=document.querySelector("#queue-jobs-table tbody");if(q){const T=o.map($=>`${$.id}|${$.status}|${$.priority}|${$.attempt}|${$.cancelRequested?1:0}|${$.errorMessage||""}|${$.startedAt||""}|${$.finishedAt||""}`).join(";"),f=vs(o)||`<tr class="empty-row"><td colspan="9">
        <div class="data-empty">
          <div class="data-empty-icon">∅</div>
          <strong>${t(e("queue.empty"))}</strong>
        </div>
      </td></tr>`;if(q.dataset.qsig!==T){const $=document.querySelector("#queue-jobs-table .table-wrap"),G=$?.scrollLeft||0;q.dataset.qsig=T,q.innerHTML=f,$s(),$a(document.querySelector("#queue-jobs-table")||document),$&&($.scrollLeft=G)}else o.forEach($=>{if($.status!=="queued"||!$.queuedAt)return;const G=Date.now()-new Date($.queuedAt).getTime(),y=String($.id||"");let k=null;q.querySelectorAll("[data-q-row]").forEach(N=>{N.getAttribute("data-q-row")===y&&(k=N)});const x=k?.querySelector("[data-q-wait]");x&&(x.textContent=`${e("queue.wait")}: ${Kt(G)}`)})}if(document.querySelector("#queue-pager .data-pager-meta span")){const T=Math.max(1,Math.ceil((i||0)/W.limit)||1),f=Math.floor(W.offset/W.limit)+1,$=document.querySelectorAll("#queue-pager .data-pager-meta > span");$[0]&&($[0].textContent=A("common.pagerTotal",{n:i||0})),$[1]&&($[1].textContent=A("common.pagerPage",{n:f,total:T}));const G=document.getElementById("queue-prev"),y=document.getElementById("queue-next");G&&(G.disabled=W.offset<=0),y&&(y.disabled=W.offset+W.limit>=i)}const M=document.getElementById("q-pause");M&&(M.textContent=s.paused?e("queue.resume"):e("queue.pause"));const P=document.getElementById("q-drain");P&&(P.textContent=s.drainMode?e("queue.undrain"):e("queue.drainBtn"));const K=document.getElementById("q-master-enabled");K&&document.activeElement!==K&&jt(s.enabled!==!1)}function ja(){Je||(Je=setInterval(()=>{if(d.page!=="queue"){clearInterval(Je),Je=null;return}const a=document.activeElement;a&&a.closest&&a.closest("#queue-policy-panel")&&(a.tagName==="INPUT"||a.tagName==="SELECT"||a.tagName==="TEXTAREA")||ue({soft:!0}).catch(()=>{})},4e3))}async function ue(a={}){const s=!!a.soft&&document.getElementById("queue-root");!s&&Je&&(clearInterval(Je),Je=null);const o=Ka(),i=!s&&o?o.scrollTop:0,n=W;n.sortBy||(n.sortBy="queuedAt"),n.sortDir||(n.sortDir="desc");const r=new URLSearchParams;r.set("limit",String(n.limit)),r.set("offset",String(n.offset)),n.status&&r.set("status",n.status),Ee(r,n);const[l,u,m]=await Promise.all([I("/queue/stats"),I(`/queue/jobs?${r}`),I("/queue/policy")]);if(d.page!=="queue")return;const p=l.data||{},c=m.data||p.policy||{},w=u.data||[],h=u.total??w.length,g=p.byStatus||{},v=p.dead??g.dead??0,S=p.leased??g.leased??0,C=p.running??g.running??0,L=p.queued??g.queued??0,R=p.depth??L+S+C,q=ks(c);if(d._queuePolicyCache={...c},s){No({s:p,pol:c,jobs:w,total:h,by:g}),ja();return}W.tab||(W.tab="overview");const U=W.tab==="jobs"||W.tab==="policy"?W.tab:"overview";W.tab=U;const M=vs(w),P=Oe({title:e("queue.filterTitle"),hint:e("queue.filterHint"),meta:A("queue.jobsMeta",{n:h}),gridHtml:`
