...
… (${e("media.previewTruncated")})`:g;v.innerHTML=`<pre class="media-lb-text">${t(C)}</pre>`}).catch(()=>{const g=document.querySelector("#modal-back .media-lb-stage");g&&(g.innerHTML=`<div class="error-box">${t(e("media.previewFail"))}</div>`)})}function Wa(){return`
  ...
  ${d.modal||""}
  `}function ce(a,s){return`<button type="button" class="nav-btn ${d.page===a?"active":""}" data-nav="${a}">${t(s)}</button>`}function de(){It(),document.querySelectorAll("[data-nav]").forEach(s=>{s.onclick=()=>{It(),ga(s.dataset.nav)}});const a=()=>Ht(!0);document.getElementById("btn-logout")?.addEventListener("click",a),document.getElementById("btn-logout-mobile")?.addEventListener("click",a),document.getElementById("nav-open")?.addEventListener("click",Ts),document.getElementById("nav-backdrop")?.addEventListener("click",It),document.addEventListener("keydown",s=>{s.key==="Escape"&&It()},{once:!0}),document.querySelectorAll("[data-lang]").forEach(s=>{s.onclick=()=>{Na(s.dataset.lang),Gt().catch(y)}}),va(document)}function As(a){if(!a)return"";const s=(a.getAttribute("data-label")||"").trim();if(s)return s;const o=a.querySelector(".th-sort-btn");return(o?[...o.childNodes].filter(n=>n.nodeType===Node.TEXT_NODE).map(n=>n.textContent||"").join(""):a.textContent||"").replace(/[▲▼]/g,"").replace(/\s+/g," ").trim()}function va(a){(a||document).querySelectorAll("table.data-table").forEach(o=>{const i=[...o.querySelectorAll("thead th")].map(As);o.querySelectorAll("tbody tr").forEach(n=>{n.classList.contains("empty-row")||[...n.children].forEach((r,l)=>{const u=[...r.children],m=r.classList.contains("row-actions")||!!r.querySelector(":scope > .row-actions")||u.length>0&&u.every(p=>p.matches("button, .btn, .row-actions"));if(r.classList.toggle("is-actions",m),r.classList.toggle("is-primary",l===0&&!m),m){r.removeAttribute("data-label");return}i[l]&&r.setAttribute("data-label",i[l])})})})}function y(a){console.error(a),Q(a.message||String(a))}async function za(){if(!d.key)return!1;const a=await I("/me");return d.me=a.data,!0}async function $t(a=!1){try{const s=await I(`/models${a?"?refresh=1":""}`);return d.models=s.data?.models||[],s.data}catch{return d.models=[],{models:[],source:"fallback",defaultModel:""}}}async function wt(){try{const a=await I("/keys?all=1");d.keys=a.data||[]}catch{d.keys=[]}}function $e(a){const s=(Array.isArray(a)?a:[a]).filter(Boolean);return s.length?`<div class="page-meta" role="status">${s.map(i=>`<span>${typeof i=="string"?t(i):i}</span>`).join('<span class="page-meta-sep" aria-hidden="true">·</span>')}</div>`:""}function Oe({title:a,hint:s,meta:o,searchHtml:i,gridHtml:n}){return`
    ...
    </div>`;document.getElementById("app").insertAdjacentHTML("beforeend",r);const l=()=>be();document.getElementById("modal-close").onclick=l,document.getElementById("modal-back").onclick=m=>{m.target.id==="modal-back"&&l()};const u=m=>{m.key==="Escape"&&(l(),document.removeEventListener("keydown",u))};document.addEventListener("keydown",u)}async function Va(){const a="ysk-omni admin otp";document.getElementById("app").innerHTML=`
...
`):s,hasRoles:!0,blocks:r}}async function pt(){await Promise.all([$t(),wt()]);const a=d.chatFilter,s=new URLSearchParams;if(s.set("limit",String(a.limit)),s.set("offset",String(a.offset)),a.status&&s.set("status",a.status),a.model&&s.set("model",a.model),a.apiKeyId&&s.set("apiKeyId",a.apiKeyId),a.q&&s.set("q",a.q),a.from&&s.set("from",new Date(a.from).toISOString()),a.to){const c=new Date(a.to);c.setHours(23,59,59,999),s.set("to",c.toISOString())}a.policyMode&&s.set("policyMode",a.policyMode),a.hasDocuments!==""&&s.set("hasDocuments",a.hasDocuments),Pe(s,a);const o=await I(`/chats?${s}`),i=o.total||0,n=[`<option value="">${t(e("chats.allModels"))}</option>`,...d.models.map(c=>`<option value="${t(c)}" ${a.model===c?"selected":""}>${t(c)}</option>`)].join(""),r=[`<option value="">${t(e("chats.allKeys"))}</option>`,...d.keys.map(c=>`<option value="${c.id}" ${a.apiKeyId===c.id?"selected":""}>${t(c.name)} (${t(c.keyPrefix)})</option>`)].join(""),l=(o.items||[]).map(c=>{const w=Ja(c.promptPreview||""),h=!!w.system,g=h?w.body.slice(0,160):c.promptPreview||"";return`
    ...
    ${u}
    ${m}
  `),de(),dt("chats",d.chatFilter,()=>pt().catch(y)),Ge(d.chatFilter,()=>pt().catch(y));const p=()=>{d.chatFilter.q=document.getElementById("f-q").value.trim(),d.chatFilter.status=document.getElementById("f-status").value,d.chatFilter.model=document.getElementById("f-model").value,d.chatFilter.apiKeyId=document.getElementById("f-key").value,d.chatFilter.policyMode=document.getElementById("f-mode").value,d.chatFilter.from=document.getElementById("f-from").value,d.chatFilter.to=document.getElementById("f-to").value,d.chatFilter.hasDocuments=document.getElementById("f-docs").checked?"true":"",d.chatFilter.offset=0,pt().catch(y)};document.querySelector("[data-filter-apply]").onclick=p,document.getElementById("f-q").onkeydown=c=>{c.key==="Enter"&&p()},document.querySelector("[data-filter-reset]").onclick=()=>{d.chatFilter={q:"",status:"",model:"",apiKeyId:"",from:"",to:"",policyMode:"",hasDocuments:"",sortBy:"createdAt",sortDir:"desc",limit:50,offset:0},pt().catch(y)},document.querySelectorAll("[data-chat]").forEach(c=>{c.onclick=()=>Xa(c.dataset.chat)})}async function Xa(a){const{data:s}=await I(`/chats/${a}`),o=s.response||{},i=s.documents||[];let n=`<p class="muted">${t(e("chats.noAttach"))}</p>`;if(i.length){const m=[];for(const p of i){let c="";if(St(p.mimeType))try{const w=await I(`/documents/${p.id}`),h=await es(w.data||{id:p.id,isImage:!0,mimeType:p.mimeType});h?.src&&(c=`<img class="preview" src="${h.src}" alt="${t(p.originalName)}" />`)}catch{c=`<span class="muted">${t(e("chats.previewFailed"))}</span>`}m.push(`
        ...
        </div>`)}n=`<div class="attach-list">${m.join("")}</div>`}const r=Ja(s.prompt||""),l=r.system?`<div class="block block-system">
    ...
    ${u}
    ${m}
  `),de(),dt("keys",d.keyFilter,()=>Fe().catch(y)),Ge(d.keyFilter,()=>Fe().catch(y)),document.querySelector("[data-filter-apply]").onclick=()=>{d.keyFilter.q=document.getElementById("kf-q").value.trim(),d.keyFilter.role=document.getElementById("kf-role").value,d.keyFilter.mode=document.getElementById("kf-mode").value,d.keyFilter.isActive=document.getElementById("kf-active").value,d.keyFilter.offset=0,Fe().catch(y)},document.querySelector("[data-filter-reset]").onclick=()=>{d.keyFilter={q:"",role:"",mode:"",isActive:"",sortBy:"createdAt",sortDir:"desc",limit:20,offset:0},Fe().catch(y)},document.getElementById("btn-new-key").onclick=()=>Ma(),document.querySelectorAll("[data-edit]").forEach(p=>{const c=n.find(w=>w.id===p.dataset.edit);p.onclick=()=>Ma(c)}),document.querySelectorAll("[data-revoke]").forEach(p=>{p.onclick=async()=>{await ee({message:e("keys.confirmRevoke"),variant:"danger",confirmText:e("keys.revoke")})&&(await I(`/keys/${p.dataset.revoke}`,{method:"DELETE"}),Fe().catch(y))}})}function Ma(a){const s=!!a,o=(a?.ipWhitelist||[]).join(`
...
${r.data?.key||JSON.stringify(r.data)}`);const u=document.getElementById("k-save");u&&(u.textContent=e("chats.close"),u.onclick=()=>{be(),Fe().catch(y)})}}catch(r){y(r)}}}function Ya(a){return e(a==="filesystem"?"docs.storageFs":"docs.storageDb")}async function Za(a,s){try{const o=await fetch(`${ht}/documents/${a}/download`,{headers:d.key?{Authorization:`Bearer ${d.key}`}:{}});if(!o.ok){const l=await o.text();let u=l;try{u=JSON.parse(l).error?.message||l}catch{}throw new Error(u||e("docs.downloadFail"))}const i=await o.blob(),n=URL.createObjectURL(i),r=document.createElement("a");r.href=n,r.download=s||"download",document.body.appendChild(r),r.click(),r.remove(),URL.revokeObjectURL(n)}catch(o){Q(o.message||e("docs.downloadFail"))}}async function st(){await wt();const a=d.docFilter,s=new URLSearchParams({limit:String(a.limit),offset:String(a.offset)});if(a.q&&s.set("q",a.q),a.apiKeyId&&s.set("apiKeyId",a.apiKeyId),a.storageType&&s.set("storageType",a.storageType),a.from&&s.set("from",new Date(a.from).toISOString()),a.to){const c=new Date(a.to);c.setHours(23,59,59,999),s.set("to",c.toISOString())}Pe(s,a);const o=await I(`/documents?${s}`),i=o.total??0,n=o.meta||{},r=A("docs.storageHint",{dir:n.storageDir||"—",dbMax:Ae(n.documentDbMaxBytes),upMax:Ae(n.uploadMaxBytes)}),l=[`<option value="">${t(e("common.all"))}</option>`,...d.keys.map(c=>`<option value="${c.id}" ${a.apiKeyId===c.id?"selected":""}>${t(c.name)}</option>`)].join(""),u=(o.data||[]).map(c=>`
  ...
  `),de(),Ge(d.usageFilter,()=>pe().catch(y)),document.getElementById("btn-usage-refresh").onclick=()=>pe().catch(y),document.querySelectorAll("[data-usage-tab]").forEach(M=>{M.onclick=()=>{const P=M.dataset.usageTab==="key"?"key":"model";d.usageFilter.tab!==P&&(d.usageFilter.tab=P,pe().catch(y))}}),document.getElementById("umodel-prev")?.addEventListener("click",()=>{d.usageFilter.modelPage=Math.max(0,a.modelPage-1),pe().catch(y)}),document.getElementById("umodel-next")?.addEventListener("click",()=>{(a.modelPage+1)*r<u&&(d.usageFilter.modelPage+=1,pe().catch(y))}),document.getElementById("umodel-limit")?.addEventListener("change",M=>{d.usageFilter.pageSize=Number(M.target.value)||10,d.usageFilter.modelPage=0,pe().catch(y)}),document.getElementById("ukey-prev")?.addEventListener("click",()=>{d.usageFilter.keyPage=Math.max(0,a.keyPage-1),pe().catch(y)}),document.getElementById("ukey-next")?.addEventListener("click",()=>{(a.keyPage+1)*r<w&&(d.usageFilter.keyPage+=1,pe().catch(y))}),document.getElementById("ukey-limit")?.addEventListener("change",M=>{d.usageFilter.pageSize=Number(M.target.value)||10,d.usageFilter.keyPage=0,pe().catch(y)}),document.querySelectorAll("#usage-tab-model [data-filter-apply]").forEach(M=>{M.onclick=()=>{d.usageFilter.modelQ=document.getElementById("uf-model")?.value?.trim()||"",d.usageFilter.modelPage=0,pe().catch(y)}}),document.querySelectorAll("#usage-tab-model [data-filter-reset]").forEach(M=>{M.onclick=()=>{d.usageFilter.modelQ="",d.usageFilter.modelPage=0,pe().catch(y)}}),document.querySelectorAll("#usage-tab-key [data-filter-apply]").forEach(M=>{M.onclick=()=>{d.usageFilter.keyQ=document.getElementById("uf-key")?.value?.trim()||"",d.usageFilter.keyActive=document.getElementById("uf-active")?.value||"",d.usageFilter.keyPage=0,pe().catch(y)}}),document.querySelectorAll("#usage-tab-key [data-filter-reset]").forEach(M=>{M.onclick=()=>{d.usageFilter.keyQ="",d.usageFilter.keyActive="",d.usageFilter.keyPage=0,pe().catch(y)}})}function qa(a){const s=a.versionStatus||(a.updateAvailable?"update_available":a.latest?"up_to_date":"unknown");return s==="update_available"?{badge:`<span class="badge warn" title="${t(e("system.statusHintUpdate"))}">${t(e("system.badgeUpdate"))}</span>`,hint:e("system.statusHintUpdate")}:s==="ahead"?{badge:`<span class="badge pending" title="${t(e("system.statusHintAhead"))}">${t(e("system.badgeAhead"))}</span>`,hint:e("system.statusHintAhead")}:s==="up_to_date"?{badge:`<span class="badge success" title="${t(e("system.statusHintOk"))}">${t(e("system.badgeOk"))}</span>`,hint:e("system.statusHintOk")}:{badge:`<span class="badge pending" title="${t(e("system.statusHintUnknown"))}">${t(e("system.badgeUnknown"))}</span>`,hint:e("system.statusHintUnknown")}}function Ks(a){return e(a==="git"?"system.channelGit":a==="npm-global"?"system.channelNpmGlobal":a==="npm-local"?"system.channelNpmLocal":"system.channelUnknown")}function js(a){return a==="required"?e("system.levelRequired"):a==="recommended"?e("system.levelRecommended"):a==="optional"?e("system.levelOptional"):a==="bundled"?e("system.levelBundled"):a||"—"}function Us(a){return a.installed?a.ok?`<span class="badge success">${t(e("system.softOk"))}</span>`:`<span class="badge warn">${t(e("system.softWarn"))}</span>`:a.level==="required"||a.level==="bundled"?`<span class="badge error">${t(e("system.softMissing"))}</span>`:`<span class="badge pending">${t(e("system.softMissing"))}</span>`}function Ta(a){return a==="up"?`<span class="badge success">${t(e("system.up"))}</span>`:`<span class="badge error">${t(e("system.down"))}</span>`}async function ot(){const{data:a}=await I("/system");if(d.page!=="system")return;const s=a.version||{},o=qa(s),i=a.software||{checks:[],allRequiredOk:!0},n=i.checks||[],r=d.systemTab==="package"||d.systemTab==="env"||d.systemTab==="sessions"?d.systemTab:"software";d.systemTab=r;let l={data:[],total:0};try{const P=new URLSearchParams({limit:r==="sessions"?"50":"1",offset:"0"});r==="sessions"&&d.grokSessionQ&&P.set("q",d.grokSessionQ);const K=await I(`/grok/sessions?${P}`);l={data:K.data||[],total:K.total||0}}catch(P){l={data:[],total:0,error:P.message||String(P)}}const u=n.map(P=>`
      ...
      </tr>`).join(""),m=i.allRequiredOk?`<span class="badge success">${t(e("system.allRequiredOk"))}</span>`:`<span class="badge error">${t(e("system.requiredMissing"))}</span>`,p=a.encryption&&a.encryption.ready,c=Ks(s.channel),w=s.installSource?`${c} · ${s.installSource}`:c,h=ve({headHtml:`
...
`))}</textarea>
    ...
    <div class="panel data-table-panel ddos-stack-panel">
      ...
      <table class="data-table">
        ...
        <tbody id="ddos-ban-body">${b||X}</tbody>
      </table>
      </div>
  ...
  `),de(),Ba(!0,K),Ge(d.ddosFilter,()=>ne().catch(y)),document.querySelectorAll("[data-ddos-tab]").forEach(Z=>{Z.addEventListener("click",()=>{const Ee=Z.getAttribute("data-ddos-tab")||"policy",Ie=Ee==="live"||Ee==="blacklist"||Ee==="events"||Ee==="policy"?Ee:"policy";d.ddosFilter.tab!==Ie&&(d.ddosFilter.tab=Ie,ne().catch(y))})}),document.getElementById("ddos-live-filter-apply")?.addEventListener("click",()=>{d.ddosFilter.liveQ=document.getElementById("ddos-live-q")?.value?.trim()||"",d.ddosFilter.livePage=0,ne().catch(y)}),document.getElementById("ddos-live-filter-reset")?.addEventListener("click",()=>{d.ddosFilter.liveQ="",d.ddosFilter.liveSortBy="startedAt",d.ddosFilter.liveSortDir="desc",d.ddosFilter.livePage=0,ne().catch(y)}),document.getElementById("ddos-ban-filter-apply")?.addEventListener("click",()=>{d.ddosFilter.banQ=document.getElementById("ddos-ban-q")?.value?.trim()||"",d.ddosFilter.banSource=document.getElementById("ddos-ban-source")?.value||"",d.ddosFilter.banPage=0,ne().catch(y)}),document.getElementById("ddos-ban-filter-reset")?.addEventListener("click",()=>{d.ddosFilter.banQ="",d.ddosFilter.banSource="",d.ddosFilter.banSortBy="createdAt",d.ddosFilter.banSortDir="desc",d.ddosFilter.banPage=0,ne().catch(y)}),document.getElementById("ddoslive-prev")?.addEventListener("click",()=>{d.ddosFilter.livePage=Math.max(0,n.livePage-1),ne().catch(y)}),document.getElementById("ddoslive-next")?.addEventListener("click",()=>{(n.livePage+1)*S<C.length&&(d.ddosFilter.livePage+=1,ne().catch(y))}),document.getElementById("ddosban-prev")?.addEventListener("click",()=>{d.ddosFilter.banPage=Math.max(0,n.banPage-1),ne().catch(y)}),document.getElementById("ddosban-next")?.addEventListener("click",()=>{(n.banPage+1)*S<R.length&&(d.ddosFilter.banPage+=1,ne().catch(y))});const ke=document.querySelector(".main");ke&&(ke.onscroll=()=>{d._ddosScrollPauseUntil=Date.now()+4e3})}!ze&&d.page==="ddos"&&(We=setInterval(()=>{d.page!=="ddos"||ze||d._ddosScrollPauseUntil&&Date.now()<d._ddosScrollPauseUntil||ne({soft:!0}).catch(()=>{})},2e3))}function Ba(a=!1,s=[]){const o=s.length?s:d._ddosPolicyCache?.whitelist||[],i=async n=>{if(!n)return;const r=o.some(l=>String(l)===n||String(l).startsWith(n));await ee({message:e(r?"ddos.banWhitelistWarn":"ddos.banConfirm"),variant:"danger",confirmText:e("ddos.ban")})&&(await I("/ddos/blacklist",{method:"POST",body:JSON.stringify({ip:n,reason:e("ddos.banReasonDefault"),ttlSeconds:null})}),ne({soft:!0}).catch(y))};if(document.querySelectorAll("[data-ban]").forEach(n=>{n.onclick=()=>i(n.dataset.ban)}),document.querySelectorAll("[data-unban]").forEach(n=>{n.onclick=async()=>{await ee({message:e("ddos.unbanConfirm"),variant:"danger",confirmText:e("ddos.unban")})&&(await I(`/ddos/blacklist/${encodeURIComponent(n.dataset.unban)}`,{method:"DELETE"}),ne({soft:!0}).catch(y))}}),a){document.getElementById("ban-add").onclick=async()=>{const r=document.getElementById("ban-ip").value.trim();if(!r||o.some(m=>String(m)===r)&&!await ee({message:e("ddos.banWhitelistWarn"),variant:"danger",confirmText:e("ddos.ban")}))return;const u=document.getElementById("ban-ttl").value;await I("/ddos/blacklist",{method:"POST",body:JSON.stringify({ip:r,reason:document.getElementById("ban-reason").value.trim()||void 0,ttlSeconds:u?Number(u):null})}),ne({soft:!0}).catch(y)},document.getElementById("ddos-refresh").onclick=()=>ne({soft:!1}).catch(y),document.getElementById("ddos-pause").onclick=()=>{ze=!ze;const r=document.getElementById("ddos-pause");r&&(r.textContent=e(ze?"ddos.resume":"ddos.pause")),ze||ne({soft:!0}).catch(y)},document.getElementById("ddos-master-autoban")?.addEventListener("click",async()=>{const r=!Xe("ddos-master-autoban");Ye("ddos-master-autoban",r,e("ddos.masterOn"),e("ddos.masterOff"));const l=document.getElementById("dp-autoBanEnabled");l&&(l.type==="checkbox"?l.checked=r:l.value=r?"1":"0"),ka(r),et("ddos-root",!r),Ze("ddos-disabled-banner",!r),_e();try{const u=na(),m=await I("/ddos/policy",{method:"PUT",body:JSON.stringify(u)});d._ddosPolicyCache=m.data,_e()}catch(u){Ye("ddos-master-autoban",!r,e("ddos.masterOn"),e("ddos.masterOff")),et("ddos-root",r),Ze("ddos-disabled-banner",r),y(u)}});const n=document.getElementById("ddos-policy-panel");n?.addEventListener("input",()=>_e()),n?.addEventListener("change",()=>_e()),document.querySelectorAll("[data-ddos-preset]").forEach(r=>{r.onclick=()=>{const l=r.dataset.ddosPreset;if(l==="custom")return;const u=d._ddosPresetsCache?.[l];u&&zt(u)}}),_e(),document.getElementById("dp-save")?.addEventListener("click",async()=>{try{const r=na(),l=await I("/ddos/policy",{method:"PUT",body:JSON.stringify(r)});d._ddosPolicyCache=l.data,zt(l.data),_e(),await ge({title:e("ddos.policyTitle"),message:e("ddos.policySaved")}),ne({soft:!0}).catch(y)}catch(r){y(r)}}),document.getElementById("dp-reset")?.addEventListener("click",async()=>{if(await ee({message:e("ddos.confirmReset"),variant:"danger",confirmText:e("ddos.resetPolicy")}))try{const r=await I("/ddos/policy/reset",{method:"POST"});d._ddosPolicyCache=r.data,zt(r.data),_e(),await ge({title:e("ddos.policyTitle"),message:e("ddos.policyReset")}),ne({soft:!0}).catch(y)}catch(r){y(r)}})}}function Mt(a){return e(a==="pm2"?"pm2.runnerPm2":a==="ysk-omni"?"pm2.runnerGctoac":a==="none"?"pm2.runnerNone":"pm2.runnerUnknown")}function la(a){if(!a)return"";const s=a.messageKey;if(s&&typeof s=="string"){if(s==="pm2.msgOk")return"";const i=a.messageParams||{},n=A(s,i);if(n&&n!==s)return n}const o=a.message||"";return!o||o==="ok"?"":o}function Ca(a=10){const s=Math.max(1,Number(a)||10)*1e3;window.setTimeout(()=>{try{window.location.reload()}catch{window.location.href=window.location.href}},s)}function zs(a,s){const o=s?.messageKey||(a==="pm2"?"pm2.msgSwitchPm2":"pm2.msgSwitchGctoac");let i=la({messageKey:o,messageParams:s?.messageParams,message:void 0});i||(i=e(a==="pm2"?"pm2.msgSwitchPm2":"pm2.msgSwitchGctoac"));const n=s?.port||s?.messageParams?.port||(typeof location<"u"&&location.port?location.port:"3850");return[i,A("pm2.portAfterRestart",{port:n}),A("pm2.autoRefreshIn",{n:10})].filter(Boolean).join(`
`)}function Vt(a){return a==="pm2"?`<span class="badge success">${t(Mt(a))}</span>`:a==="ysk-omni"?`<span class="badge agent">${t(Mt(a))}</span>`:a==="none"?`<span class="badge pending">${t(Mt(a))}</span>`:`<span class="badge warn">${t(Mt(a))}</span>`}function Vs(a){return!a||typeof a!="object"?"":Object.entries(a).map(([s,o])=>`${s}=${o}`).join(`
...
${u}`);const p=m?.files||[],c=p.length?p.filter(k=>k.exists).map(k=>`${k.label}: ${k.size<1024?k.size+" B":Math.round(k.size/1024)+" KB"}`).join(" · "):"",w=m?.maxBytes?Math.round(m.maxBytes/(1024*1024)):5,h=m?.keepBytes?Math.round(m.keepBytes/1024):512,g=o?.status||"—",v=g==="online"?e("pm2.statusOnline"):g==="errored"?e("pm2.statusErrored"):g==="stopped"?e("pm2.statusStopped"):g,S=g==="online"?`<span class="badge success">${t(v)}</span>`:g==="errored"?`<span class="badge error">${t(v)}</span>`:t(v),C=s.available,L=s.available&&o,R=s.runner||"unknown",q=l&&g!=="errored"&&s.available!==!1&&s.messageKey!=="pm2.msgErrored",U=Vs(i.env_extra),M=d.pm2Tab==="port"||d.pm2Tab==="config"||d.pm2Tab==="logs"||d.pm2Tab==="runner"?d.pm2Tab:"runner";d.pm2Tab=M;const P=`
  ...
  `),de(),document.querySelectorAll("[data-pm2-tab]").forEach(k=>{k.addEventListener("click",()=>{const x=k.getAttribute("data-pm2-tab")||"runner",N=x==="port"||x==="config"||x==="logs"||x==="runner"?x:"runner";d.pm2Tab!==N&&(d.pm2Tab=N,Le().catch(y))})}),document.getElementById("pm2-logs-refresh")?.addEventListener("click",()=>{d.pm2Tab="logs",Le().catch(y)}),document.getElementById("pm2-logs-clear")?.addEventListener("click",async()=>{if(await ee({message:e("pm2.confirmClearLogs"),variant:"danger",confirmText:e("pm2.clearLogs")}))try{const x=(await I("/pm2/logs/clear",{method:"POST",body:JSON.stringify({which:"all"})})).data?.cleared?.length||0;await ge({message:A("pm2.logsCleared",{n:x})}),Le().catch(y)}catch(k){y(k)}});const G=async k=>{if(await ee({message:e(k==="pm2"?"pm2.confirmSwitchPm2":"pm2.confirmSwitchGctoac"),variant:"confirm",confirmText:e(k==="pm2"?"pm2.switchToPm2":"pm2.switchToGctoac")}))try{const N=await I("/pm2/switch",{method:"POST",body:JSON.stringify({mode:k})}),te=N?.data||N||{},X=zs(k==="pm2"?"pm2":"ysk-omni",te);Ca(10),await ge({title:e("common.notice"),message:X,confirmText:e("common.ok")});try{window.location.reload()}catch{window.location.href=window.location.href}}catch(N){y(N)}};document.getElementById("pm2-refresh").onclick=()=>Le().catch(y),document.getElementById("pm2-switch-pm2").onclick=()=>G("pm2"),document.getElementById("pm2-switch-ysk-omni").onclick=()=>G("ysk-omni"),document.getElementById("pm2-start").onclick=()=>G("pm2"),document.getElementById("pm2-stop").onclick=async()=>{if(await ee({message:e("pm2.confirmStop"),variant:"danger",confirmText:e("pm2.stop")}))try{await I("/pm2/stop",{method:"POST",body:"{}"}),Le().catch(y)}catch(k){y(k)}},document.getElementById("pm2-restart").onclick=async()=>{if(await ee({message:e("pm2.confirmRestart"),variant:"confirm",confirmText:e("pm2.restart")}))try{await G("pm2")}catch(k){y(k)}},document.getElementById("pm2-reload").onclick=async()=>{try{await I("/pm2/reload",{method:"POST",body:"{}"}),Le().catch(y)}catch(k){y(k)}};const b=async k=>{try{const x={...Xs(),restart:k};if(x.port==null){await ge({message:e("pm2.portInvalid")});return}const N=await I("/pm2/config",{method:"PUT",body:JSON.stringify(x)});if(N.data?.scheduled){const te=N.data.portChange?`
...
${A("pm2.autoRefreshIn",{n:10})}`});try{window.location.reload()}catch{window.location.href=window.location.href}}else await ge(N.data?.portChange?A("pm2.portSavedNeedRestart",{port:N.data.port}):e("pm2.configSaved")),Le().catch(y)}catch(x){y(x)}};document.getElementById("pm2-cfg-save").onclick=()=>b(!0),document.getElementById("pm2-cfg-save-only").onclick=()=>b(!1),document.getElementById("pm2-port-default")?.addEventListener("click",()=>{const k=document.getElementById("pm2-cfg-port");k&&(k.value="3850")}),document.getElementById("pm2-port-save")?.addEventListener("click",async()=>{const k=Number(document.getElementById("pm2-cfg-port")?.value);if(!Number.isFinite(k)||k<1||k>65535){await ge({message:e("pm2.portInvalid")});return}if(await ee({message:A("pm2.confirmPortChange",{port:k}),variant:"confirm",confirmText:e("pm2.savePort")}))try{const x=await I("/pm2/config",{method:"PUT",body:JSON.stringify({port:k,restart:!0})}),N=x.data?.scheduled?.message||(x.data?.portChange?A("pm2.portChangedMsg",{from:x.data.portChange.previous,to:x.data.portChange.port}):e("pm2.configSaved"));await ge(N+`
`+A("pm2.portAfterRestart",{port:k}))}catch(x){y(x)}}),document.getElementById("pm2-cfg-reset").onclick=async()=>{if(await ee({message:e("pm2.confirmReset"),variant:"danger",confirmText:e("pm2.resetConfig")}))try{await I("/pm2/config/reset",{method:"POST",body:"{}"}),Le().catch(y)}catch(k){y(k)}}}let ae=[],he=null,ie=[],ft=!1,Ne=0;const Re=new Map,H={keyId:"",model:"",reasoning:!0,effort:"",resumeId:"",forkSession:!1,memory:!1,noPlan:!1,permissionMode:"",systemPrompt:"",systemOpen:!1,settingsOpen:!1},_={mode:"full",recentN:6,summary:"",summaryAt:null,summarySourceCount:0},Ys=3,ns=40,Zs=20,eo=2200,O={conversationId:null,historyPage:0,historyLimit:20,historyQ:"",historyTotal:0,historyItems:[],historyLoading:!1,historyOpenMobile:!1,saving:!1,saveQueued:!1,renamingId:null};let Jt=null;const Ot=10,Sa=/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;function to(a){const s=String(a||"").split(/[/\\]/).pop()||"",o=s.lastIndexOf(".");return o<0?"":s.slice(o).toLowerCase()}function _t(a){return Ka.has(to(a))}function Rt(){return e("chat.formatsHint")}function is(){_.mode="full",_.recentN=6,_.summary="",_.summaryAt=null,_.summarySourceCount=0,Ne=0,Re.clear()}function ao(a){const s=(a?.title||"").trim();if(s)return s;const o=(a?.preview||"").trim();return o||e("chat.untitled")}function so(){return ae.filter(a=>!a.streaming).map(a=>{const s={role:a.role,content:a.content||""};return a.reasoning&&(s.reasoning=a.reasoning),a.docs&&a.docs.length&&(s.docs=a.docs),a.error&&(s.error=!0),s})}function oo(){return{contextMode:_.mode,contextRecentN:_.recentN,summaryText:_.summary||"",summaryAt:_.summaryAt,summarySourceCount:_.summarySourceCount||0}}function no(a){a&&(_.mode=a.contextMode==="summary"||a.contextMode==="recent"?a.contextMode:"full",_.recentN=Math.min(40,Math.max(2,Number(a.contextRecentN)||6)),_.summary=(a.summaryText||"").trim(),_.summaryAt=a.summaryAt||null,_.summarySourceCount=Number(a.summarySourceCount)||0,_.mode==="summary"&&!_.summary&&(_.mode="full"))}function ls(a){return a.reduce((s,o)=>s+(o.content||"").length+(o.reasoning||"").length,0)}function ds(){const a=ae.filter(s=>!s.streaming);return a.length<2?!1:a.length>=Ys?!0:ls(a)>=800}function io(){const a=(H.systemPrompt||"").trim(),s=ae.filter(l=>!l.streaming),o=Math.min(40,Math.max(2,Number(_.recentN)||6));let i=s.map(l=>({role:l.role,content:l.content||""})),n=a;if(_.mode==="summary"&&_.summary){const l=(bt()==="zh-Hant"?`【先前對話摘要 — 僅供延續語境，完整記錄仍在用戶介面】
`:`[Prior conversation summary — full history remains in the UI]
`)+_.summary;n=n?`${n}

${l}`:l;const u=s.slice(_.summarySourceCount||0);i=(u.length?u:s.slice(-o)).slice(-o).map(p=>({role:p.role,content:p.content||""}))}else _.mode==="recent"&&(i=s.slice(-o).map(l=>({role:l.role,content:l.content||""})));const r=i.map(l=>({role:l.role,content:l.content}));return n&&r.unshift({role:"system",content:n}),r}function lo(){return _.mode==="summary"&&_.summary?A("chat.ctxModeSummaryLabel",{n:_.recentN}):_.mode==="recent"?A("chat.ctxModeRecentLabel",{n:_.recentN}):e("chat.ctxModeFullLabel")}function lt(){const a=document.getElementById("chat-compress");if(!a)return;const s=ae.some(n=>n.streaming),o=!!he||ft||s,i=ds();a.disabled=o||!i,a.textContent=e(ft?"chat.compressing":"chat.compress"),a.title=e(i?"chat.compress":"chat.compressNeedMore"),je(),tt()}function tt(){const a=document.getElementById("chat-ctx-mode"),s=document.getElementById("chat-ctx-n");if(a){const o=_.mode==="summary"&&!_.summary?"full":_.mode;a.value=o;const i=a.querySelector('option[value="summary"]');i&&(i.disabled=!_.summary)}s&&(s.value=String(_.recentN),s.disabled=_.mode==="full"),Ft()}function je(){const a=document.getElementById("chat-compress-banner");if(!a)return;const s=!!_.summary,o=ae.filter(r=>!r.streaming).length>40||ls(ae)>6e4;if(!s&&_.mode==="full"&&!o){a.hidden=!0,a.innerHTML="";return}a.hidden=!1;const i=s?_.summary.length>160?`${_.summary.slice(0,159)}…`:_.summary:"",n=o?`<p class="chat-compress-warn">${t(e("chat.ctxLongHint"))}</p>`:"";a.innerHTML=`
...
`)}function co(){return bt()==="zh-Hant"?["你是對話摘要助手。只輸出精簡摘要，不要使用任何工具、不要上網、不要反問。","若已有舊摘要，請合併更新為一份。","請用繁體中文（或對齊原對話語言）條列：","1) 主題與目標 2) 已確定事實／決定 3) 未完成事項 4) 用戶偏好或約束","控制在約 600–1000 字。不要大段複製原文。只輸出摘要正文。"].join(`
...
`)}async function uo(){if(ft||he||ae.some(n=>n.streaming)){Q(e("chat.compressBusy"));return}const a=ae.filter(n=>!n.streaming);if(!ds()){Q(e("chat.compressNeedMore"));return}if(!await ee({title:e("chat.compress"),message:e("chat.compressConfirm"),variant:"confirm",confirmText:e("chat.compress")}))return;const s=Et();if(!s){Q(e("chat.needKey"));return}me(),ft=!0,lt();const o=document.getElementById("chat-send");o&&(o.disabled=!0);const i=document.getElementById("chat-stream-status");i&&(i.hidden=!1,i.textContent=e("chat.compressing"));try{let n=ro(a);_.summary&&(n=(bt()==="zh-Hant"?`先前摘要：
${_.summary}
...
`)+n}]},m=Pt();m&&(u.apiKeyId=m);const p=await fetch("/admin/api/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${s}`,"Content-Type":"application/json"},body:JSON.stringify(u)});if(!p.ok){const h=await p.text();let g=h;try{g=JSON.parse(h).error?.message||h}catch{}throw new Error(g||e("chat.compressFail"))}const c=await p.json();let w=c?.choices?.[0]?.message?.content||c?.choices?.[0]?.delta?.content||"";if(typeof w!="string"&&(w=String(w||"")),w=w.trim().replace(/^【對話摘要】\s*/u,"").replace(/^\[Conversation summary\]\s*/i,""),!w)throw new Error(e("chat.compressFail"));_.summary=w,_.summaryAt=new Date().toISOString(),_.summarySourceCount=a.length,_.mode="summary",je(),tt(),Q(""),i&&(i.hidden=!1,i.textContent=e("chat.compressOk"),setTimeout(()=>{const h=document.getElementById("chat-stream-status");h&&h.textContent===e("chat.compressOk")&&(h.hidden=!0,h.textContent="")},2800)),await kt().catch(()=>{}),await rs()}catch(n){Q(n.message||e("chat.compressFail"))}finally{ft=!1,lt(),o&&(o.disabled=!1),i&&i.textContent===e("chat.compressing")&&(i.hidden=!0,i.textContent="")}}function da(a){O.historyOpenMobile=!!a,document.body.classList.toggle("chat-history-open",O.historyOpenMobile)}function ra(){da(!1)}async function Ue(){if(d.key){O.historyLoading=!0,De();try{const a=O.historyPage*O.historyLimit,s=new URLSearchParams({limit:String(O.historyLimit),offset:String(a)});O.historyQ.trim()&&s.set("q",O.historyQ.trim());const o=await I(`/conversations?${s}`);O.historyItems=o.data||[],O.historyTotal=o.total??0}catch(a){O.historyItems=[],O.historyTotal=0,console.warn(a)}finally{O.historyLoading=!1,De()}}}function De(){const a=document.getElementById("chat-history-list"),s=document.getElementById("chat-history-pager");if(a){if(O.historyLoading&&!O.historyItems.length?a.innerHTML=`<li class="chat-history-empty">${t(e("common.loading"))}</li>`:O.historyItems.length?a.innerHTML=O.historyItems.map(o=>{const i=O.conversationId===o.id?" is-active":"",n=ao(o),r=o.title&&o.preview&&o.preview!==o.title?o.preview:o.model||A("chat.msgs",{n:o.messageCount||0}),l=O.renamingId===o.id,u=n,m=l?`<input type="text" class="chat-history-title-input" data-title-input="${t(o.id)}" value="${t(u)}" maxlength="120" placeholder="${t(e("chat.renamePh"))}" aria-label="${t(e("chat.renamePh"))}" />
    ...
    `;const m=document.getElementById("chat-hist-prev"),p=document.getElementById("chat-hist-next");m&&(m.onclick=()=>{O.historyPage>0&&(O.historyPage-=1,Ue())}),p&&(p.onclick=()=>{(O.historyPage+1)*o<O.historyTotal&&(O.historyPage+=1,Ue())})}if(a.querySelectorAll("[data-open-conv]").forEach(o=>{const i=o.getAttribute("data-open-conv");if(!i)return;let n=null;const r=()=>{n&&(clearTimeout(n),n=null)};o.addEventListener("click",l=>{O.renamingId||l.target instanceof Element&&l.target.closest(".chat-history-item-actions")||(r(),n=setTimeout(()=>{n=null,!O.renamingId&&La(i)},280))}),o.addEventListener("dblclick",l=>{l.preventDefault(),l.stopPropagation(),r(),Xt(i)}),o.addEventListener("keydown",l=>{(l.key==="Enter"||l.key===" ")&&(l.preventDefault(),O.renamingId||La(i))})}),a.querySelectorAll("[data-title-label]").forEach(o=>{o.addEventListener("dblclick",i=>{i.preventDefault(),i.stopPropagation();const n=o.getAttribute("data-title-label");n&&Xt(n)})}),a.querySelectorAll("[data-rename-conv]").forEach(o=>{o.addEventListener("click",i=>{i.preventDefault(),i.stopPropagation();const n=o.getAttribute("data-rename-conv");n&&Xt(n)})}),a.querySelectorAll("[data-del-conv]").forEach(o=>{o.addEventListener("click",i=>{i.preventDefault(),i.stopPropagation();const n=o.getAttribute("data-del-conv");n&&po(n)})}),O.renamingId){const o=String(O.renamingId).replace(/\\/g,"\\\\").replace(/"/g,'\\"'),i=a.querySelector(`[data-title-input="${o}"]`);i instanceof HTMLInputElement&&(mo(i,O.renamingId),requestAnimationFrame(()=>{i.isConnected&&(i.focus(),i.select())}))}}}function Xt(a){a&&(O.renamingId&&O.renamingId!==a&&(O.renamingId=null),O.renamingId=a,De())}function mo(a,s){let o=!1;const i=async n=>{if(o)return;o=!0;const r=a.value;if(O.renamingId=null,!n){De();return}const l=String(r??"").trim().slice(0,120),u=O.historyItems.find(p=>p.id===s),m=u?(u.title||"").trim():"";if(l===m){De();return}u&&(u.title=l),De();try{await I(`/conversations/${s}`,{method:"PATCH",body:JSON.stringify({title:l})}),await Ue()}catch(p){Q(p.message||e("chat.saveFail")),await Ue()}};a.addEventListener("keydown",n=>{n.stopPropagation(),n.key==="Enter"?(n.preventDefault(),i(!0)):n.key==="Escape"&&(n.preventDefault(),i(!1))}),a.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation()}),a.addEventListener("mousedown",n=>n.stopPropagation()),a.addEventListener("dblclick",n=>{n.preventDefault(),n.stopPropagation()}),a.addEventListener("blur",()=>{setTimeout(()=>i(!0),0)})}async function La(a){(!a||he)&&he&&he.abort();try{Q("");const s=await I(`/conversations/${a}`),o=s.data||s;O.conversationId=o.id,Ne=0,Re.clear(),ae=(o.messages||[]).filter(u=>!u.compressed).map(u=>({role:u.role,content:u.content||"",reasoning:u.reasoning||void 0,docs:u.docs,error:u.error})),ie=[],H.systemPrompt=o.systemPrompt||"",no(o),o.model&&(H.model=o.model),o.apiKeyId&&(H.keyId=o.apiKeyId);const i=document.getElementById("chat-system");i&&(i.value=H.systemPrompt);const n=document.getElementById("chat-system-wrap");n&&(n.hidden=!H.systemPrompt.trim()&&!H.systemOpen);const r=document.getElementById("chat-model");r&&o.model&&(r.value=o.model);const l=document.getElementById("chat-key-select");l&&o.apiKeyId&&[...l.options].some(m=>m.value===o.apiKeyId)&&(l.value=o.apiKeyId,H.keyId=o.apiKeyId),He(),Qe(),De(),je(),tt(),ra()}catch(s){Q(s.message||e("chat.loadFail"))}}function Pt(){const a=wa();return!a||String(a).startsWith("admin-session:")||!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(a)?null:a}async function kt(){const a=so();if(!a.length&&!_.summary)return;if(O.saving){O.saveQueued=!0;return}O.saving=!0,O.saveQueued=!1,me();const s={messages:a,model:H.model||null,systemPrompt:H.systemPrompt||"",apiKeyId:Pt(),...oo()};try{if(O.conversationId)await I(`/conversations/${O.conversationId}`,{method:"PATCH",body:JSON.stringify(s)});else{if(!a.length)return;const o=await I("/conversations",{method:"POST",body:JSON.stringify({...s,title:""})}),i=o.data||o;O.conversationId=i.id}await Ue()}catch(o){console.warn(o)}finally{O.saving=!1,O.saveQueued&&(O.saveQueued=!1,kt().catch(()=>{}))}}async function po(a){if(await ee({title:e("chat.deleteConversation"),message:e("chat.deleteConfirm"),variant:"danger",confirmText:e("chat.deleteConversation")}))try{await I(`/conversations/${a}`,{method:"DELETE"}),O.conversationId===a&&(O.conversationId=null,ae=[],ie=[],is(),He(),Qe(),je(),tt()),O.historyItems.length<=1&&O.historyPage>0&&(O.historyPage-=1),await Ue()}catch(s){Q(s.message||e("common.requestFailed"))}}function go(a=!0){he&&he.abort(),ae=[],ie=[],O.conversationId=null,is(),a||(H.systemPrompt="",H.systemOpen=!1),He(),Qe(),De(),je(),tt()}function Et(){return d.key}function wa(){const s=document.getElementById("chat-key-select")?.value||H.keyId||"";return s&&s!=="session"?s:d.me?.id||""}function me(){const a=document.getElementById("chat-key-select"),s=document.getElementById("chat-model"),o=document.getElementById("chat-reasoning"),i=document.getElementById("chat-system");a&&(H.keyId=a.value==="session"?"":a.value),s&&(H.model=s.value),o&&(H.reasoning=o.checked);const n=document.getElementById("chat-effort");n&&(H.effort=n.value||"");const r=document.getElementById("chat-resume");r&&(H.resumeId=r.value.trim());const l=document.getElementById("chat-fork");l&&(H.forkSession=l.checked);const u=document.getElementById("chat-memory");u&&(H.memory=u.checked);const m=document.getElementById("chat-no-plan");m&&(H.noPlan=m.checked);const p=document.getElementById("chat-perm");p&&(H.permissionMode=p.value||""),i&&(H.systemPrompt=i.value),Ft()}function Ft(){const a=document.querySelector(".chat-shell"),s=document.getElementById("chat-settings-toggle"),o=document.getElementById("chat-settings-summary"),i=document.getElementById("chat-settings-toggle-label");if(a&&a.classList.toggle("is-settings-open",!!H.settingsOpen),s&&s.setAttribute("aria-expanded",H.settingsOpen?"true":"false"),i&&(i.textContent=H.settingsOpen?e("chat.settingsHide"):e("chat.settings")),o){const n=H.model||"—",r=_.mode==="summary"?"chat.ctxModeSummary":_.mode==="recent"?"chat.ctxModeRecent":"chat.ctxModeFull";o.textContent=`${n} · ${e(r)}`}}function Qe(){const a=document.getElementById("chat-pending");if(a){if(!ie.length){a.innerHTML="",a.hidden=!0;return}a.hidden=!1,a.innerHTML=ie.map((s,o)=>`
      ...
      </div>`).join(""),a.querySelectorAll("[data-rm-doc]").forEach(s=>{s.onclick=()=>{const o=Number(s.getAttribute("data-rm-doc"));ie.splice(o,1),Qe()}})}}function fo(a,s){return`${a}:${(s||"").length}:${(s||"").slice(0,40)}`}function He(){const a=document.getElementById("chat-messages");if(!a)return;const s=a.scrollHeight-a.scrollTop-a.clientHeight<120,o=ae.some(c=>c.streaming),i=document.getElementById("chat-stream-status");if(i&&(i.hidden=!o,i.textContent=o?e("chat.streaming"):""),!ae.length){a.innerHTML=`
      <div class="chat-empty">
        <strong>${t(e("chat.emptyTitle"))}</strong>
        <p>${t(e("chat.emptyHint"))}</p>
      </div>`,lt();return}const n=ae.length,r=Math.max(0,n-ns);Ne>r&&(Ne=r);const l=Ne,u=ae.slice(l),m=l,p=m>0?`<div class="chat-load-older">
          ...
          </details>`:"";let R=c.content||"";!R&&c.streaming&&(R=c.reasoning?"":"…");const q=c.error?" error":"",U=c.streaming?" is-streaming":"",M=g==="assistant"&&!c.streaming&&!!R;let P;if(M){const x=fo(h,R);if(Re.has(x))P=Re.get(x);else if(P=Ua(R),Re.set(x,P),Re.size>200){const N=Re.keys().next().value;Re.delete(N)}}else P=t(R);const K=!c.streaming&&R.length>eo,T=`${M?"chat-content md":"chat-content"}${K?" is-collapsible":""}`,f=K?`<button type="button" class="btn ghost sm chat-expand-btn" data-expand="${h}">${t(e("chat.showMore"))}</button>`:"",$=yo(c),G=$?`<div class="muted chat-spend">${t($)}</div>`:"",b=Array.isArray(c.tools)&&c.tools.length?`<div class="chat-tools">${c.tools.map(x=>`<span class="chat-tool-chip" title="${t(x.arguments||"")}">${t(x.name||"tool")}</span>`).join("")}</div>`:"",k=R?`<button type="button" class="chat-copy-btn" data-copy-msg="${h}" title="${t(e("chat.copy"))}">${t(e("chat.copy"))}</button>`:"";return`<div class="chat-bubble ${g}${q}${U}" data-msg-idx="${h}">
...
✗ ${L.message||L}`,Q(L.message||String(L)))}finally{c.streaming=!1,he=null,He(),lt(),g&&(g.disabled=!1),S&&(S.disabled=!1),C&&(C.disabled=!1),v&&(v.disabled=!0),kt().catch(()=>{})}}const Zt="email@ysk.hk",Eo="https://github.com/sponsors/yanshekki",Io="https://linktr.ee/yanshekki",Mo="https://ysk.hk/",xo="https://github.com/yanshekki/ysk-omni#readme";async function qo(){const s=[[e("support.netEvm"),"yanshekki.eth"],[e("support.netNear"),"yanshekki.near"],[e("support.netAda"),"$yanshekki"]].map(([o,i])=>`
  ...
  `),de(),document.querySelectorAll("[data-jump]").forEach(o=>{o.addEventListener("click",()=>{const i=o.getAttribute("data-jump");i&&document.getElementById(i)?.scrollIntoView({behavior:"smooth",block:"start"})})}),document.querySelectorAll("[data-copy]").forEach(o=>{o.addEventListener("click",async()=>{const i=o.getAttribute("data-copy")||"",n=await jt(i);o.textContent=e(n?"chat.copied":"support.copy"),setTimeout(()=>{o.textContent=e("support.copy")},1400)})})}const To=[{id:"Qwen/Qwen2.5-0.5B-Instruct-GGUF",modality:"text",runtime:"llamacpp",quants:["Q4_K_M","Q5_K_M","Q8_0"],vramMb:512},{id:"Qwen/Qwen2.5-7B-Instruct",modality:"text",runtime:"vllm",quants:[],vramMb:16e3},{id:"Tongyi-MAI/Z-Image-Turbo",label:"Z-Image-Turbo",modality:"image",runtime:"diffusion",quants:[],vramMb:8e3},{id:"black-forest-labs/FLUX.2-klein-4B",label:"FLUX.2 Klein 4B",modality:"image",runtime:"diffusion",quants:[],vramMb:8e3},{id:"Qwen/Qwen3-TTS",modality:"tts",runtime:"diffusion",quants:[],vramMb:4e3},{id:"Systran/faster-whisper-small",modality:"stt",runtime:"whisper",quants:[],vramMb:1e3},{id:"Lightricks/LTX-2.5",label:"LTX-2.5",modality:"video",runtime:"diffusion",quants:[],vramMb:12e3},{id:"Wan-AI/Wan2.2",label:"Wan 2.2",modality:"video",runtime:"diffusion",quants:[],vramMb:2e4}];function ea(a){return a.label||String(a.id||"").split("/").pop()||a.id||""}function Da(a,s){const o=String(a.id||"");return(s||[]).filter(i=>i.id===o||i.repoId===o||String(i.id||"").startsWith(`${o}:`))}function Tt(a){const s=`catalog.mod.${a}`;return Me(s)?e(s):a||"—"}function Ao(a){const s=a||[];return s.includes("Q4_K_M")?"Q4_K_M":s[0]||""}function Bo(a){const s=Number(a)||0;return s>=1e6?`${(s/1e6).toFixed(1)}M`:s>=1e3?`${(s/1e3).toFixed(1)}k`:String(s)}async function mt({append:a=!1}={}){if(!d.catalogHubBusy){d.catalogHubBusy=!0;try{const s=new URLSearchParams;d.catalogHubQ&&s.set("q",d.catalogHubQ),d.catalogModality&&s.set("modality",d.catalogModality),a&&d.catalogHubNext&&s.set("cursor",d.catalogHubNext);const o=await I(`/catalog/hub?${s}`),i=o.hits||[];d.catalogHubHits=a?[...d.catalogHubHits||[],...i]:i,d.catalogHubNext=o.nextCursor||""}catch(s){a||(d.catalogHubHits=[]),y(s)}finally{d.catalogHubBusy=!1,await ye()}}}async function ye(){let a={};try{a=await I("/catalog")}catch(b){y(b)}const s=a.packs&&a.packs.length?a.packs:To,o=a.local||[],i=a.loaded||[];a.popularSyncedAt&&(d.catalogPopularSyncedAt=a.popularSyncedAt),d.catalogHubHits==null&&Array.isArray(a.popular)&&a.popular.length&&(d.catalogHubHits=a.popular);const n=new Set(i.map(b=>b.id)),r=a.usedMb??0,l=a.budgetMb??0,u=l?Math.min(100,Math.round(r/l*100)):0,m=d.catalogTab==="hub"||d.catalogTab==="local"?d.catalogTab:"local";d.catalogTab=m;const p=d.catalogModality||"",c=["text","image","video","tts","stt"],w=p?s.filter(b=>b.modality===p):s,h=d.catalogPulling||"",g=w.map(b=>{const k=Da(b,o),x=b.quants||[],N=Ao(x),te=x.length?`<select class="catalog-quant-select" data-quant-for="${t(b.id)}">${x.map(z=>`<option value="${t(z)}" ${z===N?"selected":""}>${t(z)}</option>`).join("")}</select>`:`<span class="muted">${t(e("catalog.noQuant"))}</span>`,X=h===b.id,j=X?e("catalog.pulling"):k.length?e("catalog.pullAgain"):e("catalog.pull");return`
      <tr>
        <td>
          <div class="cell-primary">${t(ea(b))}</div>
          <div class="cell-sub mono" title="${t(b.id)}">${t(b.id)}</div>
        </td>
        <td><span class="badge muted">${t(Tt(b.modality))}</span></td>
        ...
        <td>
          <div class="row-actions">
            <button type="button" class="btn ${k.length?"secondary":""} sm" data-pull="${t(b.id)}" ${X?"disabled":""}>${t(j)}</button>
            <span class="muted" data-pull-status="${t(b.id)}"></span>
          </div>
        </td>
      </tr>`}).join(""),v=`
    ...
    </td></tr>`,S=o.map(b=>{const k=n.has(b.id);return`
      <tr>
        <td>
    ...
    </td></tr>`,L=i.map(b=>ea({id:b.id})).join(", "),R=`
    <div class="grid catalog-kpi-grid media-kpi-grid">
      ...
      <label>${t(e("catalog.filterModality"))}
        <select id="cat-mod">
          ...
          ${c.map(b=>`<option value="${t(b)}" ${p===b?"selected":""}>${t(Tt(b))}</option>`).join("")}
        </select>
    ...
    </div>`,U=(Array.isArray(d.catalogHubHits)?d.catalogHubHits:[]).filter(b=>b.supported!==!1),M=U.map(b=>{const k=Da({id:b.id},o),x=h===b.id,N=b.supported?`<button type="button" class="btn ${k.length?"secondary":""} sm" data-pull="${t(b.id)}" ${x?"disabled":""}>${t(x?e("catalog.pulling"):k.length?e("catalog.pullAgain"):e("catalog.pull"))}</button>`:`<span class="muted">${t(e("catalog.unsupported"))}</span>`;return`
      <tr>
        <td>
          <div class="cell-primary">${t(ea(b))}</div>
    ...
    </td></tr>`,K=[["",e("catalog.filterAll")],...c.map(b=>[b,Tt(b)])].map(([b,k])=>`<button type="button" class="catalog-mod-chip ${p===b?"is-on":""}" data-hub-mod="${t(b)}" aria-pressed="${p===b}">${t(k)}</button>`).join(""),T=`
  ...
  `),de(),document.querySelectorAll("[data-catalog-tab]").forEach(b=>{b.onclick=()=>{d.catalogTab=b.getAttribute("data-catalog-tab")||"packs",ye().catch(y)}});const $=document.getElementById("cat-mod");$&&($.onchange=()=>{d.catalogModality=$.value,ye().catch(y)}),document.querySelector("#catalog-tab-packs [data-filter-apply]")?.addEventListener("click",()=>{d.catalogModality=document.getElementById("cat-mod")?.value||"",ye().catch(y)}),document.querySelector("#catalog-tab-packs [data-filter-reset]")?.addEventListener("click",()=>{d.catalogModality="",ye().catch(y)});const G=()=>{d.catalogHubQ=document.getElementById("cat-hub-q")?.value.trim()||"",d.catalogHubHits=null,mt().catch(y)};document.getElementById("cat-hub-go")?.addEventListener("click",G),document.getElementById("cat-hub-reset")?.addEventListener("click",()=>{d.catalogHubQ="",d.catalogModality="",d.catalogHubHits=null,mt().catch(y)}),document.querySelectorAll("[data-hub-mod]").forEach(b=>{b.addEventListener("click",()=>{d.catalogModality=b.getAttribute("data-hub-mod")||"",d.catalogHubQ=document.getElementById("cat-hub-q")?.value.trim()||"",d.catalogHubHits=null,mt().catch(y)})}),document.getElementById("cat-hub-q")?.addEventListener("keydown",b=>{b.key==="Enter"&&(b.preventDefault(),G())}),document.getElementById("cat-spec")?.addEventListener("keydown",b=>{b.key==="Enter"&&(b.preventDefault(),document.getElementById("cat-spec-pull")?.click())}),document.getElementById("cat-hub-more")?.addEventListener("click",()=>{mt({append:!0}).catch(y)}),m==="hub"&&d.catalogHubHits==null&&!d.catalogHubBusy&&mt().catch(y),document.getElementById("cat-sync")?.addEventListener("click",async()=>{const b=document.getElementById("cat-sync");if(b){b.setAttribute("disabled","disabled"),b.textContent=e("catalog.syncing");try{const k=await I("/catalog/sync",{method:"POST",body:JSON.stringify({})});d.catalogHubHits=k.hits||[],d.catalogHubNext="",d.catalogHubQ="",d.catalogTab="hub",d.catalogPopularSyncedAt=k.syncedAt||"",await ye()}catch(k){b.removeAttribute("disabled"),b.textContent=e("catalog.sync"),y(k)}}}),document.querySelectorAll("[data-pull]").forEach(b=>{b.onclick=async()=>{const k=b.getAttribute("data-pull")||"",x=document.querySelector(`[data-quant-for="${CSS.escape(k)}"]`),N=x&&x.value?x.value:"",te=N?`${k}:${N}`:k,X=document.querySelector(`[data-pull-status="${CSS.escape(k)}"]`);d.catalogPulling=k,b.disabled=!0,b.textContent=e("catalog.pulling");try{const j=await fetch(`${ht}/catalog/pull`,{method:"POST",headers:{"Content-Type":"application/json",...d.key?{Authorization:`Bearer ${d.key}`}:{}},body:JSON.stringify({model:te})}),z=j.body&&j.body.getReader?j.body.getReader():null;let D="";if(z){const Y=new TextDecoder;for(;;){const{done:re,value:Ce}=await z.read();if(re)break;const fe=Y.decode(Ce,{stream:!0});D+=fe;const oe=D.split(`
...
`).map(j=>j.trim()).filter(Boolean),X=te.length?JSON.parse(te[te.length-1]):{};if(!x.ok||X.status==="error")throw new Error(X.error?.message||X.reason||x.statusText);d.catalogPulling="",d.catalogTab="local",await ye()}catch(x){d.catalogPulling="",k&&(k.removeAttribute("disabled"),k.textContent=e("catalog.pullSpecBtn")),y(x)}})}async function Gt(){const a=document.getElementById("app");try{if(!d.key){await Va();return}d.me||await za(),d.page==="dashboard"?await aa():d.page==="chat"?await So():d.page==="chats"?await pt():d.page==="keys"?await Fe():d.page==="documents"?await st():d.page==="media"?await xe():d.page==="catalog"?await ye():d.page==="audit"?await gt():d.page==="settings"?await as():d.page==="apiFeatures"?await Bt():d.page==="usage"?await pe():d.page==="ddos"?await ne():d.page==="queue"?await ue():d.page==="pm2"?await Le():d.page==="system"?await ot():d.page==="support"?await qo():await aa()}catch(s){a.innerHTML=le(`<div class="error-box">${t(s.message)}</div>`),de()}}let Je=null;const W={tab:"overview",status:"",sortBy:"queuedAt",sortDir:"desc",limit:20,offset:0};function Nt(a){return!a||a<0?"—":a<1e3?`${a}ms`:a<6e4?`${Math.round(a/1e3)}s`:a<36e5?`${Math.round(a/6e4)}m`:`${(a/36e5).toFixed(1)}h`}const Co=["enabled","globalConcurrency","perKeyConcurrency","maxQueueDepth","maxQueueDepthPerKey","fairness","defaultPriority","playgroundPriority","leaseMs","maxWaitMs"];function us(){return{relaxed:{enabled:!0,globalConcurrency:6,perKeyConcurrency:2,maxQueueDepth:200,maxQueueDepthPerKey:40,fairness:"weighted_round_robin",defaultPriority:100,playgroundPriority:40,leaseMs:6e4,maxWaitMs:9e5},balanced:{enabled:!0,globalConcurrency:4,perKeyConcurrency:1,maxQueueDepth:100,maxQueueDepthPerKey:20,fairness:"weighted_round_robin",defaultPriority:100,playgroundPriority:50,leaseMs:45e3,maxWaitMs:6e5},strict:{enabled:!0,globalConcurrency:2,perKeyConcurrency:1,maxQueueDepth:40,maxQueueDepthPerKey:8,fairness:"fifo_global",defaultPriority:100,playgroundPriority:80,leaseMs:3e4,maxWaitMs:3e5}}}function Oa(a){if(!a)return{};const s={};for(const o of Co){const i=a[o];typeof i=="boolean"?s[o]=i:typeof i=="number"&&Number.isFinite(i)?s[o]=Math.round(i):typeof i=="string"?s[o]=i:i==null?s[o]=null:s[o]=i}return s}function ms(a,s){return JSON.stringify(Oa(a))===JSON.stringify(Oa(s))}function ca(a){if(!a)return"custom";const s=us();for(const o of["relaxed","balanced","strict"])if(ms(a,s[o]))return o;return"custom"}function Lo(a){return e(a==="relaxed"?"queue.presetRelaxed":a==="balanced"?"queue.presetBalanced":a==="strict"?"queue.presetStrict":"queue.presetCustom")}function ps(a,{unsaved:s=!1}={}){const o=Lo(a),i=a==="relaxed"?"relaxed":a==="balanced"?"balanced":a==="strict"?"strict":"custom",n=s?A("queue.presetFormLabel",{name:o}):A("queue.presetActiveLabel",{name:o});return`<span class="ddos-preset-badge is-${i}" id="queue-preset-badge" title="${t(n)}">${t(n)}</span>`}function gs(){return{enabled:document.getElementById("q-master-enabled")?Xe("q-master-enabled"):!0,globalConcurrency:Math.max(1,Math.min(64,Math.floor(V("qp-gconc",4)))),perKeyConcurrency:Math.max(1,Math.min(16,Math.floor(V("qp-kconc",1)))),maxQueueDepth:Math.max(1,Math.floor(V("qp-depth",100))),maxQueueDepthPerKey:Math.max(1,Math.floor(V("qp-depthk",20))),fairness:document.getElementById("qp-fair")?.value==="fifo_global"?"fifo_global":"weighted_round_robin",defaultPriority:Math.max(0,Math.min(1e3,Math.floor(V("qp-pri",100)))),playgroundPriority:Math.max(0,Math.min(1e3,Math.floor(V("qp-ppri",50)))),leaseMs:Math.max(5e3,Math.floor(V("qp-lease",45e3))),maxWaitMs:Math.max(5e3,Math.floor(V("qp-wait",6e5)))}}function Kt(a){Ye("q-master-enabled",a,e("queue.masterOn"),e("queue.masterOff")),et("queue-root",!a),Ze("queue-disabled-banner",!a);const s=document.getElementById("qk-pill-enabled");s&&(s.innerHTML=Ke(a,e("dash.on"),e("dash.off")))}function Ho(a){if(!a)return;const s=(i,n)=>{const r=document.getElementById(i);r&&(r.value=String(n))};Kt(a.enabled!==!1),s("qp-gconc",a.globalConcurrency),s("qp-kconc",a.perKeyConcurrency),s("qp-depth",a.maxQueueDepth),s("qp-depthk",a.maxQueueDepthPerKey);const o=document.getElementById("qp-fair");o&&(o.value=a.fairness||"weighted_round_robin"),s("qp-pri",a.defaultPriority),s("qp-ppri",a.playgroundPriority),s("qp-lease",a.leaseMs),s("qp-wait",a.maxWaitMs),yt()}function yt(){if(!document.getElementById("queue-policy-panel"))return;let a;try{a=gs()}catch{return}const s=ca(a),o=ca(d._queuePolicyCache||a),i=!ms(a,d._queuePolicyCache||a);document.querySelectorAll("[data-queue-preset]").forEach(l=>{const u=l.dataset.queuePreset;if(u==="custom"){const w=s==="custom";l.classList.toggle("is-active",w),l.setAttribute("aria-pressed",w?"true":"false"),l.disabled=!w;return}const m=u===s,p=u===o;l.classList.toggle("is-active",m),l.classList.toggle("is-saved",p&&!m),l.setAttribute("aria-pressed",m?"true":"false");const c=e(u==="relaxed"?"queue.presetRelaxed":u==="balanced"?"queue.presetBalanced":"queue.presetStrict");m&&p?l.innerHTML=`${t(c)} <span class="preset-tag">${t(e("queue.presetTagActive"))}</span>`:m&&i?l.innerHTML=`${t(c)} <span class="preset-tag preset-tag--draft">${t(e("queue.presetTagDraft"))}</span>`:p?l.innerHTML=`${t(c)} <span class="preset-tag preset-tag--saved">${t(e("queue.presetTagSaved"))}</span>`:l.textContent=c});const n=document.getElementById("queue-preset-badge");n&&(n.outerHTML=ps(s,{unsaved:i&&s!==o}));const r=document.getElementById("queue-preset-hint");if(r){const l={relaxed:e("queue.presetRelaxedHint"),balanced:e("queue.presetBalancedHint"),strict:e("queue.presetStrictHint"),custom:e("queue.presetCustomHint")};r.textContent=l[s]||l.custom}}function Do(){document.querySelectorAll("[data-queue-preset]").forEach(a=>{a.dataset.queuePreset!=="custom"&&(a.onclick=()=>{const s=a.dataset.queuePreset,o=us()[s];o&&Ho(o)})}),["qp-gconc","qp-kconc","qp-depth","qp-depthk","qp-fair","qp-pri","qp-ppri","qp-lease","qp-wait"].forEach(a=>{const s=document.getElementById(a);s&&(s.addEventListener("change",()=>yt()),s.addEventListener("input",()=>yt()))}),yt()}function _a(){return document.querySelector(".main")}function fs(a){return a.map(s=>{const o=s.status==="queued"||s.status==="leased"||s.status==="running",i=s.status==="failed"||s.status==="dead"||s.status==="cancelled",n=s.startedAt||s.finishedAt?null:s.queuedAt?Date.now()-new Date(s.queuedAt).getTime():null;return`
    <tr data-q-row="${t(s.id)}">
      ...
      <td>
        ...
        </div>`,document.getElementById("q-filter-dead")?.addEventListener("click",()=>{W.status="dead",W.offset=0,W.tab="jobs",ue().catch(y)})):C.innerHTML="");const L=(T,f)=>{const $=document.getElementById(T);$&&($.textContent=String(f??0))};L("q-tab-count-jobs",i),L("q-tab-count-dead",r);const R=document.getElementById("qk-jobs-meta");R&&(R.textContent=A("queue.jobsMeta",{n:i}));const q=document.querySelector("#queue-jobs-table tbody");if(q){const T=o.map($=>`${$.id}|${$.status}|${$.priority}|${$.attempt}|${$.cancelRequested?1:0}|${$.errorMessage||""}|${$.startedAt||""}|${$.finishedAt||""}`).join(";"),f=fs(o)||`<tr class="empty-row"><td colspan="9">
        <div class="data-empty">
          <div class="data-empty-icon">∅</div>
          <strong>${t(e("queue.empty"))}</strong>
        </div>
      </td></tr>`;if(q.dataset.qsig!==T){const $=document.querySelector("#queue-jobs-table .table-wrap"),G=$?.scrollLeft||0;q.dataset.qsig=T,q.innerHTML=f,ys(),va(document.querySelector("#queue-jobs-table")||document),$&&($.scrollLeft=G)}else o.forEach($=>{if($.status!=="queued"||!$.queuedAt)return;const G=Date.now()-new Date($.queuedAt).getTime(),b=String($.id||"");let k=null;q.querySelectorAll("[data-q-row]").forEach(N=>{N.getAttribute("data-q-row")===b&&(k=N)});const x=k?.querySelector("[data-q-wait]");x&&(x.textContent=`${e("queue.wait")}: ${Nt(G)}`)})}if(document.querySelector("#queue-pager .data-pager-meta span")){const T=Math.max(1,Math.ceil((i||0)/W.limit)||1),f=Math.floor(W.offset/W.limit)+1,$=document.querySelectorAll("#queue-pager .data-pager-meta > span");$[0]&&($[0].textContent=A("common.pagerTotal",{n:i||0})),$[1]&&($[1].textContent=A("common.pagerPage",{n:f,total:T}));const G=document.getElementById("queue-prev"),b=document.getElementById("queue-next");G&&(G.disabled=W.offset<=0),b&&(b.disabled=W.offset+W.limit>=i)}const M=document.getElementById("q-pause");M&&(M.textContent=s.paused?e("queue.resume"):e("queue.pause"));const P=document.getElementById("q-drain");P&&(P.textContent=s.drainMode?e("queue.undrain"):e("queue.drainBtn"));const K=document.getElementById("q-master-enabled");K&&document.activeElement!==K&&Kt(s.enabled!==!1)}function Ra(){Je||(Je=setInterval(()=>{if(d.page!=="queue"){clearInterval(Je),Je=null;return}const a=document.activeElement;a&&a.closest&&a.closest("#queue-policy-panel")&&(a.tagName==="INPUT"||a.tagName==="SELECT"||a.tagName==="TEXTAREA")||ue({soft:!0}).catch(()=>{})},4e3))}async function ue(a={}){const s=!!a.soft&&document.getElementById("queue-root");!s&&Je&&(clearInterval(Je),Je=null);const o=_a(),i=!s&&o?o.scrollTop:0,n=W;n.sortBy||(n.sortBy="queuedAt"),n.sortDir||(n.sortDir="desc");const r=new URLSearchParams;r.set("limit",String(n.limit)),r.set("offset",String(n.offset)),n.status&&r.set("status",n.status),Pe(r,n);const[l,u,m]=await Promise.all([I("/queue/stats"),I(`/queue/jobs?${r}`),I("/queue/policy")]);if(d.page!=="queue")return;const p=l.data||{},c=m.data||p.policy||{},w=u.data||[],h=u.total??w.length,g=p.byStatus||{},v=p.dead??g.dead??0,S=p.leased??g.leased??0,C=p.running??g.running??0,L=p.queued??g.queued??0,R=p.depth??L+S+C,q=bs(c);if(d._queuePolicyCache={...c},s){Oo({s:p,pol:c,jobs:w,total:h,by:g}),Ra();return}W.tab||(W.tab="overview");const U=W.tab==="jobs"||W.tab==="policy"?W.tab:"overview";W.tab=U;const M=fs(w),P=Oe({title:e("queue.filterTitle"),hint:e("queue.filterHint"),meta:A("queue.jobsMeta",{n:h}),gridHtml:`
  ...
  `),de(),document.querySelectorAll("[data-queue-tab]").forEach(j=>{j.addEventListener("click",()=>{const z=j.getAttribute("data-queue-tab")||"overview";z!=="overview"&&z!=="jobs"&&z!=="policy"||W.tab!==z&&(W.tab=z,ue().catch(y))})}),i>0){const j=_a();j&&(j.scrollTop=i,requestAnimationFrame(()=>{j.scrollTop=i}))}document.getElementById("q-master-enabled").onclick=async()=>{const j=!Xe("q-master-enabled");Kt(j);try{const z=await I("/queue/policy",{method:"PUT",body:JSON.stringify({enabled:j})});d._queuePolicyCache={...d._queuePolicyCache||{},...z.data||{enabled:j}},yt()}catch(z){Kt(!j),y(z)}},document.getElementById("q-pause").onclick=async()=>{await I(c.paused?"/queue/resume":"/queue/pause",{method:"POST",body:"{}"}),ue().catch(y)},document.getElementById("q-drain").onclick=async()=>{await I(c.drainMode?"/queue/undrain":"/queue/drain",{method:"POST",body:"{}"}),ue().catch(y)};let N=!1;const te=async()=>{if(!N){N=!0;try{if(!await ee({title:e("queue.purgeTitle"),message:e("queue.purgeConfirm"),variant:"danger",confirmText:e("queue.purgeConfirmBtn"),cancelText:e("common.cancel")}))return;const z=await I("/queue/purge-dead",{method:"POST",body:"{}"}),D=Number(z?.data?.deleted??0);await ge({title:e("queue.purgeDoneTitle"),message:A("queue.purgeDoneMsg",{n:D}),confirmText:e("common.ok")}),await ue()}finally{N=!1}}},X=document.getElementById("queue-root");X&&(X.onclick=j=>{j.target?.closest?.("#q-purge, #q-purge-dlq")&&(j.preventDefault(),te().catch(y))}),document.getElementById("q-filter-dead")?.addEventListener("click",()=>{W.status="dead",W.offset=0,W.tab="jobs",ue().catch(y)}),document.querySelectorAll("[data-filter-apply]").forEach(j=>{j.onclick=()=>{W.status=document.getElementById("qf-status")?.value||"",W.offset=0,ue().catch(y)}}),document.querySelectorAll("[data-filter-reset]").forEach(j=>{j.onclick=()=>{W.status="",W.sortBy="queuedAt",W.sortDir="desc",W.offset=0,ue().catch(y)}}),dt("queue",W,()=>ue().catch(y)),Ge(W,()=>ue().catch(y)),document.getElementById("qp-save").onclick=async()=>{const j=gs();await I("/queue/policy",{method:"PUT",body:JSON.stringify(j)}),d._queuePolicyCache={...d._queuePolicyCache||{},...j},Q(""),ue().catch(y)},Do(),ys(),Ra()}d.page=Ss();(!location.hash||location.hash==="#"||location.hash==="#/")&&pa(d.page);window.addEventListener("hashchange",()=>{const a=ma(location.hash);a&&a!==d.page&&fa(a,{writeHash:!1})});window.addEventListener("popstate",()=>{const a=ma(location.hash);!a||a===d.page||fa(a,{writeHash:!1})});Gt();
