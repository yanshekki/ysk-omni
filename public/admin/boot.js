  ...
  `,document.querySelectorAll("[data-lang]").forEach(s=>{s.onclick=()=>{Ra(s.dataset.lang),Wa().catch(y)}}),document.getElementById("btn-copy-cmd").onclick=async()=>{try{await navigator.clipboard.writeText(a);const s=document.getElementById("btn-copy-cmd");s.textContent=e("loginCopied"),setTimeout(()=>{s.textContent=e("loginCopy")},1500)}catch{}},document.getElementById("btn-login").onclick=async()=>{const s=document.getElementById("login-key").value.trim();if(!s)return U(e("needOtp"));try{const o=await fetch("/admin/api/auth/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({code:s})}),i=await o.json().catch(()=>({}));if(!o.ok)throw new Error(i?.error?.message||i?.message||e("loginOtpFail"));const n=i?.data?.token;if(!n)throw new Error(e("loginOtpFail"));c.key=n,sessionStorage.setItem(At,n),await Qa(),c.error="",ua("dashboard")}catch(o){c.key="",sessionStorage.removeItem(At),U(o.message||e("loginOtpFail"))}},document.getElementById("login-key").onkeydown=s=>{s.key==="Enter"&&document.getElementById("btn-login").click()}}function ve({label:a,value:s,sub:o,tone:i,href:n,valueId:d,subId:l}){const u=i?` dash-kpi--${i}`:"",m=d?` id="${t(d)}"`:"",p=l?` id="${t(l)}"`:"",r=`
    ...
    <tr>
      ...
      <th>${t(e("chats.time"))}</th>`,bodyHtml:R,colSpan:6,emptyText:e("dash.empty")}),j=Math.max(1,...m.map(P=>P.requests||0)),x=m.length?m.map(P=>{const g=Math.round((P.requests||0)/j*100);return`
          ...
          </div>`}).join(""):`<div class="data-empty" style="padding:20px"><strong>${t(e("dash.emptyModels"))}</strong></div>`,w=(P,g)=>`<span class="dash-rule-chip ${P?"is-on":"is-off"}">${t(g)}</span>`,k=l?`
    ...
    <div class="dash-kpi-grid">
      ...
      ${ve({label:e("dash.kpiQueue"),value:b,sub:f,tone:h,href:"queue"})}
...
`):s,hasRoles:!0,blocks:d}}async function mt(){await Promise.all([ht(),kt()]);const a=c.chatFilter,s=new URLSearchParams;if(s.set("limit",String(a.limit)),s.set("offset",String(a.offset)),a.status&&s.set("status",a.status),a.model&&s.set("model",a.model),a.apiKeyId&&s.set("apiKeyId",a.apiKeyId),a.q&&s.set("q",a.q),a.from&&s.set("from",new Date(a.from).toISOString()),a.to){const r=new Date(a.to);r.setHours(23,59,59,999),s.set("to",r.toISOString())}a.policyMode&&s.set("policyMode",a.policyMode),a.hasDocuments!==""&&s.set("hasDocuments",a.hasDocuments),ke(s,a);const o=await M(`/chats?${s}`),i=o.total||0,n=[`<option value="">${t(e("chats.allModels"))}</option>`,...c.models.map(r=>`<option value="${t(r)}" ${a.model===r?"selected":""}>${t(r)}</option>`)].join(""),d=[`<option value="">${t(e("chats.allKeys"))}</option>`,...c.keys.map(r=>`<option value="${r.id}" ${a.apiKeyId===r.id?"selected":""}>${t(r.name)} (${t(r.keyPrefix)})</option>`)].join(""),l=(o.items||[]).map(r=>{const S=za(r.promptPreview||""),b=!!S.system,f=b?S.body.slice(0,160):r.promptPreview||"";return`
  ...
  `),le(),dt("chats",c.chatFilter,()=>mt().catch(y)),Ue(c.chatFilter,()=>mt().catch(y));const p=()=>{c.chatFilter.q=document.getElementById("f-q").value.trim(),c.chatFilter.status=document.getElementById("f-status").value,c.chatFilter.model=document.getElementById("f-model").value,c.chatFilter.apiKeyId=document.getElementById("f-key").value,c.chatFilter.policyMode=document.getElementById("f-mode").value,c.chatFilter.from=document.getElementById("f-from").value,c.chatFilter.to=document.getElementById("f-to").value,c.chatFilter.hasDocuments=document.getElementById("f-docs").checked?"true":"",c.chatFilter.offset=0,mt().catch(y)};document.querySelector("[data-filter-apply]").onclick=p,document.getElementById("f-q").onkeydown=r=>{r.key==="Enter"&&p()},document.querySelector("[data-filter-reset]").onclick=()=>{c.chatFilter={q:"",status:"",model:"",apiKeyId:"",from:"",to:"",policyMode:"",hasDocuments:"",sortBy:"createdAt",sortDir:"desc",limit:50,offset:0},mt().catch(y)},document.querySelectorAll("[data-chat]").forEach(r=>{r.onclick=()=>Va(r.dataset.chat)})}async function Va(a){const{data:s}=await M(`/chats/${a}`),o=s.response||{},i=s.documents||[];let n=`<p class="muted">${t(e("chats.noAttach"))}</p>`;if(i.length){const m=[];for(const p of i){let r="";if($t(p.mimeType))try{const S=await M(`/documents/${p.id}`),b=await Ya(S.data||{id:p.id,isImage:!0,mimeType:p.mimeType});b?.src&&(r=`<img class="preview" src="${b.src}" alt="${t(p.originalName)}" />`)}catch{r=`<span class="muted">${t(e("chats.previewFailed"))}</span>`}m.push(`
    ...
    ${u}
    ${m}
  `),le(),dt("keys",c.keyFilter,()=>Re().catch(y)),Ue(c.keyFilter,()=>Re().catch(y)),document.querySelector("[data-filter-apply]").onclick=()=>{c.keyFilter.q=document.getElementById("kf-q").value.trim(),c.keyFilter.role=document.getElementById("kf-role").value,c.keyFilter.mode=document.getElementById("kf-mode").value,c.keyFilter.isActive=document.getElementById("kf-active").value,c.keyFilter.offset=0,Re().catch(y)},document.querySelector("[data-filter-reset]").onclick=()=>{c.keyFilter={q:"",role:"",mode:"",isActive:"",sortBy:"createdAt",sortDir:"desc",limit:20,offset:0},Re().catch(y)},document.getElementById("btn-new-key").onclick=()=>Pa(),document.querySelectorAll("[data-edit]").forEach(p=>{const r=n.find(S=>S.id===p.dataset.edit);p.onclick=()=>Pa(r)}),document.querySelectorAll("[data-revoke]").forEach(p=>{p.onclick=async()=>{await Z({message:e("keys.confirmRevoke"),variant:"danger",confirmText:e("keys.revoke")})&&(await M(`/keys/${p.dataset.revoke}`,{method:"DELETE"}),Re().catch(y))}})}function Pa(a){const s=!!a,o=(a?.ipWhitelist||[]).join(`
...
${d.data?.key||JSON.stringify(d.data)}`);const u=document.getElementById("k-save");u&&(u.textContent=e("chats.close"),u.onclick=()=>{ge(),Re().catch(y)})}}catch(d){y(d)}}}function Ja(a){return e(a==="filesystem"?"docs.storageFs":"docs.storageDb")}async function Xa(a,s){try{const o=await fetch(`${Ft}/documents/${a}/download`,{headers:c.key?{Authorization:`Bearer ${c.key}`}:{}});if(!o.ok){const l=await o.text();let u=l;try{u=JSON.parse(l).error?.message||l}catch{}throw new Error(u||e("docs.downloadFail"))}const i=await o.blob(),n=URL.createObjectURL(i),d=document.createElement("a");d.href=n,d.download=s||"download",document.body.appendChild(d),d.click(),d.remove(),URL.revokeObjectURL(n)}catch(o){U(o.message||e("docs.downloadFail"))}}async function st(){await kt();const a=c.docFilter,s=new URLSearchParams({limit:String(a.limit),offset:String(a.offset)});if(a.q&&s.set("q",a.q),a.apiKeyId&&s.set("apiKeyId",a.apiKeyId),a.storageType&&s.set("storageType",a.storageType),a.from&&s.set("from",new Date(a.from).toISOString()),a.to){const r=new Date(a.to);r.setHours(23,59,59,999),s.set("to",r.toISOString())}ke(s,a);const o=await M(`/documents?${s}`),i=o.total??0,n=o.meta||{},d=A("docs.storageHint",{dir:n.storageDir||"—",dbMax:qe(n.documentDbMaxBytes),upMax:qe(n.uploadMaxBytes)}),l=[`<option value="">${t(e("common.all"))}</option>`,...c.keys.map(r=>`<option value="${r.id}" ${a.apiKeyId===r.id?"selected":""}>${t(r.name)}</option>`)].join(""),u=(o.data||[]).map(r=>`
  ...
  `),le(),document.getElementById("s-tools").value=a.safeToolsMode||"none";const d=()=>nt();["s-tools","s-turns","s-timeout"].forEach(l=>{const u=document.getElementById(l);u&&(u.addEventListener("change",d),u.addEventListener("input",d))}),nt(),document.getElementById("s-master-global")?.addEventListener("click",async()=>{const l=!Xe("s-master-global");Ye("s-master-global",l,e("settings.masterOn"),e("settings.masterOff")),et("settings-root",!l),Ze("settings-disabled-banner",!l),nt();try{await M("/settings",{method:"PUT",body:JSON.stringify({globalSafeMode:l,safeToolsMode:document.getElementById("s-tools").value,safeMaxTurns:Number(document.getElementById("s-turns").value),safeTimeoutMs:Number(document.getElementById("s-timeout").value),defaultModel:document.getElementById("s-model").value.trim()})})}catch(u){Ye("s-master-global",!l,e("settings.masterOn"),e("settings.masterOff")),et("settings-root",l),Ze("settings-disabled-banner",l),y(u)}}),document.getElementById("btn-refresh-models").onclick=async()=>{await ht(!0),es().catch(y)},document.getElementById("s-save").onclick=async()=>{try{await M("/settings",{method:"PUT",body:JSON.stringify({globalSafeMode:Xe("s-master-global"),safeToolsMode:document.getElementById("s-tools").value,safeMaxTurns:Number(document.getElementById("s-turns").value),safeTimeoutMs:Number(document.getElementById("s-timeout").value),defaultModel:document.getElementById("s-model").value.trim()})}),nt();const l=document.querySelector("#flash-error");l&&(l.hidden=!1,l.classList.add("flash-ok"),l.textContent=e("settings.saved"),setTimeout(()=>{l.hidden=!0,l.classList.remove("flash-ok"),l.textContent=""},2e3))}catch(l){y(l)}},document.querySelectorAll("[data-apply-preset]").forEach(l=>{l.addEventListener("click",async()=>{if(l.disabled)return;const u=l.getAttribute("data-apply-preset"),m=ea().find(p=>p.id===u);m&&await _s(m)})}),document.getElementById("s-disable-panel").onclick=async()=>{if(await Z({message:e("settings.disablePanelConfirm"),variant:"danger",confirmText:e("settings.disablePanel")}))try{await M("/settings",{method:"PUT",body:JSON.stringify({adminPanelEnabled:!1})}),await pe({message:e("settings.disablePanelDone"),title:e("common.notice")}),Bt(!1)}catch(l){y(l)}}}async function qt(){const a=await M("/api-features");if(c.page!=="apiFeatures")return;const s=a.data||{},o=[{id:"protocols",title:e("apiFeatures.groupProtocols"),tabLabel:e("apiFeatures.tabProtocols"),keys:["openaiChat","openaiResponses","anthropicMessages"]},{id:"media",title:e("apiFeatures.groupMedia"),tabLabel:e("apiFeatures.tabMedia"),keys:["imagesApi","filesOpenAiAlias","videoApi","audioApi"]},{id:"caps",title:e("apiFeatures.groupCaps"),tabLabel:e("apiFeatures.tabCaps"),keys:["tools","structuredOutput","vision","reasoningEffort","webSearch","subagents","planMode","memory","sessionResume","bestOfN","checkLoop","systemOverride","rules","permissionMode","sandbox"]},{id:"emu",title:e("apiFeatures.groupEmu"),tabLabel:e("apiFeatures.tabEmu"),keys:["usageEstimate","assistantsEmulation","strictSampling","forceDisableToolsInSafe"]}],i=c.apiFeaturesTab==="media"||c.apiFeaturesTab==="caps"||c.apiFeaturesTab==="emu"||c.apiFeaturesTab==="protocols"?c.apiFeaturesTab:"protocols";c.apiFeaturesTab=i;const n=f=>e(`apiFeatures.flag.${f}`)||f,d=f=>e(`apiFeatures.hint.${f}`)||"",l=f=>f.filter(h=>!!s[h]).length,u=f=>f.map(h=>{const $=!!s[h];return`
    ...
    </div>`,S=o.map(f=>{const h=l(f.keys);return`
  ...
  `),le(),document.querySelectorAll("[data-media-tab]").forEach(g=>{g.addEventListener("click",()=>{const v=g.getAttribute("data-media-tab")||"studio",N=v==="assets"||v==="jobs"||v==="studio"?v:"studio";c.mediaFilter.tab!==N&&(c.mediaFilter.tab=N,Ie().catch(y))})});async function P(g){const v=await fetch(`/admin/api/media/assets/${g}/download`,{headers:{Authorization:`Bearer ${c.key}`}});if(!v.ok)throw new Error(await v.text());return v.blob()}{let g="generate",v=null;const N=()=>{const I=document.getElementById("mg-source-chip"),T=document.getElementById("mg-clear-source");if(!I)return;if(!v){I.hidden=!0,I.innerHTML="",T&&(T.hidden=!0);return}const Y=v.kind==="file"?e("media.sourceKindUpload"):v.kind==="asset"?e("media.sourceKindAsset"):e("media.sourceKindDocument");I.hidden=!1,I.innerHTML=`<span class="chip">${t(Y)}</span> <span class="mono">${t(v.name||v.id||"")}</span>`,T&&(T.hidden=!1)},G=I=>{v=I;const T=document.getElementById("mg-file");T&&I?.kind!=="file"&&(T.value=""),N()},E=I=>{g=I==="edit"||I==="video"?I:"generate",document.querySelectorAll("[data-mg-mode]").forEach(we=>{const Pe=we.getAttribute("data-mg-mode")===g;we.classList.toggle("is-active",Pe),we.setAttribute("aria-selected",Pe?"true":"false")});const T=document.getElementById("mg-source-section"),Y=document.getElementById("mg-n-wrap"),V=document.getElementById("mg-duration-wrap"),Qe=document.getElementById("mg-voice-wrap"),Se=document.getElementById("mg-submit");T&&(T.hidden=g==="generate"),Y&&(Y.hidden=g==="video"),V&&(V.hidden=g!=="video"),Qe&&(Qe.hidden=g!=="video"),Se&&(Se.textContent=e(g==="edit"?"media.editSubmit":g==="video"?"media.videoSubmit":"media.generateSubmit"));const de=document.getElementById("mg-prompt");de&&(de.placeholder=e(g==="edit"?"media.editPromptPh":g==="video"?"media.videoPromptPh":"media.generatePromptPh"));const De=document.getElementById("mg-drop-title"),fe=document.getElementById("mg-drop-hint");De&&(De.textContent=e(g==="video"?"media.dropTitleVideo":"media.dropTitle")),fe&&(fe.textContent=e(g==="video"?"media.dropHintVideo":"media.dropHint"))};document.querySelectorAll("[data-mg-mode]").forEach(I=>{I.addEventListener("click",()=>E(I.getAttribute("data-mg-mode")||"generate"))}),E("generate");const H=document.getElementById("mg-dropzone"),K=document.getElementById("mg-file"),se=I=>I?I.type&&I.type.startsWith("image/")?!0:/\.(png|jpe?g|webp|gif|bmp|svg)$/i.test(I.name||""):!1,oe=I=>{const T=[...I||[]].find(se);if(!T){U(e("media.sourceNeedImage"));return}G({kind:"file",file:T,name:T.name,mime:T.type||"image/*"}),U("")};if(document.getElementById("mg-pick-file")?.addEventListener("click",I=>{I.preventDefault(),I.stopPropagation(),K?.click()}),K?.addEventListener("change",()=>{K.files?.length&&oe(K.files)}),document.getElementById("mg-clear-source")?.addEventListener("click",I=>{I.preventDefault(),I.stopPropagation(),G(null)}),document.getElementById("mg-pick-lib")?.addEventListener("click",I=>{I.preventDefault(),I.stopPropagation(),Rs({imagesOnly:!0,onPick:T=>{G({kind:T.kind,id:T.id,name:T.name,mime:T.mime}),U("")}}).catch(T=>U(T.message||e("media.libraryLoadFail")))}),H&&(H.addEventListener("click",I=>{I.target.closest("button")||K?.click()}),H.addEventListener("keydown",I=>{(I.key==="Enter"||I.key===" ")&&(I.preventDefault(),K?.click())}),["dragenter","dragover"].forEach(I=>{H.addEventListener(I,T=>{T.preventDefault(),T.stopPropagation(),H.classList.add("is-dragover")})}),["dragleave","drop"].forEach(I=>{H.addEventListener(I,T=>{T.preventDefault(),T.stopPropagation(),H.classList.remove("is-dragover")})}),H.addEventListener("drop",I=>{const T=I.dataTransfer;T?.files?.length&&oe(T.files)})),c._mediaDragAbort)try{c._mediaDragAbort.abort()}catch{}c._mediaDragAbort=new AbortController;const Q={signal:c._mediaDragAbort.signal},X=document.getElementById("app");let _=0;window.addEventListener("dragenter",I=>{g!=="generate"&&[...I.dataTransfer?.types||[]].includes("Files")&&(_+=1,X?.classList.add("is-media-file-drag"))},Q),window.addEventListener("dragleave",()=>{_=Math.max(0,_-1),_===0&&X?.classList.remove("is-media-file-drag")},Q),window.addEventListener("drop",I=>{_=0,X?.classList.remove("is-media-file-drag"),g!=="generate"&&I.dataTransfer?.files?.length&&(I.preventDefault(),oe(I.dataTransfer.files))},Q),window.addEventListener("dragover",I=>{g!=="generate"&&[...I.dataTransfer?.types||[]].includes("Files")&&I.preventDefault()},Q),document.getElementById("mg-submit")?.addEventListener("click",async()=>{const I=document.getElementById("mg-prompt")?.value?.trim()||"";if(!I){U(e("media.generateNeedPrompt"));return}const T=document.getElementById("mg-key")?.value||"",Y=document.getElementById("mg-model")?.value||void 0,V=document.getElementById("mg-aspect")?.value||"1:1",Qe=Math.min(4,Math.max(1,Number(document.getElementById("mg-n")?.value)||1)),Se=document.getElementById("mg-submit"),de=document.getElementById("mg-status"),De=e(g==="video"?"media.videoBusy":g==="edit"?"media.editBusy":"media.generateBusy");Se&&(Se.disabled=!0,Se.textContent=De),de&&(de.hidden=!1,de.textContent=De),U("");try{if(g==="edit"){if(!v)throw new Error(e("media.editNeedImage"));const J=new FormData;J.append("prompt",I),J.append("aspect_ratio",V),J.append("n",String(Qe)),J.append("response_format","url"),Y&&J.append("model",Y),T&&J.append("apiKeyId",T),v.kind==="file"&&v.file?J.append("image",v.file):v.kind==="asset"&&v.id?J.append("sourceAssetId",v.id):v.kind==="document"&&v.id&&J.append("sourceDocumentId",v.id),await Sa(await fetch("/admin/api/media/edit",{method:"POST",headers:{Authorization:`Bearer ${c.key}`},body:J})),de&&(de.textContent=e("media.editOk")),c.mediaFilter.tab="assets",c.mediaFilter.offset=0,await Ie();return}if(g==="video"){const J=new FormData;J.append("prompt",I),J.append("aspect_ratio",V),J.append("seconds",String(document.getElementById("mg-duration")?.value||6)),Y&&J.append("model",Y),T&&J.append("apiKeyId",T);const ka=document.getElementById("mg-voice")?.value||"";ka&&J.append("voices",ka),v?.kind==="file"&&v.file?J.append("image",v.file):v?.kind==="asset"&&v.id?J.append("source_asset_id",v.id):v?.kind==="document"&&v.id&&J.append("source_document_id",v.id),await Sa(await fetch("/admin/api/media/videos",{method:"POST",headers:{Authorization:`Bearer ${c.key}`},body:J})),de&&(de.textContent=e("media.videoOk")),c.mediaFilter.tab="jobs",await Ie();return}const fe={prompt:I,aspect_ratio:V,n:Qe,response_format:"url"};Y&&(fe.model=Y),T&&(fe.apiKeyId=T);const Pe=(await M("/media/generate",{method:"POST",body:JSON.stringify(fe)}))?.data?.grok?.asset_ids||[];if(de&&(de.textContent=e("media.generateOk")),c.mediaFilter.tab="assets",c.mediaFilter.offset=0,await Ie(),Pe[0])try{const J=await P(Pe[0]);wa({id:Pe[0],mime:J.type||"image/png",filename:`generated-${String(Pe[0]).slice(0,8)}`,kind:"image",bytes:J.size,prompt:I},J)}catch{}}catch(fe){y(fe),de&&(de.textContent=fe.message||e("media.generateFail")),Se&&(Se.disabled=!1,E(g))}})}(s==="assets"||s==="jobs")&&Ue(c.mediaFilter,()=>Ie().catch(y)),s==="assets"&&(dt("media",c.mediaFilter,()=>Ie().catch(y)),document.querySelector("#media-tab-assets [data-filter-apply]")?.addEventListener("click",()=>{c.mediaFilter.q=document.getElementById("mf-q")?.value.trim()||"",c.mediaFilter.kind=document.getElementById("mf-kind")?.value||"",c.mediaFilter.provider=document.getElementById("mf-provider")?.value.trim()||"",c.mediaFilter.from=document.getElementById("mf-from")?.value||"",c.mediaFilter.to=document.getElementById("mf-to")?.value||"",c.mediaFilter.offset=0,Ie().catch(y)}),document.querySelector("#media-tab-assets [data-filter-reset]")?.addEventListener("click",()=>{const g=c.mediaFilter.tab;c.mediaFilter={tab:g,q:"",kind:"",provider:"",from:"",to:"",sortBy:"createdAt",sortDir:"desc",jobSortBy:"createdAt",jobSortDir:"desc",limit:20,offset:0},Ie().catch(y)}),document.querySelectorAll("[data-media-preview]").forEach(g=>{g.addEventListener("click",async()=>{try{const v=g.getAttribute("data-media-preview");if(!v)return;const N=g.getAttribute("data-media-mime")||"",G=g.getAttribute("data-media-name")||"",E=g.getAttribute("data-media-kind")||"",H=g.getAttribute("data-media-bytes")||"",K=g.getAttribute("data-media-prompt")||"",se=await P(v);wa({id:v,mime:N||se.type||"",filename:G,kind:E,bytes:H?Number(H):se.size,prompt:K},se)}catch(v){y(v)}})}),document.querySelectorAll("[data-media-dl]").forEach(g=>{g.addEventListener("click",async()=>{try{const v=g.getAttribute("data-media-dl"),N=g.getAttribute("data-media-name")||"",G=await P(v),E=document.createElement("a");E.href=URL.createObjectURL(G),E.download=N||`media-${String(v).slice(0,8)}`,E.click(),setTimeout(()=>URL.revokeObjectURL(E.href),3e4)}catch(v){y(v)}})}),document.querySelectorAll("[data-media-del]").forEach(g=>{g.addEventListener("click",async()=>{const v=g.getAttribute("data-media-del");if(await Z({message:e("media.deleteConfirm"),variant:"danger",confirmText:e("media.delete")}))try{await M(`/media/assets/${v}`,{method:"DELETE"}),await Ie()}catch(N){y(N)}})}))}async function Rs(a){const s=a.imagesOnly!==!1;let o="documents",i=0,n=null;rt({title:e("media.libraryTitle"),subtitle:t(e("media.librarySubtitle")),size:"md",bodyHtml:`
      ...
      </tr>`}).join(""),h=Te({total:u,limit:d,offset:a.modelPage*d,idPrefix:"umodel"}),$=Te({total:S,limit:d,offset:a.keyPage*d,idPrefix:"ukey"}),B=a.tab==="key"?"key":"model",C=Le({title:e("usage.byModel"),hint:e("common.filterHint"),searchHtml:`<div class="data-filter-search"><label>${t(e("common.search"))}<input type="search" id="uf-model" value="${t(a.modelQ)}" placeholder="${t(e("chats.model"))}" /></label></div>`,gridHtml:""}),R=be({headHtml:`
  ...
  `),le(),Ue(c.usageFilter,()=>me().catch(y)),document.getElementById("btn-usage-refresh").onclick=()=>me().catch(y),document.querySelectorAll("[data-usage-tab]").forEach(x=>{x.onclick=()=>{const w=x.dataset.usageTab==="key"?"key":"model";c.usageFilter.tab!==w&&(c.usageFilter.tab=w,me().catch(y))}}),document.getElementById("umodel-prev")?.addEventListener("click",()=>{c.usageFilter.modelPage=Math.max(0,a.modelPage-1),me().catch(y)}),document.getElementById("umodel-next")?.addEventListener("click",()=>{(a.modelPage+1)*d<u&&(c.usageFilter.modelPage+=1,me().catch(y))}),document.getElementById("umodel-limit")?.addEventListener("change",x=>{c.usageFilter.pageSize=Number(x.target.value)||10,c.usageFilter.modelPage=0,me().catch(y)}),document.getElementById("ukey-prev")?.addEventListener("click",()=>{c.usageFilter.keyPage=Math.max(0,a.keyPage-1),me().catch(y)}),document.getElementById("ukey-next")?.addEventListener("click",()=>{(a.keyPage+1)*d<S&&(c.usageFilter.keyPage+=1,me().catch(y))}),document.getElementById("ukey-limit")?.addEventListener("change",x=>{c.usageFilter.pageSize=Number(x.target.value)||10,c.usageFilter.keyPage=0,me().catch(y)}),document.querySelectorAll("#usage-tab-model [data-filter-apply]").forEach(x=>{x.onclick=()=>{c.usageFilter.modelQ=document.getElementById("uf-model")?.value?.trim()||"",c.usageFilter.modelPage=0,me().catch(y)}}),document.querySelectorAll("#usage-tab-model [data-filter-reset]").forEach(x=>{x.onclick=()=>{c.usageFilter.modelQ="",c.usageFilter.modelPage=0,me().catch(y)}}),document.querySelectorAll("#usage-tab-key [data-filter-apply]").forEach(x=>{x.onclick=()=>{c.usageFilter.keyQ=document.getElementById("uf-key")?.value?.trim()||"",c.usageFilter.keyActive=document.getElementById("uf-active")?.value||"",c.usageFilter.keyPage=0,me().catch(y)}}),document.querySelectorAll("#usage-tab-key [data-filter-reset]").forEach(x=>{x.onclick=()=>{c.usageFilter.keyQ="",c.usageFilter.keyActive="",c.usageFilter.keyPage=0,me().catch(y)}})}function Ia(a){const s=a.versionStatus||(a.updateAvailable?"update_available":a.latest?"up_to_date":"unknown");return s==="update_available"?{badge:`<span class="badge warn" title="${t(e("system.statusHintUpdate"))}">${t(e("system.badgeUpdate"))}</span>`,hint:e("system.statusHintUpdate")}:s==="ahead"?{badge:`<span class="badge pending" title="${t(e("system.statusHintAhead"))}">${t(e("system.badgeAhead"))}</span>`,hint:e("system.statusHintAhead")}:s==="up_to_date"?{badge:`<span class="badge success" title="${t(e("system.statusHintOk"))}">${t(e("system.badgeOk"))}</span>`,hint:e("system.statusHintOk")}:{badge:`<span class="badge pending" title="${t(e("system.statusHintUnknown"))}">${t(e("system.badgeUnknown"))}</span>`,hint:e("system.statusHintUnknown")}}function Fs(a){return e(a==="git"?"system.channelGit":a==="npm-global"?"system.channelNpmGlobal":a==="npm-local"?"system.channelNpmLocal":"system.channelUnknown")}function Ns(a){return a==="required"?e("system.levelRequired"):a==="recommended"?e("system.levelRecommended"):a==="optional"?e("system.levelOptional"):a==="bundled"?e("system.levelBundled"):a||"—"}function Ks(a){return a.installed?a.ok?`<span class="badge success">${t(e("system.softOk"))}</span>`:`<span class="badge warn">${t(e("system.softWarn"))}</span>`:a.level==="required"||a.level==="bundled"?`<span class="badge error">${t(e("system.softMissing"))}</span>`:`<span class="badge pending">${t(e("system.softMissing"))}</span>`}function Ma(a){return a==="up"?`<span class="badge success">${t(e("system.up"))}</span>`:`<span class="badge error">${t(e("system.down"))}</span>`}async function ot(){const{data:a}=await M("/system");if(c.page!=="system")return;const s=a.version||{},o=Ia(s),i=a.software||{checks:[],allRequiredOk:!0},n=i.checks||[],d=c.systemTab==="package"||c.systemTab==="env"||c.systemTab==="sessions"?c.systemTab:"software";c.systemTab=d;let l={data:[],total:0};try{const w=new URLSearchParams({limit:d==="sessions"?"50":"1",offset:"0"});d==="sessions"&&c.grokSessionQ&&w.set("q",c.grokSessionQ);const k=await M(`/grok/sessions?${w}`);l={data:k.data||[],total:k.total||0}}catch(w){l={data:[],total:0,error:w.message||String(w)}}const u=n.map(w=>`
      <tr>
      ...
      </tr>`).join(""),m=i.allRequiredOk?`<span class="badge success">${t(e("system.allRequiredOk"))}</span>`:`<span class="badge error">${t(e("system.requiredMissing"))}</span>`,p=a.encryption&&a.encryption.ready,r=Fs(s.channel),S=s.installSource?`${r} · ${s.installSource}`:r,b=be({headHtml:`
...
`))}</textarea>
  ...
  `),le(),qa(!0,k),Ue(c.ddosFilter,()=>te().catch(y)),document.querySelectorAll("[data-ddos-tab]").forEach(fe=>{fe.addEventListener("click",()=>{const we=fe.getAttribute("data-ddos-tab")||"policy",Pe=we==="live"||we==="blacklist"||we==="events"||we==="policy"?we:"policy";c.ddosFilter.tab!==Pe&&(c.ddosFilter.tab=Pe,te().catch(y))})}),document.getElementById("ddos-live-filter-apply")?.addEventListener("click",()=>{c.ddosFilter.liveQ=document.getElementById("ddos-live-q")?.value?.trim()||"",c.ddosFilter.livePage=0,te().catch(y)}),document.getElementById("ddos-live-filter-reset")?.addEventListener("click",()=>{c.ddosFilter.liveQ="",c.ddosFilter.liveSortBy="startedAt",c.ddosFilter.liveSortDir="desc",c.ddosFilter.livePage=0,te().catch(y)}),document.getElementById("ddos-ban-filter-apply")?.addEventListener("click",()=>{c.ddosFilter.banQ=document.getElementById("ddos-ban-q")?.value?.trim()||"",c.ddosFilter.banSource=document.getElementById("ddos-ban-source")?.value||"",c.ddosFilter.banPage=0,te().catch(y)}),document.getElementById("ddos-ban-filter-reset")?.addEventListener("click",()=>{c.ddosFilter.banQ="",c.ddosFilter.banSource="",c.ddosFilter.banSortBy="createdAt",c.ddosFilter.banSortDir="desc",c.ddosFilter.banPage=0,te().catch(y)}),document.getElementById("ddoslive-prev")?.addEventListener("click",()=>{c.ddosFilter.livePage=Math.max(0,n.livePage-1),te().catch(y)}),document.getElementById("ddoslive-next")?.addEventListener("click",()=>{(n.livePage+1)*$<B.length&&(c.ddosFilter.livePage+=1,te().catch(y))}),document.getElementById("ddosban-prev")?.addEventListener("click",()=>{c.ddosFilter.banPage=Math.max(0,n.banPage-1),te().catch(y)}),document.getElementById("ddosban-next")?.addEventListener("click",()=>{(n.banPage+1)*$<R.length&&(c.ddosFilter.banPage+=1,te().catch(y))});const De=document.querySelector(".main");De&&(De.onscroll=()=>{c._ddosScrollPauseUntil=Date.now()+4e3})}!ze&&c.page==="ddos"&&(We=setInterval(()=>{c.page!=="ddos"||ze||c._ddosScrollPauseUntil&&Date.now()<c._ddosScrollPauseUntil||te({soft:!0}).catch(()=>{})},2e3))}function qa(a=!1,s=[]){const o=s.length?s:c._ddosPolicyCache?.whitelist||[],i=async n=>{if(!n)return;const d=o.some(l=>String(l)===n||String(l).startsWith(n));await Z({message:e(d?"ddos.banWhitelistWarn":"ddos.banConfirm"),variant:"danger",confirmText:e("ddos.ban")})&&(await M("/ddos/blacklist",{method:"POST",body:JSON.stringify({ip:n,reason:e("ddos.banReasonDefault"),ttlSeconds:null})}),te({soft:!0}).catch(y))};if(document.querySelectorAll("[data-ban]").forEach(n=>{n.onclick=()=>i(n.dataset.ban)}),document.querySelectorAll("[data-unban]").forEach(n=>{n.onclick=async()=>{await Z({message:e("ddos.unbanConfirm"),variant:"danger",confirmText:e("ddos.unban")})&&(await M(`/ddos/blacklist/${encodeURIComponent(n.dataset.unban)}`,{method:"DELETE"}),te({soft:!0}).catch(y))}}),a){document.getElementById("ban-add").onclick=async()=>{const d=document.getElementById("ban-ip").value.trim();if(!d||o.some(m=>String(m)===d)&&!await Z({message:e("ddos.banWhitelistWarn"),variant:"danger",confirmText:e("ddos.ban")}))return;const u=document.getElementById("ban-ttl").value;await M("/ddos/blacklist",{method:"POST",body:JSON.stringify({ip:d,reason:document.getElementById("ban-reason").value.trim()||void 0,ttlSeconds:u?Number(u):null})}),te({soft:!0}).catch(y)},document.getElementById("ddos-refresh").onclick=()=>te({soft:!1}).catch(y),document.getElementById("ddos-pause").onclick=()=>{ze=!ze;const d=document.getElementById("ddos-pause");d&&(d.textContent=e(ze?"ddos.resume":"ddos.pause")),ze||te({soft:!0}).catch(y)},document.getElementById("ddos-master-autoban")?.addEventListener("click",async()=>{const d=!Xe("ddos-master-autoban");Ye("ddos-master-autoban",d,e("ddos.masterOn"),e("ddos.masterOff"));const l=document.getElementById("dp-autoBanEnabled");l&&(l.type==="checkbox"?l.checked=d:l.value=d?"1":"0"),ha(d),et("ddos-root",!d),Ze("ddos-disabled-banner",!d),He();try{const u=aa(),m=await M("/ddos/policy",{method:"PUT",body:JSON.stringify(u)});c._ddosPolicyCache=m.data,He()}catch(u){Ye("ddos-master-autoban",!d,e("ddos.masterOn"),e("ddos.masterOff")),et("ddos-root",d),Ze("ddos-disabled-banner",d),y(u)}});const n=document.getElementById("ddos-policy-panel");n?.addEventListener("input",()=>He()),n?.addEventListener("change",()=>He()),document.querySelectorAll("[data-ddos-preset]").forEach(d=>{d.onclick=()=>{const l=d.dataset.ddosPreset;if(l==="custom")return;const u=c._ddosPresetsCache?.[l];u&&Qt(u)}}),He(),document.getElementById("dp-save")?.addEventListener("click",async()=>{try{const d=aa(),l=await M("/ddos/policy",{method:"PUT",body:JSON.stringify(d)});c._ddosPolicyCache=l.data,Qt(l.data),He(),await pe({title:e("ddos.policyTitle"),message:e("ddos.policySaved")}),te({soft:!0}).catch(y)}catch(d){y(d)}}),document.getElementById("dp-reset")?.addEventListener("click",async()=>{if(await Z({message:e("ddos.confirmReset"),variant:"danger",confirmText:e("ddos.resetPolicy")}))try{const d=await M("/ddos/policy/reset",{method:"POST"});c._ddosPolicyCache=d.data,Qt(d.data),He(),await pe({title:e("ddos.policyTitle"),message:e("ddos.policyReset")}),te({soft:!0}).catch(y)}catch(d){y(d)}})}}function Et(a){return e(a==="pm2"?"pm2.runnerPm2":a==="ysk-omni"?"pm2.runnerGctoac":a==="none"?"pm2.runnerNone":"pm2.runnerUnknown")}function oa(a){if(!a)return"";const s=a.messageKey;if(s&&typeof s=="string"){if(s==="pm2.msgOk")return"";const i=a.messageParams||{},n=A(s,i);if(n&&n!==s)return n}const o=a.message||"";return!o||o==="ok"?"":o}function Ta(a=10){const s=Math.max(1,Number(a)||10)*1e3;window.setTimeout(()=>{try{window.location.reload()}catch{window.location.href=window.location.href}},s)}function Qs(a,s){const o=s?.messageKey||(a==="pm2"?"pm2.msgSwitchPm2":"pm2.msgSwitchGctoac");let i=oa({messageKey:o,messageParams:s?.messageParams,message:void 0});i||(i=e(a==="pm2"?"pm2.msgSwitchPm2":"pm2.msgSwitchGctoac"));const n=s?.port||s?.messageParams?.port||(typeof location<"u"&&location.port?location.port:"3850");return[i,A("pm2.portAfterRestart",{port:n}),A("pm2.autoRefreshIn",{n:10})].filter(Boolean).join(`
...
${u}`);const p=m?.files||[],r=p.length?p.filter(E=>E.exists).map(E=>`${E.label}: ${E.size<1024?E.size+" B":Math.round(E.size/1024)+" KB"}`).join(" · "):"",S=m?.maxBytes?Math.round(m.maxBytes/(1024*1024)):5,b=m?.keepBytes?Math.round(m.keepBytes/1024):512,f=o?.status||"—",h=f==="online"?e("pm2.statusOnline"):f==="errored"?e("pm2.statusErrored"):f==="stopped"?e("pm2.statusStopped"):f,$=f==="online"?`<span class="badge success">${t(h)}</span>`:f==="errored"?`<span class="badge error">${t(h)}</span>`:t(h),B=s.available,C=s.available&&o,R=s.runner||"unknown",q=l&&f!=="errored"&&s.available!==!1&&s.messageKey!=="pm2.msgErrored",j=Ws(i.env_extra),x=c.pm2Tab==="port"||c.pm2Tab==="config"||c.pm2Tab==="logs"||c.pm2Tab==="runner"?c.pm2Tab:"runner";c.pm2Tab=x;const w=`
    ...
    <div class="panel data-table-panel pm2-section-panel pm2-logs-panel">
      ...
      <div class="panel-pad">
        <p class="muted" style="margin:0 0 8px;font-size:0.82rem">
          ${t(A("pm2.logsAutoTrim",{maxMb:S,keepKb:b}))}
          ${r?` · ${t(r)}`:""}
        </p>
  ...
  `),le(),document.querySelectorAll("[data-pm2-tab]").forEach(E=>{E.addEventListener("click",()=>{const H=E.getAttribute("data-pm2-tab")||"runner",K=H==="port"||H==="config"||H==="logs"||H==="runner"?H:"runner";c.pm2Tab!==K&&(c.pm2Tab=K,Ae().catch(y))})}),document.getElementById("pm2-logs-refresh")?.addEventListener("click",()=>{c.pm2Tab="logs",Ae().catch(y)}),document.getElementById("pm2-logs-clear")?.addEventListener("click",async()=>{if(await Z({message:e("pm2.confirmClearLogs"),variant:"danger",confirmText:e("pm2.clearLogs")}))try{const H=(await M("/pm2/logs/clear",{method:"POST",body:JSON.stringify({which:"all"})})).data?.cleared?.length||0;await pe({message:A("pm2.logsCleared",{n:H})}),Ae().catch(y)}catch(E){y(E)}});const N=async E=>{if(await Z({message:e(E==="pm2"?"pm2.confirmSwitchPm2":"pm2.confirmSwitchGctoac"),variant:"confirm",confirmText:e(E==="pm2"?"pm2.switchToPm2":"pm2.switchToGctoac")}))try{const K=await M("/pm2/switch",{method:"POST",body:JSON.stringify({mode:E})}),se=K?.data||K||{},oe=Qs(E==="pm2"?"pm2":"ysk-omni",se);Ta(10),await pe({title:e("common.notice"),message:oe,confirmText:e("common.ok")});try{window.location.reload()}catch{window.location.href=window.location.href}}catch(K){y(K)}};document.getElementById("pm2-refresh").onclick=()=>Ae().catch(y),document.getElementById("pm2-switch-pm2").onclick=()=>N("pm2"),document.getElementById("pm2-switch-ysk-omni").onclick=()=>N("ysk-omni"),document.getElementById("pm2-start").onclick=()=>N("pm2"),document.getElementById("pm2-stop").onclick=async()=>{if(await Z({message:e("pm2.confirmStop"),variant:"danger",confirmText:e("pm2.stop")}))try{await M("/pm2/stop",{method:"POST",body:"{}"}),Ae().catch(y)}catch(E){y(E)}},document.getElementById("pm2-restart").onclick=async()=>{if(await Z({message:e("pm2.confirmRestart"),variant:"confirm",confirmText:e("pm2.restart")}))try{await N("pm2")}catch(E){y(E)}},document.getElementById("pm2-reload").onclick=async()=>{try{await M("/pm2/reload",{method:"POST",body:"{}"}),Ae().catch(y)}catch(E){y(E)}};const G=async E=>{try{const H={...Vs(),restart:E};if(H.port==null){await pe({message:e("pm2.portInvalid")});return}const K=await M("/pm2/config",{method:"PUT",body:JSON.stringify(H)});if(K.data?.scheduled){const se=K.data.portChange?`
...
`)+n}]},m=St();m&&(u.apiKeyId=m);const p=await fetch("/admin/api/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${s}`,"Content-Type":"application/json"},body:JSON.stringify(u)});if(!p.ok){const b=await p.text();let f=b;try{f=JSON.parse(b).error?.message||b}catch{}throw new Error(f||e("chat.compressFail"))}const r=await p.json();let S=r?.choices?.[0]?.message?.content||r?.choices?.[0]?.delta?.content||"";if(typeof S!="string"&&(S=String(S||"")),S=S.trim().replace(/^【對話摘要】\s*/u,"").replace(/^\[Conversation summary\]\s*/i,""),!S)throw new Error(e("chat.compressFail"));O.summary=S,O.summaryAt=new Date().toISOString(),O.summarySourceCount=a.length,O.mode="summary",Ke(),tt(),U(""),i&&(i.hidden=!1,i.textContent=e("chat.compressOk"),setTimeout(()=>{const b=document.getElementById("chat-stream-status");b&&b.textContent===e("chat.compressOk")&&(b.hidden=!0,b.textContent="")},2800)),await vt().catch(()=>{}),await ls()}catch(n){U(n.message||e("chat.compressFail"))}finally{ft=!1,lt(),o&&(o.disabled=!1),i&&i.textContent===e("chat.compressing")&&(i.hidden=!0,i.textContent="")}}function na(a){D.historyOpenMobile=!!a,document.body.classList.toggle("chat-history-open",D.historyOpenMobile)}function ia(){na(!1)}async function je(){if(c.key){D.historyLoading=!0,Ce();try{const a=D.historyPage*D.historyLimit,s=new URLSearchParams({limit:String(D.historyLimit),offset:String(a)});D.historyQ.trim()&&s.set("q",D.historyQ.trim());const o=await M(`/conversations?${s}`);D.historyItems=o.data||[],D.historyTotal=o.total??0}catch(a){D.historyItems=[],D.historyTotal=0,console.warn(a)}finally{D.historyLoading=!1,Ce()}}}function Ce(){const a=document.getElementById("chat-history-list"),s=document.getElementById("chat-history-pager");if(a){if(D.historyLoading&&!D.historyItems.length?a.innerHTML=`<li class="chat-history-empty">${t(e("common.loading"))}</li>`:D.historyItems.length?a.innerHTML=D.historyItems.map(o=>{const i=D.conversationId===o.id?" is-active":"",n=eo(o),d=o.title&&o.preview&&o.preview!==o.title?o.preview:o.model||A("chat.msgs",{n:o.messageCount||0}),l=D.renamingId===o.id,u=n,m=l?`<input type="text" class="chat-history-title-input" data-title-input="${t(o.id)}" value="${t(u)}" maxlength="120" placeholder="${t(e("chat.renamePh"))}" aria-label="${t(e("chat.renamePh"))}" />
    ...
    `;const m=document.getElementById("chat-hist-prev"),p=document.getElementById("chat-hist-next");m&&(m.onclick=()=>{D.historyPage>0&&(D.historyPage-=1,je())}),p&&(p.onclick=()=>{(D.historyPage+1)*o<D.historyTotal&&(D.historyPage+=1,je())})}if(a.querySelectorAll("[data-open-conv]").forEach(o=>{const i=o.getAttribute("data-open-conv");if(!i)return;let n=null;const d=()=>{n&&(clearTimeout(n),n=null)};o.addEventListener("click",l=>{D.renamingId||l.target instanceof Element&&l.target.closest(".chat-history-item-actions")||(d(),n=setTimeout(()=>{n=null,!D.renamingId&&Aa(i)},280))}),o.addEventListener("dblclick",l=>{l.preventDefault(),l.stopPropagation(),d(),Vt(i)}),o.addEventListener("keydown",l=>{(l.key==="Enter"||l.key===" ")&&(l.preventDefault(),D.renamingId||Aa(i))})}),a.querySelectorAll("[data-title-label]").forEach(o=>{o.addEventListener("dblclick",i=>{i.preventDefault(),i.stopPropagation();const n=o.getAttribute("data-title-label");n&&Vt(n)})}),a.querySelectorAll("[data-rename-conv]").forEach(o=>{o.addEventListener("click",i=>{i.preventDefault(),i.stopPropagation();const n=o.getAttribute("data-rename-conv");n&&Vt(n)})}),a.querySelectorAll("[data-del-conv]").forEach(o=>{o.addEventListener("click",i=>{i.preventDefault(),i.stopPropagation();const n=o.getAttribute("data-del-conv");n&&uo(n)})}),D.renamingId){const o=String(D.renamingId).replace(/\\/g,"\\\\").replace(/"/g,'\\"'),i=a.querySelector(`[data-title-input="${o}"]`);i instanceof HTMLInputElement&&(co(i,D.renamingId),requestAnimationFrame(()=>{i.isConnected&&(i.focus(),i.select())}))}}}function Vt(a){a&&(D.renamingId&&D.renamingId!==a&&(D.renamingId=null),D.renamingId=a,Ce())}function co(a,s){let o=!1;const i=async n=>{if(o)return;o=!0;const d=a.value;if(D.renamingId=null,!n){Ce();return}const l=String(d??"").trim().slice(0,120),u=D.historyItems.find(p=>p.id===s),m=u?(u.title||"").trim():"";if(l===m){Ce();return}u&&(u.title=l),Ce();try{await M(`/conversations/${s}`,{method:"PATCH",body:JSON.stringify({title:l})}),await je()}catch(p){U(p.message||e("chat.saveFail")),await je()}};a.addEventListener("keydown",n=>{n.stopPropagation(),n.key==="Enter"?(n.preventDefault(),i(!0)):n.key==="Escape"&&(n.preventDefault(),i(!1))}),a.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation()}),a.addEventListener("mousedown",n=>n.stopPropagation()),a.addEventListener("dblclick",n=>{n.preventDefault(),n.stopPropagation()}),a.addEventListener("blur",()=>{setTimeout(()=>i(!0),0)})}async function Aa(a){(!a||ye)&&ye&&ye.abort();try{U("");const s=await M(`/conversations/${a}`),o=s.data||s;D.conversationId=o.id,Fe=0,_e.clear(),ee=(o.messages||[]).filter(u=>!u.compressed).map(u=>({role:u.role,content:u.content||"",reasoning:u.reasoning||void 0,docs:u.docs,error:u.error})),ne=[],L.systemPrompt=o.systemPrompt||"",so(o),o.model&&(L.model=o.model),o.apiKeyId&&(L.keyId=o.apiKeyId);const i=document.getElementById("chat-system");i&&(i.value=L.systemPrompt);const n=document.getElementById("chat-system-wrap");n&&(n.hidden=!L.systemPrompt.trim()&&!L.systemOpen);const d=document.getElementById("chat-model");d&&o.model&&(d.value=o.model);const l=document.getElementById("chat-key-select");l&&o.apiKeyId&&[...l.options].some(m=>m.value===o.apiKeyId)&&(l.value=o.apiKeyId,L.keyId=o.apiKeyId),Be(),Ge(),Ce(),Ke(),tt(),ia()}catch(s){U(s.message||e("chat.loadFail"))}}function St(){const a=$a();return!a||String(a).startsWith("admin-session:")||!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(a)?null:a}async function vt(){const a=to();if(!a.length&&!O.summary)return;if(D.saving){D.saveQueued=!0;return}D.saving=!0,D.saveQueued=!1,ue();const s={messages:a,model:L.model||null,systemPrompt:L.systemPrompt||"",apiKeyId:St(),...ao()};try{if(D.conversationId)await M(`/conversations/${D.conversationId}`,{method:"PATCH",body:JSON.stringify(s)});else{if(!a.length)return;const o=await M("/conversations",{method:"POST",body:JSON.stringify({...s,title:""})}),i=o.data||o;D.conversationId=i.id}await je()}catch(o){console.warn(o)}finally{D.saving=!1,D.saveQueued&&(D.saveQueued=!1,vt().catch(()=>{}))}}async function uo(a){if(await Z({title:e("chat.deleteConversation"),message:e("chat.deleteConfirm"),variant:"danger",confirmText:e("chat.deleteConversation")}))try{await M(`/conversations/${a}`,{method:"DELETE"}),D.conversationId===a&&(D.conversationId=null,ee=[],ne=[],os(),Be(),Ge(),Ke(),tt()),D.historyItems.length<=1&&D.historyPage>0&&(D.historyPage-=1),await je()}catch(s){U(s.message||e("common.requestFailed"))}}function mo(a=!0){ye&&ye.abort(),ee=[],ne=[],D.conversationId=null,os(),a||(L.systemPrompt="",L.systemOpen=!1),Be(),Ge(),Ce(),Ke(),tt()}function wt(){return c.key}function $a(){const s=document.getElementById("chat-key-select")?.value||L.keyId||"";return s&&s!=="session"?s:c.me?.id||""}function ue(){const a=document.getElementById("chat-key-select"),s=document.getElementById("chat-model"),o=document.getElementById("chat-reasoning"),i=document.getElementById("chat-system");a&&(L.keyId=a.value==="session"?"":a.value),s&&(L.model=s.value),o&&(L.reasoning=o.checked);const n=document.getElementById("chat-effort");n&&(L.effort=n.value||"");const d=document.getElementById("chat-resume");d&&(L.resumeId=d.value.trim());const l=document.getElementById("chat-fork");l&&(L.forkSession=l.checked);const u=document.getElementById("chat-memory");u&&(L.memory=u.checked);const m=document.getElementById("chat-no-plan");m&&(L.noPlan=m.checked);const p=document.getElementById("chat-perm");p&&(L.permissionMode=p.value||""),i&&(L.systemPrompt=i.value),Ot()}function Ot(){const a=document.querySelector(".chat-shell"),s=document.getElementById("chat-settings-toggle"),o=document.getElementById("chat-settings-summary"),i=document.getElementById("chat-settings-toggle-label");if(a&&a.classList.toggle("is-settings-open",!!L.settingsOpen),s&&s.setAttribute("aria-expanded",L.settingsOpen?"true":"false"),i&&(i.textContent=L.settingsOpen?e("chat.settingsHide"):e("chat.settings")),o){const n=L.model||"—",d=O.mode==="summary"?"chat.ctxModeSummary":O.mode==="recent"?"chat.ctxModeRecent":"chat.ctxModeFull";o.textContent=`${n} · ${e(d)}`}}function Ge(){const a=document.getElementById("chat-pending");if(a){if(!ne.length){a.innerHTML="",a.hidden=!0;return}a.hidden=!1,a.innerHTML=ne.map((s,o)=>`
      ...
      <div class="chat-empty">
        ...
        </div>`:"";a.innerHTML=p+u.map((r,S)=>{const b=l+S,f=r.role==="user"?"user":"assistant",h=r.role==="user"?e("chat.you"):e("chat.assistant"),$=r.docs&&r.docs.length?`<div class="chat-attach-list">${r.docs.map(H=>`<span class="chat-attach-chip" title="${t(H.name)}"><span>📎 ${t(H.name)}</span></span>`).join("")}</div>`:"",C=!!r.reasoning?`<details class="chat-reasoning" ${r.streaming||!r.content?"open":""}>
            <summary>${t(e("chat.reasoning"))}${r.streaming&&!r.content?` · ${t(e("chat.streaming"))}`:""}</summary>
            <pre>${t(r.reasoning)}</pre>
          </details>`:"";let R=r.content||"";!R&&r.streaming&&(R=r.reasoning?"":"…");const q=r.error?" error":"",j=r.streaming?" is-streaming":"",x=f==="assistant"&&!r.streaming&&!!R;let w;if(x){const H=po(b,R);if(_e.has(H))w=_e.get(H);else if(w=Ka(R),_e.set(H,w),_e.size>200){const K=_e.keys().next().value;_e.delete(K)}}else w=t(R);const k=!r.streaming&&R.length>Ys,P=`${x?"chat-content md":"chat-content"}${k?" is-collapsible":""}`,g=k?`<button type="button" class="btn ghost sm chat-expand-btn" data-expand="${b}">${t(e("chat.showMore"))}</button>`:"",v=fo(r),N=v?`<div class="muted chat-spend">${t(v)}</div>`:"",G=Array.isArray(r.tools)&&r.tools.length?`<div class="chat-tools">${r.tools.map(H=>`<span class="chat-tool-chip" title="${t(H.arguments||"")}">${t(H.name||"tool")}</span>`).join("")}</div>`:"",E=R?`<button type="button" class="chat-copy-btn" data-copy-msg="${b}" title="${t(e("chat.copy"))}">${t(e("chat.copy"))}</button>`:"";return`<div class="chat-bubble ${f}${q}${j}" data-msg-idx="${b}">
        ...
        ${C}
        ${G}
        <div class="${P}" data-content-idx="${b}">${w}${r.streaming?'<span class="chat-cursor">▍</span>':""}</div>
        ${g}
        ${N}
      </div>`}).join(""),(s||o)&&(a.scrollTop=a.scrollHeight),lt(),document.getElementById("chat-load-older")?.addEventListener("click",()=>{const r=a.scrollHeight;Fe=Math.max(0,Fe-Xs),Be();const S=document.getElementById("chat-messages");S&&(S.scrollTop=S.scrollHeight-r)}),a.querySelectorAll("[data-expand]").forEach(r=>{r.addEventListener("click",()=>{const S=a.querySelector(`[data-content-idx="${r.getAttribute("data-expand")}"]`);S&&(S.classList.toggle("is-expanded"),r.textContent=S.classList.contains("is-expanded")?e("chat.showLess"):e("chat.showMore"))})}),a.querySelectorAll("[data-copy-msg]").forEach(r=>{r.addEventListener("click",async S=>{S.preventDefault(),S.stopPropagation();const b=Number(r.getAttribute("data-copy-msg")),f=ee[b];if(!f?.content)return;if(await Nt(f.content)){const $=r.textContent;r.textContent=e("chat.copied"),r.classList.add("is-copied"),setTimeout(()=>{r.isConnected&&(r.textContent=$||e("chat.copy"),r.classList.remove("is-copied"))},1600)}else U(e("chat.copyFail"))})})}function Jt(a){const o=a.replace(/\r\n/g,`
`).replace(/\r/g,`
`).split(`
...
✗ ${l}`,!0}const o=s.choices?.[0]?.delta||{};let i=!1;if(o.reasoning_content&&(a.reasoning=(a.reasoning||"")+o.reasoning_content,i=!0),(o.thought&&!o.reasoning_content||o.thought&&o.reasoning_content&&o.thought!==o.reasoning_content)&&(a.reasoning=(a.reasoning||"")+o.thought,i=!0),typeof o.content=="string"&&o.content.length&&(a.content=(a.content||"")+o.content,i=!0),s.usage&&typeof s.usage=="object"&&(a.usage=s.usage,i=!0),s.grok&&typeof s.grok=="object"){if(a.grok={...a.grok||{},...s.grok},a.grok.sessionId){L.resumeId=a.grok.sessionId;const l=document.getElementById("chat-resume");l&&(l.value=a.grok.sessionId)}i=!0}if(s.grok_event&&typeof s.grok_event=="object"){const l=s.grok_event;if(Array.isArray(a.tools)||(a.tools=[]),l.type==="tool_call"||l.type==="tool_call_update"){const u=l.toolCallId,m=u?a.tools.find(b=>b.id===u):null,p=l.toolName||l.title||m?.name||"tool",r=l.rawInput!=null?typeof l.rawInput=="string"?l.rawInput:JSON.stringify(l.rawInput):m?.arguments||"",S=l.status?`${p} (${l.status})`:p;m?(m.name=S,r&&(m.arguments=r)):a.tools.push({id:u,name:S,arguments:r}),i=!0}}const n=s.choices?.[0]?.delta?.tool_calls;if(Array.isArray(n)&&n.length){Array.isArray(a.tools)||(a.tools=[]);for(const l of n){const u=l?.function?.name||l?.name||"tool",m=l?.function?.arguments||"";a.tools.push({id:l?.id,name:u,arguments:typeof m=="string"?m:JSON.stringify(m||{})})}i=!0}const d=s.choices?.[0]?.message;return d&&(d.content&&!a.content&&(a.content=d.content,i=!0),d.reasoning_content&&!a.reasoning&&(a.reasoning=d.reasoning_content,i=!0)),i}function go(a,s){const o=wt();return o?new Promise((i,n)=>{const d=new FormData;d.append("file",a,a.name);const l=St();l&&d.append("apiKeyId",l);const u=new XMLHttpRequest;u.open("POST","/admin/api/documents"),u.setRequestHeader("Authorization",`Bearer ${o}`),u.upload.onprogress=m=>{if(s)if(m.lengthComputable&&m.total>0){const p=Math.min(100,Math.round(m.loaded/m.total*100));s({loaded:m.loaded,total:m.total,percent:p})}else s({loaded:m.loaded||0,total:0,percent:-1})},u.onload=()=>{let m=null;try{m=u.responseText?JSON.parse(u.responseText):null}catch{m=null}if(u.status<200||u.status>=300){const S=m?.error?.message||m?.message||u.responseText||u.statusText;n(new Error(S||e("chat.uploadFail")));return}const p=m?.data||m,r=p?.id;if(!r||typeof r!="string"){n(new Error(e("chat.uploadFail")));return}i({id:r,name:p.originalName||p.filename||a.name,mime:p.mimeType||a.type||"",size:p.sizeBytes??p.size??a.size??0})},u.onerror=()=>n(new Error(e("chat.uploadFail"))),u.onabort=()=>n(new Error(e("chat.uploadFail"))),u.send(d)}):Promise.reject(new Error(e("chat.needKey")))}function Mt(a){const s=document.getElementById("chat-upload-progress");if(!s)return;const{visible:o,fileName:i,fileIndex:n,fileTotal:d,percent:l,indeterminate:u}=a;if(!o){s.hidden=!0,s.setAttribute("aria-hidden","true");return}s.hidden=!1,s.setAttribute("aria-hidden","false");const m=document.getElementById("chat-upload-label"),p=document.getElementById("chat-upload-bar"),r=document.getElementById("chat-upload-pct"),S=i||"",b=n||1,f=d||1;m&&(m.textContent=f>1?A("chat.uploadProgressMulti",{name:S,i:b,n:f}):A("chat.uploadProgress",{name:S}));const h=!!u||l<0;p&&(p.classList.toggle("is-indeterminate",h),h?p.style.width="40%":p.style.width=`${Math.max(0,Math.min(100,l))}%`),r&&(r.textContent=h?e("chat.uploading"):A("common.percent",{n:Math.max(0,Math.min(100,l))}))}function yo(a){const s=Array.isArray(a)?a:[];if(!s.length)return{added:0,skipped:0};let o=0,i=0;const n=new Set(ne.map(d=>d.id));for(const d of s){if(ne.length>=Lt){i+=s.length-o-i;break}const l=d?.id,u=d?.name||d?.originalName||"";if(!l||!va.test(String(l))){i+=1;continue}if(!Dt(u)){i+=1;continue}if(n.has(l)){i+=1;continue}ne.push({id:l,name:u||l,mime:d.mime||d.mimeType||"",size:d.size??d.sizeBytes??0}),n.add(l),o+=1}return Ge(),{added:o,skipped:i}}async function bo(){if(!wt()){U(e("chat.needKey"));return}const a=St(),s=Math.max(0,Lt-ne.length);if(s<=0){U(e("chat.tooManyFiles"));return}const o=new Map;let i=0;rt({title:e("chat.libraryTitle"),subtitle:t(e("chat.librarySubtitle")),size:"md",bodyHtml:`
      ...
      <div class="chat-body">
        ...
        <div class="chat-shell${L.settingsOpen?" is-settings-open":""}">
          ...
          <div class="chat-toolbar" id="chat-toolbar">
            ...
            <label>${t(e("chats.model"))}
              <select id="chat-model">${s||'<option value="echo">echo</option>'}</select>
            </label>
            ...
            <label class="chat-ctx-label">${t(e("chat.effort"))}
              <select id="chat-effort">
                ${["","none","minimal","low","medium","high","xhigh","max"].map(m=>{const p=e(m?`chat.effort_${m}`:"chat.effortDefault"),r=(L.effort||"")===m?" selected":"";return`<option value="${t(m)}"${r}>${t(p)}</option>`}).join("")}
              </select>
            </label>
            ...
            <label class="chat-ctx-label chat-field-full">${t(e("chat.permission"))}
              <select id="chat-perm">
                ${["","default","acceptEdits","auto","dontAsk","bypassPermissions","plan"].map(m=>{const p=m||e("chat.effortDefault"),r=(L.permissionMode||"")===m?" selected":"";return`<option value="${t(m)}"${r}>${t(p)}</option>`}).join("")}
              </select>
            </label>
          </div>
  ...
  `),le(),Be(),Ge(),Ce(),lt(),Ke(),tt(),Ot(),je().catch(()=>{}),document.getElementById("chat-settings-toggle")?.addEventListener("click",()=>{L.settingsOpen=!L.settingsOpen,Ot()}),document.getElementById("chat-key-select").onchange=()=>ue();const o=document.getElementById("chat-ctx-mode"),i=document.getElementById("chat-ctx-n");o&&(o.onchange=()=>{const m=o.value;if(m==="summary"&&!O.summary){U(e("chat.compressNeedSummary")),o.value=O.mode==="recent"?"recent":"full";return}O.mode=m==="summary"||m==="recent"?m:"full",Ke(),tt(),vt().catch(()=>{})}),i&&(i.onchange=()=>{O.recentN=Math.min(40,Math.max(2,Number(i.value)||6)),Ke(),vt().catch(()=>{})}),document.getElementById("chat-model").onchange=()=>ue(),document.getElementById("chat-reasoning").onchange=()=>ue(),document.getElementById("chat-effort")?.addEventListener("change",()=>ue()),document.getElementById("chat-resume")?.addEventListener("change",()=>ue()),document.getElementById("chat-fork")?.addEventListener("change",()=>ue()),document.getElementById("chat-memory")?.addEventListener("change",()=>ue()),document.getElementById("chat-no-plan")?.addEventListener("change",()=>ue()),document.getElementById("chat-perm")?.addEventListener("change",()=>ue()),document.getElementById("chat-system").oninput=()=>ue(),document.getElementById("chat-system-toggle").onclick=()=>{ue(),L.systemOpen=!L.systemOpen;const m=document.getElementById("chat-system-wrap");m&&(m.hidden=!L.systemOpen&&!L.systemPrompt.trim()),L.systemOpen&&document.getElementById("chat-system")?.focus()},document.getElementById("chat-new").onclick=()=>{mo(!0)},document.getElementById("chat-compress").onclick=()=>{ro().catch(()=>{})},document.getElementById("chat-stop").onclick=()=>{ye&&ye.abort()},document.getElementById("chat-send").onclick=()=>Ba(),document.getElementById("chat-attach").onclick=()=>{document.getElementById("chat-file")?.click()},document.getElementById("chat-attach-lib")?.addEventListener("click",()=>{bo().catch(m=>U(m.message||e("chat.libraryLoadFail")))}),document.getElementById("chat-file").onchange=m=>{const p=m.target;ds(p.files).finally(()=>{p.value=""})};const n=document.getElementById("chat-history-toggle"),d=document.getElementById("chat-history-backdrop"),l=document.getElementById("chat-history-close-mobile");n&&(n.onclick=()=>{na(!D.historyOpenMobile)}),d&&(d.onclick=()=>ia()),l&&(l.onclick=()=>ia());const u=document.getElementById("chat-history-search");u&&(u.oninput=()=>{D.historyQ=u.value,zt&&clearTimeout(zt),zt=setTimeout(()=>{D.historyPage=0,je()},280)}),$o(),document.getElementById("chat-input").onkeydown=m=>{m.key==="Enter"&&!m.shiftKey&&(m.preventDefault(),Ba())}}function $o(){const a=document.getElementById("chat-page"),s=document.getElementById("chat-drop-overlay"),o=document.getElementById("chat-composer");if(!a)return;let i=0;const n=b=>{const f=b.dataTransfer?.types;return f?typeof f.includes=="function"?f.includes("Files"):[...f].includes("Files"):!1},d=b=>{a.classList.toggle("is-file-drag",b),o&&o.classList.toggle("is-dragover",b),s&&(s.hidden=!b,s.setAttribute("aria-hidden",b?"false":"true"))},l=b=>{n(b)&&(b.preventDefault(),b.stopPropagation(),i+=1,d(!0))},u=b=>{n(b)&&(b.preventDefault(),b.stopPropagation(),b.dataTransfer&&(b.dataTransfer.dropEffect="copy"),d(!0))},m=b=>{n(b)&&(b.preventDefault(),b.stopPropagation(),i=Math.max(0,i-1),i===0&&d(!1))},p=b=>{if(!n(b))return;b.preventDefault(),b.stopPropagation(),i=0,d(!1);const f=b.dataTransfer?.files;f?.length&&ds(f).catch(h=>U(h.message||e("chat.uploadFail")))};a.addEventListener("dragenter",l),a.addEventListener("dragover",u),a.addEventListener("dragleave",m),a.addEventListener("drop",p);const r=b=>{c.page==="chat"&&n(b)&&b.preventDefault()},S=b=>{c.page==="chat"&&n(b)&&b.preventDefault()};window.addEventListener("dragover",r),window.addEventListener("drop",S),a._chatDropCleanup=()=>{window.removeEventListener("dragover",r),window.removeEventListener("drop",S)}}function ko(a){const s=new Set,o=[],i=n=>{if(!n||typeof n!="string")return;const d=n.trim();!va.test(d)||s.has(d)||(s.add(d),o.push(d))};for(const n of a||[])i(n?.id);for(const n of ee)if(n?.docs?.length)for(const d of n.docs)i(d?.id);return o}async function Ba(){ue();const a=document.getElementById("chat-input");let s=a?.value.trim()||"";const o=[...ne];if(!s&&!o.length){U(e("chat.needContent"));return}const i=wt();if(!i){U(e("chat.needKey"));return}if(!s&&o.length&&(s=e("chat.fileOnlyPrompt")),o.filter(C=>!C?.id||!va.test(String(C.id))).length){U(e("chat.uploadFail"));return}const d=document.getElementById("chat-model")?.value||L.model||"echo",l=document.getElementById("chat-reasoning")?.checked!==!1,u=document.getElementById("chat-effort")?.value||L.effort||"";$a();const m=o.map(C=>({id:C.id,name:C.name})),p=ko(o);ee.push({role:"user",content:s,docs:m.length?m:void 0}),a&&(a.value=""),ne=[],Ge();const r={role:"assistant",content:"",reasoning:"",streaming:!0};ee.push(r),Fe=Math.max(0,ee.length-ss),Be();const b=oo(),f=document.getElementById("chat-send"),h=document.getElementById("chat-stop"),$=document.getElementById("chat-attach"),B=document.getElementById("chat-attach-lib");f&&(f.disabled=!0),$&&($.disabled=!0),B&&(B.disabled=!0),h&&(h.disabled=!1),ye=new AbortController;try{const C={model:d,stream:!0,include_reasoning:l,messages:b};u&&(C.reasoning_effort=u),ue(),L.resumeId&&(C.resume=L.resumeId),L.forkSession&&(C.fork_session=!0),L.memory&&(C.experimental_memory=!0),L.noPlan&&(C.no_plan=!0),L.permissionMode&&(C.permission_mode=L.permissionMode),p.length&&(C.document_ids=p);const R=St();R&&(C.apiKeyId=R);const q=await fetch("/admin/api/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${i}`,"Content-Type":"application/json"},body:JSON.stringify(C),signal:ye.signal});if(!q.ok){const j=await q.text();let x=j;try{x=JSON.parse(j).error?.message||j}catch{}throw new Error(x||q.statusText)}if(q.body&&typeof q.body.getReader=="function"){const j=q.body.getReader(),x=new TextDecoder;let w="",k=0;const P=(g=!1)=>{const v=performance.now();(g||v-k>40)&&(k=v,Be())};for(;;){const{done:g,value:v}=await j.read();if(g)break;w+=x.decode(v,{stream:!0});const{events:N,rest:G}=Jt(w);w=G;let E=!1;for(const H of N)if(H!=="[DONE]")try{const K=JSON.parse(H);It(r,K)&&(E=!0)}catch{}E&&P(!1)}if(w.trim()){const{events:g}=Jt(w+`
`);for(const v of g)if(v!=="[DONE]")try{It(r,JSON.parse(v))}catch{}}P(!0)}else{const j=await q.text(),{events:x}=Jt(j+`
`);for(const w of x)if(w!=="[DONE]")try{It(r,JSON.parse(w))}catch{try{const k=JSON.parse(j);It(r,k)}catch{}}Be()}!r.content&&!r.reasoning&&(r.content=e("chat.emptyReply")),U("")}catch(C){C.name==="AbortError"?r.content=(r.content||"")+`
[${e("chat.stopped")}]`:(r.error=!0,r.content=(r.content||"")+`
✗ ${C.message||C}`,U(C.message||String(C)))}finally{r.streaming=!1,ye=null,Be(),lt(),f&&(f.disabled=!1),$&&($.disabled=!1),B&&(B.disabled=!1),h&&(h.disabled=!0),vt().catch(()=>{})}}const Xt="email@ysk.hk",So="https://github.com/sponsors/yanshekki",wo="https://linktr.ee/yanshekki",Po="https://ysk.hk/",Eo="https://github.com/yanshekki/ysk-omni#readme";async function Io(){const s=[[e("support.netEvm"),"yanshekki.eth"],[e("support.netNear"),"yanshekki.near"],[e("support.netAda"),"$yanshekki"]].map(([o,i])=>`
      <tr>
        <td>${t(o)}</td>
    ...
    <div class="support-stack">
      ...
      <section class="panel support-panel" id="support-sponsor">
        ...
        <div class="panel-pad">
          ...
          <div class="support-actions">
            <a class="btn" href="${So}" target="_blank" rel="noopener noreferrer">${t(e("support.githubSponsors"))}</a>
            <a class="btn secondary" href="${wo}" target="_blank" rel="noopener noreferrer">${t(e("support.linktree"))}</a>
          </div>
      ...
      <section class="panel support-panel" id="support-ysk">
        ...
        <div class="panel-pad">
          ...
          <a class="btn secondary sm" href="${Po}" target="_blank" rel="noopener noreferrer">${t(e("support.site"))}</a>
        </div>
      </section>
      <section class="panel support-panel" id="support-help">
        ...
        <div class="panel-pad">
          ...
          <p class="support-docs"><a href="${Eo}" target="_blank" rel="noopener noreferrer">${t(e("support.docs"))}</a></p>
        </div>
      </section>
    </div>
  `),le(),document.querySelectorAll("[data-jump]").forEach(o=>{o.addEventListener("click",()=>{const i=o.getAttribute("data-jump");i&&document.getElementById(i)?.scrollIntoView({behavior:"smooth",block:"start"})})}),document.querySelectorAll("[data-copy]").forEach(o=>{o.addEventListener("click",async()=>{const i=o.getAttribute("data-copy")||"",n=await Nt(i);o.textContent=e(n?"chat.copied":"support.copy"),setTimeout(()=>{o.textContent=e("support.copy")},1400)})})}const Mo=[{id:"Qwen/Qwen2.5-0.5B-Instruct-GGUF",modality:"text",runtime:"llamacpp",quants:["Q4_K_M","Q5_K_M","Q8_0"],vramMb:512},{id:"Qwen/Qwen2.5-7B-Instruct",modality:"text",runtime:"vllm",quants:[],vramMb:16e3},{id:"Tongyi-MAI/Z-Image-Turbo",label:"Z-Image-Turbo",modality:"image",runtime:"diffusion",quants:[],vramMb:8e3},{id:"black-forest-labs/FLUX.2-klein-4B",label:"FLUX.2 Klein 4B",modality:"image",runtime:"diffusion",quants:[],vramMb:8e3},{id:"Qwen/Qwen3-TTS",modality:"tts",runtime:"diffusion",quants:[],vramMb:4e3},{id:"Systran/faster-whisper-small",modality:"stt",runtime:"whisper",quants:[],vramMb:1e3},{id:"Lightricks/LTX-2.5",label:"LTX-2.5",modality:"video",runtime:"diffusion",quants:[],vramMb:12e3},{id:"Wan-AI/Wan2.2",label:"Wan 2.2",modality:"video",runtime:"diffusion",quants:[],vramMb:2e4}];function Ca(a){return a.label||String(a.id||"").split("/").pop()||a.id||""}function xo(a,s){const o=String(a.id||"");return(s||[]).filter(i=>i.id===o||i.repoId===o||String(i.id||"").startsWith(`${o}:`))}function La(a){const s=`catalog.mod.${a}`;return Ee(s)?e(s):a||"—"}function qo(a){const s=a||[];return s.includes("Q4_K_M")?"Q4_K_M":s[0]||""}async function Oe(){let a={};try{a=await M("/catalog")}catch(k){y(k)}const s=a.packs&&a.packs.length?a.packs:Mo,o=a.local||[],i=a.loaded||[],n=new Set(i.map(k=>k.id)),d=a.usedMb??0,l=a.budgetMb??0,u=l?Math.min(100,Math.round(d/l*100)):0,m=c.catalogTab==="local"||c.catalogTab==="packs"?c.catalogTab:"packs";c.catalogTab=m;const p=c.catalogModality||"",r=["text","image","video","tts","stt"],S=p?s.filter(k=>k.modality===p):s,b=c.catalogPulling||"",f=S.map(k=>{const P=xo(k,o),g=k.quants||[],v=qo(g),N=g.length?`<select class="catalog-quant-select" data-quant-for="${t(k.id)}">${g.map(H=>`<option value="${t(H)}" ${H===v?"selected":""}>${t(H)}</option>`).join("")}</select>`:`<span class="muted">${t(e("catalog.noQuant"))}</span>`,G=b===k.id,E=G?e("catalog.pulling"):P.length?e("catalog.pullAgain"):e("catalog.pull");return`
      <tr>
        <td>
          <div class="cell-primary">${t(Ca(k))}</div>
          <div class="cell-sub mono" title="${t(k.id)}">${t(k.id)}</div>
        </td>
        <td><span class="badge muted">${t(La(k.modality))}</span></td>
        <td><span class="badge muted">${t(k.runtime||"—")}</span></td>
        <td>${N}</td>
        <td class="catalog-vram-cell">${k.vramMb??0} MB</td>
        <td>${P.length?`<span class="badge success">${t(e("catalog.onDisk"))}</span>`:'<span class="muted">—</span>'}</td>
        <td>
          <div class="row-actions">
            <button type="button" class="btn ${P.length?"secondary":""} sm" data-pull="${t(k.id)}" ${G?"disabled":""}>${t(E)}</button>
            <span class="muted" data-pull-status="${t(k.id)}"></span>
          </div>
        </td>
      </tr>`}).join(""),h=`
    ...
    </td></tr>`,$=o.map(k=>{const P=n.has(k.id);return`
      <tr>
        <td>
          <div class="cell-primary mono">${t(k.id)}</div>
          <div class="cell-sub" title="${t(k.path||"")}">${t(k.path||"—")}</div>
        </td>
        <td class="catalog-vram-cell">${k.vramMb??0} MB</td>
        <td>${P?`<span class="badge success">${t(e("catalog.loaded"))}</span>`:`<span class="badge muted">${t(e("catalog.idle"))}</span>`}</td>
        <td>
          <div class="row-actions">
            <button type="button" class="btn sm" data-load="${t(k.id)}" data-vram="${k.vramMb??0}" ${P?"disabled":""}>${t(e("catalog.load"))}</button>
            <button type="button" class="btn secondary sm" data-unload="${t(k.id)}" ${P?"":"disabled"}>${t(e("catalog.unload"))}</button>
          </div>
        </td>
      </tr>`}).join(""),B=`
    ...
    </td></tr>`,C=i.map(k=>Ca({id:k.id})).join(", "),R=`
    ...
    </div>`,q=Le({title:e("catalog.filterModality"),hint:e("catalog.intro"),meta:A("common.pagerTotal",{n:S.length}),searchHtml:"",gridHtml:`
      <label>${t(e("catalog.filterModality"))}
        <select id="cat-mod">
          <option value="">${t(e("catalog.filterAll"))}</option>
          ${r.map(k=>`<option value="${t(k)}" ${p===k?"selected":""}>${t(La(k))}</option>`).join("")}
        </select>
      </label>`}),j=`
    <div class="panel data-table-panel">
      <div class="table-wrap">
  ...
  `),le(),document.querySelectorAll("[data-catalog-tab]").forEach(k=>{k.onclick=()=>{c.catalogTab=k.getAttribute("data-catalog-tab")||"packs",Oe().catch(y)}});const w=document.getElementById("cat-mod");w&&(w.onchange=()=>{c.catalogModality=w.value,Oe().catch(y)}),document.querySelector("[data-filter-apply]")?.addEventListener("click",()=>{c.catalogModality=document.getElementById("cat-mod")?.value||"",Oe().catch(y)}),document.querySelector("[data-filter-reset]")?.addEventListener("click",()=>{c.catalogModality="",Oe().catch(y)}),document.querySelectorAll("[data-pull]").forEach(k=>{k.onclick=async()=>{const P=k.getAttribute("data-pull")||"",g=document.querySelector(`[data-quant-for="${CSS.escape(P)}"]`),v=g&&g.value?g.value:"",N=v?`${P}:${v}`:P,G=document.querySelector(`[data-pull-status="${CSS.escape(P)}"]`);c.catalogPulling=P,k.disabled=!0,k.textContent=e("catalog.pulling");try{const E=await fetch(`${Ft}/catalog/pull`,{method:"POST",headers:{"Content-Type":"application/json",...c.key?{Authorization:`Bearer ${c.key}`}:{}},body:JSON.stringify({model:N})}),H=E.body&&E.body.getReader?E.body.getReader():null;let K="";if(H){const Q=new TextDecoder;for(;;){const{done:X,value:_}=await H.read();if(X)break;const I=Q.decode(_,{stream:!0});K+=I;const T=K.split(`
...
`).map(Q=>Q.trim()).filter(Boolean),oe=se.length?JSON.parse(se[se.length-1]):{};if(!E.ok)throw new Error(oe.error?.message||oe.reason||E.statusText);if(oe.status==="error")throw new Error(oe.reason||e("catalog.pullFail"));c.catalogPulling="",c.catalogTab="local",await Oe()}catch(E){c.catalogPulling="",k.disabled=!1,k.textContent=e("catalog.pull"),y(E)}}}),document.querySelectorAll("[data-load]").forEach(k=>{k.onclick=async()=>{try{await M("/models/load",{method:"POST",body:JSON.stringify({id:k.getAttribute("data-load"),vramMb:Number(k.getAttribute("data-vram")||0)})}),await Oe()}catch(P){y(P)}}}),document.querySelectorAll("[data-unload]").forEach(k=>{k.onclick=async()=>{try{await M("/models/unload",{method:"POST",body:JSON.stringify({id:k.getAttribute("data-unload")})}),await Oe()}catch(P){y(P)}}})}async function jt(){const a=document.getElementById("app");try{if(!c.key){await Wa();return}c.me||await Qa(),c.page==="dashboard"?await Zt():c.page==="chat"?await vo():c.page==="chats"?await mt():c.page==="keys"?await Re():c.page==="documents"?await st():c.page==="media"?await Ie():c.page==="catalog"?await Oe():c.page==="audit"?await pt():c.page==="settings"?await es():c.page==="apiFeatures"?await qt():c.page==="usage"?await me():c.page==="ddos"?await te():c.page==="queue"?await ce():c.page==="pm2"?await Ae():c.page==="system"?await ot():c.page==="support"?await Io():await Zt()}catch(s){a.innerHTML=ie(`<div class="error-box">${t(s.message)}</div>`),le()}}let Je=null;const W={tab:"overview",status:"",sortBy:"queuedAt",sortDir:"desc",limit:20,offset:0};function _t(a){return!a||a<0?"—":a<1e3?`${a}ms`:a<6e4?`${Math.round(a/1e3)}s`:a<36e5?`${Math.round(a/6e4)}m`:`${(a/36e5).toFixed(1)}h`}const To=["enabled","globalConcurrency","perKeyConcurrency","maxQueueDepth","maxQueueDepthPerKey","fairness","defaultPriority","playgroundPriority","leaseMs","maxWaitMs"];function rs(){return{relaxed:{enabled:!0,globalConcurrency:6,perKeyConcurrency:2,maxQueueDepth:200,maxQueueDepthPerKey:40,fairness:"weighted_round_robin",defaultPriority:100,playgroundPriority:40,leaseMs:6e4,maxWaitMs:9e5},balanced:{enabled:!0,globalConcurrency:4,perKeyConcurrency:1,maxQueueDepth:100,maxQueueDepthPerKey:20,fairness:"weighted_round_robin",defaultPriority:100,playgroundPriority:50,leaseMs:45e3,maxWaitMs:6e5},strict:{enabled:!0,globalConcurrency:2,perKeyConcurrency:1,maxQueueDepth:40,maxQueueDepthPerKey:8,fairness:"fifo_global",defaultPriority:100,playgroundPriority:80,leaseMs:3e4,maxWaitMs:3e5}}}function Da(a){if(!a)return{};const s={};for(const o of To){const i=a[o];typeof i=="boolean"?s[o]=i:typeof i=="number"&&Number.isFinite(i)?s[o]=Math.round(i):typeof i=="string"?s[o]=i:i==null?s[o]=null:s[o]=i}return s}function cs(a,s){return JSON.stringify(Da(a))===JSON.stringify(Da(s))}function la(a){if(!a)return"custom";const s=rs();for(const o of["relaxed","balanced","strict"])if(cs(a,s[o]))return o;return"custom"}function Ao(a){return e(a==="relaxed"?"queue.presetRelaxed":a==="balanced"?"queue.presetBalanced":a==="strict"?"queue.presetStrict":"queue.presetCustom")}function us(a,{unsaved:s=!1}={}){const o=Ao(a),i=a==="relaxed"?"relaxed":a==="balanced"?"balanced":a==="strict"?"strict":"custom",n=s?A("queue.presetFormLabel",{name:o}):A("queue.presetActiveLabel",{name:o});return`<span class="ddos-preset-badge is-${i}" id="queue-preset-badge" title="${t(n)}">${t(n)}</span>`}function ms(){return{enabled:document.getElementById("q-master-enabled")?Xe("q-master-enabled"):!0,globalConcurrency:Math.max(1,Math.min(64,Math.floor(z("qp-gconc",4)))),perKeyConcurrency:Math.max(1,Math.min(16,Math.floor(z("qp-kconc",1)))),maxQueueDepth:Math.max(1,Math.floor(z("qp-depth",100))),maxQueueDepthPerKey:Math.max(1,Math.floor(z("qp-depthk",20))),fairness:document.getElementById("qp-fair")?.value==="fifo_global"?"fifo_global":"weighted_round_robin",defaultPriority:Math.max(0,Math.min(1e3,Math.floor(z("qp-pri",100)))),playgroundPriority:Math.max(0,Math.min(1e3,Math.floor(z("qp-ppri",50)))),leaseMs:Math.max(5e3,Math.floor(z("qp-lease",45e3))),maxWaitMs:Math.max(5e3,Math.floor(z("qp-wait",6e5)))}}function Rt(a){Ye("q-master-enabled",a,e("queue.masterOn"),e("queue.masterOff")),et("queue-root",!a),Ze("queue-disabled-banner",!a);const s=document.getElementById("qk-pill-enabled");s&&(s.innerHTML=Ne(a,e("dash.on"),e("dash.off")))}function Bo(a){if(!a)return;const s=(i,n)=>{const d=document.getElementById(i);d&&(d.value=String(n))};Rt(a.enabled!==!1),s("qp-gconc",a.globalConcurrency),s("qp-kconc",a.perKeyConcurrency),s("qp-depth",a.maxQueueDepth),s("qp-depthk",a.maxQueueDepthPerKey);const o=document.getElementById("qp-fair");o&&(o.value=a.fairness||"weighted_round_robin"),s("qp-pri",a.defaultPriority),s("qp-ppri",a.playgroundPriority),s("qp-lease",a.leaseMs),s("qp-wait",a.maxWaitMs),gt()}function gt(){if(!document.getElementById("queue-policy-panel"))return;let a;try{a=ms()}catch{return}const s=la(a),o=la(c._queuePolicyCache||a),i=!cs(a,c._queuePolicyCache||a);document.querySelectorAll("[data-queue-preset]").forEach(l=>{const u=l.dataset.queuePreset;if(u==="custom"){const S=s==="custom";l.classList.toggle("is-active",S),l.setAttribute("aria-pressed",S?"true":"false"),l.disabled=!S;return}const m=u===s,p=u===o;l.classList.toggle("is-active",m),l.classList.toggle("is-saved",p&&!m),l.setAttribute("aria-pressed",m?"true":"false");const r=e(u==="relaxed"?"queue.presetRelaxed":u==="balanced"?"queue.presetBalanced":"queue.presetStrict");m&&p?l.innerHTML=`${t(r)} <span class="preset-tag">${t(e("queue.presetTagActive"))}</span>`:m&&i?l.innerHTML=`${t(r)} <span class="preset-tag preset-tag--draft">${t(e("queue.presetTagDraft"))}</span>`:p?l.innerHTML=`${t(r)} <span class="preset-tag preset-tag--saved">${t(e("queue.presetTagSaved"))}</span>`:l.textContent=r});const n=document.getElementById("queue-preset-badge");n&&(n.outerHTML=us(s,{unsaved:i&&s!==o}));const d=document.getElementById("queue-preset-hint");if(d){const l={relaxed:e("queue.presetRelaxedHint"),balanced:e("queue.presetBalancedHint"),strict:e("queue.presetStrictHint"),custom:e("queue.presetCustomHint")};d.textContent=l[s]||l.custom}}function Co(){document.querySelectorAll("[data-queue-preset]").forEach(a=>{a.dataset.queuePreset!=="custom"&&(a.onclick=()=>{const s=a.dataset.queuePreset,o=rs()[s];o&&Bo(o)})}),["qp-gconc","qp-kconc","qp-depth","qp-depthk","qp-fair","qp-pri","qp-ppri","qp-lease","qp-wait"].forEach(a=>{const s=document.getElementById(a);s&&(s.addEventListener("change",()=>gt()),s.addEventListener("input",()=>gt()))}),gt()}function Ha(){return document.querySelector(".main")}function ps(a){return a.map(s=>{const o=s.status==="queued"||s.status==="leased"||s.status==="running",i=s.status==="failed"||s.status==="dead"||s.status==="cancelled",n=s.startedAt||s.finishedAt?null:s.queuedAt?Date.now()-new Date(s.queuedAt).getTime():null;return`
    <tr data-q-row="${t(s.id)}">
      ...
      <td>
    ...
    </tr>`}).join("")}function fs(){document.querySelectorAll("[data-q-cancel]").forEach(a=>{a.onclick=async()=>{await Z({title:e("queue.cancel"),message:e("queue.cancelConfirm"),variant:"danger",confirmText:e("queue.cancel")})&&(await M(`/queue/jobs/${a.dataset.qCancel}/cancel`,{method:"POST",body:"{}"}),ce().catch(y))}}),document.querySelectorAll("[data-q-requeue]").forEach(a=>{a.onclick=async()=>{await M(`/queue/jobs/${a.dataset.qRequeue}/requeue`,{method:"POST",body:"{}"}),ce().catch(y)}}),document.querySelectorAll("[data-q-pri]").forEach(a=>{a.onclick=async()=>{const s=Number(a.dataset.pri)||100,o=window.prompt(e("queue.priorityPh"),String(s));if(o==null)return;const i=Number(o);!Number.isFinite(i)||i<0||i>1e3||(await M(`/queue/jobs/${a.dataset.qPri}/priority`,{method:"POST",body:JSON.stringify({priority:i})}),ce().catch(y))}})}function gs(a){return a.enabled?a.paused?e("queue.paused"):a.drainMode?e("queue.drain"):e("queue.running"):e("queue.modeOff")}function Lo({s:a,pol:s,jobs:o,total:i,by:n}){const d=a.dead??n.dead??0,l=a.leased??n.leased??0,u=a.running??n.running??0,m=a.queued??n.queued??0,p=a.depth??m+l+u,r=gs(s),S=s.fairness==="fifo_global"?e("queue.fifo"):e("queue.wrr"),b=(P,g)=>{const v=document.getElementById(P);v&&(v.textContent=g)},f=(P,g)=>{const v=document.getElementById(P);v&&(v.innerHTML=g)};b("qk-depth",String(p)),b("qk-depth-sub",A("queue.kpiDepthSub",{q:m,l})),f("qk-running",`${u}<span class="dash-kpi-den">/${s.globalConcurrency??"—"}</span>`),b("qk-running-sub",A("queue.kpiActiveSub",{n:a.workerActive??0})),b("qk-queued",String(m)),b("qk-dead",String(d)),b("qk-oldest",a.oldestQueuedAgeMs?_t(a.oldestQueuedAgeMs):"—"),b("qk-mode",r),b("qk-mode-sub",S);const h=document.getElementById("qk-worker-id");if(h){const P=a.workerId||"—";h.textContent=P,h.title=P}const $=(P,g,v,N)=>{const G=document.getElementById(P);G&&(G.outerHTML=`<span id="${P}">${Ne(g,v,N)}</span>`)};$("qk-pill-enabled",s.enabled!==!1,e("dash.on"),e("dash.off")),$("qk-pill-consumer",!s.paused&&s.enabled!==!1,e("queue.running"),s.paused?e("queue.paused"):e("queue.modeOff")),$("qk-pill-admission",!s.drainMode,e("queue.accepting"),e("queue.drain")),b("qk-fairness-val",S),b("qk-conc-val",`${s.perKeyConcurrency??1} / ${s.globalConcurrency??"—"}`);const B=document.getElementById("queue-dlq-slot");B&&(d>0?(B.innerHTML=`
        ...
        </div>`,document.getElementById("q-filter-dead")?.addEventListener("click",()=>{W.status="dead",W.offset=0,W.tab="jobs",ce().catch(y)})):B.innerHTML="");const C=(P,g)=>{const v=document.getElementById(P);v&&(v.textContent=String(g??0))};C("q-tab-count-jobs",i),C("q-tab-count-dead",d);const R=document.getElementById("qk-jobs-meta");R&&(R.textContent=A("queue.jobsMeta",{n:i}));const q=document.querySelector("#queue-jobs-table tbody");if(q){const P=o.map(v=>`${v.id}|${v.status}|${v.priority}|${v.attempt}|${v.cancelRequested?1:0}|${v.errorMessage||""}|${v.startedAt||""}|${v.finishedAt||""}`).join(";"),g=ps(o)||`<tr class="empty-row"><td colspan="9">
        <div class="data-empty">
          <div class="data-empty-icon">∅</div>
          <strong>${t(e("queue.empty"))}</strong>
        </div>
      </td></tr>`;if(q.dataset.qsig!==P){const v=document.querySelector("#queue-jobs-table .table-wrap"),N=v?.scrollLeft||0;q.dataset.qsig=P,q.innerHTML=g,fs(),ya(document.querySelector("#queue-jobs-table")||document),v&&(v.scrollLeft=N)}else o.forEach(v=>{if(v.status!=="queued"||!v.queuedAt)return;const N=Date.now()-new Date(v.queuedAt).getTime(),G=String(v.id||"");let E=null;q.querySelectorAll("[data-q-row]").forEach(K=>{K.getAttribute("data-q-row")===G&&(E=K)});const H=E?.querySelector("[data-q-wait]");H&&(H.textContent=`${e("queue.wait")}: ${_t(N)}`)})}if(document.querySelector("#queue-pager .data-pager-meta span")){const P=Math.max(1,Math.ceil((i||0)/W.limit)||1),g=Math.floor(W.offset/W.limit)+1,v=document.querySelectorAll("#queue-pager .data-pager-meta > span");v[0]&&(v[0].textContent=A("common.pagerTotal",{n:i||0})),v[1]&&(v[1].textContent=A("common.pagerPage",{n:g,total:P}));const N=document.getElementById("queue-prev"),G=document.getElementById("queue-next");N&&(N.disabled=W.offset<=0),G&&(G.disabled=W.offset+W.limit>=i)}const x=document.getElementById("q-pause");x&&(x.textContent=s.paused?e("queue.resume"):e("queue.pause"));const w=document.getElementById("q-drain");w&&(w.textContent=s.drainMode?e("queue.undrain"):e("queue.drainBtn"));const k=document.getElementById("q-master-enabled");k&&document.activeElement!==k&&Rt(s.enabled!==!1)}function Oa(){Je||(Je=setInterval(()=>{if(c.page!=="queue"){clearInterval(Je),Je=null;return}const a=document.activeElement;a&&a.closest&&a.closest("#queue-policy-panel")&&(a.tagName==="INPUT"||a.tagName==="SELECT"||a.tagName==="TEXTAREA")||ce({soft:!0}).catch(()=>{})},4e3))}async function ce(a={}){const s=!!a.soft&&document.getElementById("queue-root");!s&&Je&&(clearInterval(Je),Je=null);const o=Ha(),i=!s&&o?o.scrollTop:0,n=W;n.sortBy||(n.sortBy="queuedAt"),n.sortDir||(n.sortDir="desc");const d=new URLSearchParams;d.set("limit",String(n.limit)),d.set("offset",String(n.offset)),n.status&&d.set("status",n.status),ke(d,n);const[l,u,m]=await Promise.all([M("/queue/stats"),M(`/queue/jobs?${d}`),M("/queue/policy")]);if(c.page!=="queue")return;const p=l.data||{},r=m.data||p.policy||{},S=u.data||[],b=u.total??S.length,f=p.byStatus||{},h=p.dead??f.dead??0,$=p.leased??f.leased??0,B=p.running??f.running??0,C=p.queued??f.queued??0,R=p.depth??C+$+B,q=gs(r);if(c._queuePolicyCache={...r},s){Lo({s:p,pol:r,jobs:S,total:b,by:f}),Oa();return}W.tab||(W.tab="overview");const j=W.tab==="jobs"||W.tab==="policy"?W.tab:"overview";W.tab=j;const x=ps(S),w=Le({title:e("queue.filterTitle"),hint:e("queue.filterHint"),meta:A("queue.jobsMeta",{n:b}),gridHtml:`
      ...
      <th>${t(e("common.actions"))}</th>`,bodyHtml:x,colSpan:9,emptyText:e("queue.empty"),pagerHtml:Te({total:b,limit:n.limit,offset:n.offset,idPrefix:"queue"})}),P=r.fairness==="fifo_global"?e("queue.fifo"):e("queue.wrr"),g=r.enabled!==!1,v=(Q,X,_,I,T)=>`
    <div class="card">
    ...
    ${w}
    <div id="queue-jobs-table" class="queue-jobs-table-host">${k}</div>`,H=`
  ...
  `),le(),document.querySelectorAll("[data-queue-tab]").forEach(Q=>{Q.addEventListener("click",()=>{const X=Q.getAttribute("data-queue-tab")||"overview";X!=="overview"&&X!=="jobs"&&X!=="policy"||W.tab!==X&&(W.tab=X,ce().catch(y))})}),i>0){const Q=Ha();Q&&(Q.scrollTop=i,requestAnimationFrame(()=>{Q.scrollTop=i}))}document.getElementById("q-master-enabled").onclick=async()=>{const Q=!Xe("q-master-enabled");Rt(Q);try{const X=await M("/queue/policy",{method:"PUT",body:JSON.stringify({enabled:Q})});c._queuePolicyCache={...c._queuePolicyCache||{},...X.data||{enabled:Q}},gt()}catch(X){Rt(!Q),y(X)}},document.getElementById("q-pause").onclick=async()=>{await M(r.paused?"/queue/resume":"/queue/pause",{method:"POST",body:"{}"}),ce().catch(y)},document.getElementById("q-drain").onclick=async()=>{await M(r.drainMode?"/queue/undrain":"/queue/drain",{method:"POST",body:"{}"}),ce().catch(y)};let K=!1;const se=async()=>{if(!K){K=!0;try{if(!await Z({title:e("queue.purgeTitle"),message:e("queue.purgeConfirm"),variant:"danger",confirmText:e("queue.purgeConfirmBtn"),cancelText:e("common.cancel")}))return;const X=await M("/queue/purge-dead",{method:"POST",body:"{}"}),_=Number(X?.data?.deleted??0);await pe({title:e("queue.purgeDoneTitle"),message:A("queue.purgeDoneMsg",{n:_}),confirmText:e("common.ok")}),await ce()}finally{K=!1}}},oe=document.getElementById("queue-root");oe&&(oe.onclick=Q=>{Q.target?.closest?.("#q-purge, #q-purge-dlq")&&(Q.preventDefault(),se().catch(y))}),document.getElementById("q-filter-dead")?.addEventListener("click",()=>{W.status="dead",W.offset=0,W.tab="jobs",ce().catch(y)}),document.querySelectorAll("[data-filter-apply]").forEach(Q=>{Q.onclick=()=>{W.status=document.getElementById("qf-status")?.value||"",W.offset=0,ce().catch(y)}}),document.querySelectorAll("[data-filter-reset]").forEach(Q=>{Q.onclick=()=>{W.status="",W.sortBy="queuedAt",W.sortDir="desc",W.offset=0,ce().catch(y)}}),dt("queue",W,()=>ce().catch(y)),Ue(W,()=>ce().catch(y)),document.getElementById("qp-save").onclick=async()=>{const Q=ms();await M("/queue/policy",{method:"PUT",body:JSON.stringify(Q)}),c._queuePolicyCache={...c._queuePolicyCache||{},...Q},U(""),ce().catch(y)},Co(),fs(),Oa()}c.page=$s();(!location.hash||location.hash==="#"||location.hash==="#/")&&ca(c.page);window.addEventListener("hashchange",()=>{const a=ra(location.hash);a&&a!==c.page&&ma(a,{writeHash:!1})});window.addEventListener("popstate",()=>{const a=ra(location.hash);!a||a===c.page||ma(a,{writeHash:!1})});jt();
